...
| `react`             | `@copilotkit/react`             | React hooks (`useAgent`, `useFrontendTool`, `useAgentContext`, etc.) and `CopilotKitProvider` (apps should use the `CopilotKit` provider from `@copilotkit/react-core/v2`, a superset of it). Thin wrappers that register/unregister with Core. |
