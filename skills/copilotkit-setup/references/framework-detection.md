...
- **Uses:** `@copilotkit/runtime` (imports from `@copilotkit/runtime/v2`)
- **Does NOT need:** `@copilotkit/react-core`
- **Endpoint factory:** `createCopilotExpressHandler` from `@copilotkit/runtime/v2/express` -- multi-route by default, or `mode: "single-route"`
- **Env file:** `.env` (loaded via `dotenv`)

### Standalone Backend (Hono)

- **Same as Express** but uses `createCopilotHonoHandler` from `@copilotkit/runtime/v2` (multi-route by default, or `mode: "single-route"`)
