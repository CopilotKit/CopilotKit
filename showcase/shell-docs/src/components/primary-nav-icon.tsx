...
export type PrimaryNavIconKind = "docs" | "reference" | "cookbook";

interface PrimaryNavIconProps {
  active: boolean;
  className?: string;
  kind: PrimaryNavIconKind;
}
    ...
    <defs>
      ...
      >
        <stop
          offset="0%"
          style={{
            stopColor: "var(--shell-docs-brand-gradient-start)",
          }}
        />
        <stop
          offset="48%"
          style={{
            stopColor: "var(--shell-docs-brand-gradient-mid)",
          }}
        />
        <stop
          offset="100%"
          style={{
            stopColor: "var(--shell-docs-brand-gradient-end)",
          }}
        />
      </linearGradient>
    </defs>
...
export function PrimaryNavIcon({
  ...
  const gradientId = `shell-docs-nav-icon-${reactId}`;
  const paint = active ? `url(#${gradientId})` : "currentColor";
  const svgClassName = ["shell-docs-nav-link-icon", className]
    .filter(Boolean)
    .join(" ");
  ...
  if (kind === "reference") {
    return (
      <svg
        ...
        aria-hidden="true"
      >
        {active ? <GradientDefs id={gradientId} /> : null}
        <path
