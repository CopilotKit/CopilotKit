...
test("prefers Vue variants before backend-authored content", () => {
  ...
  expect(resolution).toContain("activeFrontendPage");
  expect(resolution).toContain("docsMode");
  expect(resolution).toContain("docsFolder");
  expect(resolution).toContain("slugPath");
  expect(resolution).toContain("contentSlugPath = candidate");
});
