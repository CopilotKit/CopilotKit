...
describe("buildAngularBackendOverview", () => {
  ...
  it("derives the Vue backend overview exclusively from runnable Vue catalog cells", () => {
    ...
    const runnableVueCells = catalogCells.filter(
      (cell) =>
        cell.frontend === "vue" &&
        cell.integration === "langgraph-python" &&
        cell.runnable,
    );
    const langgraphPython = registryData.integrations.find(
      (integration) => integration.slug === "langgraph-python",
    )!;
    ...
    expect(overview.supportedFeatures[0]?.demoLink).toBe(
      `${langgraphPython.backend_url}/vue/agentic-chat`,
    );
