...
function selfManagedText(frontend: "react" | "vue") {
  const loadSlug =
    frontend === "vue"
      ? "frontends/vue/threads-self-managed"
      : "threads-self-managed";
  const doc = loadDoc(loadSlug)!;
  return renderPageToLlmText({
    loadSlug,
    url: frontend === "vue" ? "vue/threads-self-managed" : loadSlug,
...
test("the framework-neutral thread routes reuse the canonical shared pages", () => {
  ...
  expect(selfManaged).toContain("@/snippets/shared/threads/self-managed.mdx");
  for (const frontend of ["react", "vue"] as const) {
    const text = selfManagedText(frontend);
    expect(text).toContain("my own database");
    expect(text).toContain("AsyncPostgresSaver.from_conn_string(DB_URL)");
    expect(text).toContain("Scope every query to the signed-in user");
    expect(text).toContain(
      frontend === "vue" ? "@copilotkit/vue/v2" : "@copilotkit/react-core/v2",
    );
    expect(text).not.toContain(
      frontend === "vue" ? "@copilotkit/react-core/v2" : "@copilotkit/vue/v2",
    );
    expect(text).not.toContain("<RestoreHistory components=");
  }
});
...
test("the Vue docs point Vue-owned routes at the explicit /vue path", () => {
  ...
  const selfManaged = selfManagedText("vue");
  const sharedState = sourceFor("frontends/vue/shared-state");
...
test("the Vue thread docs distinguish store notification from active-thread changes", () => {
  const reference = readFileSync(
    new URL(
      "../../content/reference/vue/hooks/useThreads.mdx",
      import.meta.url,
    ),
    "utf8",
  );
  const lifecycle = sourceFor("frontends/vue/threads-lifecycle");

  expect(reference).toContain("clears stale\n    thread-list error state");
  expect(reference).toContain(
    "It does not mint or switch the active chat thread",
  );
  expect(lifecycle).toContain(
    "must call this configuration method to mint and switch the active thread",
  );
  expect(lifecycle).toContain(
    "also call `useThreads().startNewThread()` to clear stale thread-store error state",
  );
  expect(lifecycle).not.toContain(
    "also available from `useThreads()` for custom sidebars",
  );
});
