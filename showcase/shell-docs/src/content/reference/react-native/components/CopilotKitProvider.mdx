...
<Callout type="info">
  Do not put `publicLicenseKey` or another browser license token in a React
  Native bundle. For cloud-hosted Intelligence, point `runtimeUrl` at your
  public app Runtime; that server reports the managed entitlement and keeps
  hosted project credentials off the device. You can also point it at a
  self-hosted Runtime.
</Callout>
