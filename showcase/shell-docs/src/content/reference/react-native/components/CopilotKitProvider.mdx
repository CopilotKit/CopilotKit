...
- **Shared render-tool registry**: the provider instantiates the CopilotKit client whose renderer registry is shared with the rest of CopilotKit. Render functions registered by [`useRenderTool`](/reference/react-native/hooks/useRenderTool) (or [`useComponent`](/reference/react-native/hooks/useComponent)) write into that registry, so [`useRenderTool`](/reference/react-native/hooks/useRenderTool) and [`useRenderToolCall`](/reference/react-native/hooks/useRenderTool#rendering-a-tool-call-outside-the-chat) work anywhere below the provider — there is no separate `RenderToolProvider` wrapper.
