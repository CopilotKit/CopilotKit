...
| Your call | Routed to | Warns in development |
| --- | --- | --- |
| `name: "*"`, whatever else the config carries | react-core's `useRenderTool` — a **renderer-only** fallback | yes, if `description`, `handler` or `parameters` is present (all three are ignored on this path) |
| any other name, carrying `description` or `handler` | `useFrontendTool` — the tool **and** its renderer, which is what the old hook did | yes |
| any other name, carrying neither | react-core's `useRenderTool` | no — this is the call this page documents |
...
named `*` is registered or advertised to the model, and that a real frontend tool
needs a real name and `useFrontendTool`.
...
| renderer-only registration: impossible (`description` was required) | `useRenderTool({ name, parameters, render, agentId? }, deps)` — `parameters` required on a named renderer | works, and is silent |
...
**literally named** `*`, with no description and no schema, and advertised it to
the model on every run. Through the shim, and on core's hook afterwards, `"*"` is
what it is on the web: a schema-less fallback renderer that registers no tool at
all.
...
The routing is unconditional, but the notice is not. Four gaps, all worth
knowing before you treat a quiet console as "nothing to migrate":
...
- **Only when an old field is actually present.** A call with neither
  `description` nor `handler` is routed to react-core's `useRenderTool` and says
