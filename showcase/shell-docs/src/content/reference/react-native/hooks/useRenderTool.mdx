...
`ToolCallStatus.Complete`, so narrow with the enum members
(`status === ToolCallStatus.Executing`) and not with the raw strings. Assigning a
bare `"executing"` where a `status` is expected is a type error (TS2322).
...
<PropertyReference name="status" type="ToolCallStatus">
  Current tool-call status, as a member of the `ToolCallStatus` enum
  (`@copilotkit/core`, re-exported from `@copilotkit/react-native`):
  `ToolCallStatus.InProgress` while arguments stream,
  `ToolCallStatus.Executing` while the handler runs, and
  `ToolCallStatus.Complete` once the result is available.

  Compare against the enum members, not the underlying strings. The members carry
  those strings at runtime, but TypeScript does not accept a raw `"executing"`
  where a `status` is expected (TS2322), so `status === ToolCallStatus.Executing`
  is the form that type-checks.
</PropertyReference>
...
- they declare `status` as the string literals `"inProgress"` / `"executing"` /
  `"complete"`, where React Native's `status` is the `ToolCallStatus` **enum**;
...
- **Arguments stream.** While the agent is still writing the call, `status` is `ToolCallStatus.InProgress` and `args` is partial — fields arrive progressively. Write renderers that tolerate missing fields; that is what lets UI build as the agent writes it.
...
function ToolCallStage({ agentId = "default" }: { agentId?: string }) {
  ...
  // Pair each call with its result message. `renderToolCall` derives status from
  // that pairing: pass `toolMessage` and the call resolves to
  // `ToolCallStatus.Complete` with `result`; omit it and `status` stays
  // `ToolCallStatus.InProgress` and `result` is `undefined` forever — the same
  // correlation the prebuilt chat does.
...
Render props also change. They gain `name` and `toolCallId`, and `status` changes
in two ways at once. It was the two-member string-literal union `"executing" |
"complete"`; it is now the three-member **`ToolCallStatus` enum**
(`ToolCallStatus.InProgress` was added, for the state where the agent is still
streaming the call). Both halves of that are breaking: string comparisons no
longer type-check, so `status === "complete"` becomes
...
while in progress. `result` is narrowed the other way: it was `string |
undefined` in every state, and is now `undefined` unless `status` is
