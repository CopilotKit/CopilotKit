...
<PropertyReference name="credentials" type="Signal<RequestCredentials | undefined>">
  The fetch credentials mode currently used for runtime requests.
</PropertyReference>

<PropertyReference name="intelligence" type="Signal<IntelligenceRuntimeInfo | undefined>">
  Runtime intelligence metadata, including the websocket URL used for realtime
  thread updates when the connected runtime provides one.
</PropertyReference>

<PropertyReference name="threadEndpoints" type="Signal<ThreadEndpointRuntimeInfo | undefined>">
  Thread endpoint capabilities reported by the runtime. `injectThreads()` uses
  this to disable unsupported list, mutation, inspection, or realtime metadata
  calls.
</PropertyReference>
...
<PropertyReference name="updateRuntime" type="(options) => void">
  ...
  signals. Pass `credentials: undefined` to clear a previously configured
  credentials mode.
</PropertyReference>
