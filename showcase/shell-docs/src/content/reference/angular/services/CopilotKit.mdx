...
<PropertyReference name="audioFileTranscriptionEnabled" type="Signal<boolean>">
  Whether the connected runtime's `/info` response advertises audio file
  transcription support. It remains `false` until the runtime reports the
  capability.
</PropertyReference>
