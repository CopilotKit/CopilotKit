...
- **WebMCP registration.** With `webmcp` set, the tool is also registered on `document.modelContext` for as long as the owning injector is alive. The tool needs a `description` for this (WebMCP rejects tools without one). To import page tools into CopilotKit, use [`registerWebmcpTools`](/reference/angular/functions/registerWebmcpTools).
