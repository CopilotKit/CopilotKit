...
<PropertyReference name="config" type="CopilotKitConfig" required>
  ...
  <PropertyReference name="runtimeTransport" type={`"auto" | "rest" | "single"`}>
    Runtime request transport. Defaults to `"auto"`; set `"rest"` or `"single"`
    when your deployment requires a deterministic transport from bootstrap.
  </PropertyReference>
  ...
  <PropertyReference name="credentials" type="RequestCredentials">
    Fetch credentials mode used for runtime requests. Set this to `"include"`
    when your runtime relies on cookies or other browser-managed credentials,
    especially for cross-origin thread and agent requests.
  </PropertyReference>
