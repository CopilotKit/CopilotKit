...
<PropertyReference name="subagentComponent" type="Type<any>">
  Component class used for each subagent group. It gets the inputs `subagentRunId`, `subagent`, `messages`, `body` and `bodyContext` that it declares.
</PropertyReference>

<PropertyReference name="subagentTemplate" type="TemplateRef<any>">
  Template used for each subagent group. Takes precedence over `subagentComponent`.
  The context has the subagent as `$implicit`, plus `subagentRunId`, `messages`, `body` and `bodyContext`.
