...
<PropertyReference name="onLicensed" type="() => void">
  Called when the user selects the locked view's Upgrade action. Use it to
  record the action or handle navigation yourself.
</PropertyReference>

<PropertyReference name="licenseUrl" type="string">
  Destination opened in a new tab by the locked view's Upgrade action. Omit it
  to use the built-in CopilotKit Intelligence docs URL. Set it to an empty
  string to suppress default navigation and handle the action with
  `onLicensed`.
</PropertyReference>
...
- **Mobile**: at 768px or below the drawer is an off-canvas modal. Closed, it shows the same floating cluster (a launcher to open it + a "New Conversation" button); open, it slides in and is dismissed by tapping the backdrop or pressing Escape. Customize the launcher position with the `--cpk-drawer-launcher-*` variables.
