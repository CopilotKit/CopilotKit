...
The chat it pairs with must be the v2 `CopilotChat` or `CopilotSidebar` from `@copilotkit/react-core/v2`. The same names exist in `@copilotkit/react-ui`, but those [deprecated v1 components](/migrate/v2) never mount `CopilotChatConfigurationProvider`, so they cannot drive the drawer: it renders and lists threads, and selecting a row changes nothing.
