...
- **Persistent sidebar**: on larger screens the drawer is a persistent, always-visible sidebar — there is no collapse control. The header region is empty by default and renders only when you project content into `slot="header"`; "New Conversation" is its own row below it.
...
- **Mobile**: below 768px the drawer is an off-canvas modal opened via its own floating launcher and dismissed by tapping the backdrop or pressing Escape; the launcher icon (`launcher-icon` slot) and position (`--cpk-drawer-launcher-*`) are customizable.
