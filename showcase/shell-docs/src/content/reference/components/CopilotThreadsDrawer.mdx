...
`CopilotThreadsDrawer` is a prebuilt thread switcher. Dropped in next to [`CopilotChat`](/reference/components/CopilotChat), it lists the user's conversations and lets them switch, start, rename, archive, and delete threads — with **no active-thread state to wire up**. It is a thin React wrapper around the framework-agnostic `<copilotkit-threads-drawer>` shadow-DOM web component, fed by the [`useThreads`](/reference/hooks/useThreads) hook and the surrounding chat configuration.
...
<PropertyReference name="onCollapseChange" type="(collapsed: boolean) => void">
  Optional callback fired when the user collapses or expands the drawer. The
  element emits a `collapse-change` CustomEvent (`detail: { collapsed: boolean }`)
  that the wrapper routes here. Hosts use it to reclaim the drawer's reserved
  layout space when it collapses — for example, shrinking a fixed grid track to
  `auto` so the rest of the UI expands.
</PropertyReference>
...
<PropertyReference name="collapsible" type="boolean" default="true">
  Whether the drawer can be collapsed. When `true` (default), the header shows a
  collapse toggle that hides the drawer down to a floating `collapsed-cluster`.
  Set to `false` to remove the toggle and keep the drawer always expanded.
</PropertyReference>
...
| Slot | Replaces |
| --- | --- |
| `header` | The drawer header region (empty by default; the redesigned header is an icon row with the collapse toggle). |
...
Style structural elements with `::part()`: `root`, `header`,
`collapse-toggle`, `new-thread-button`, `section-heading`, `filters`,
`filter-toggle`, `filter-indicator`, `filter-active`, `filter-all`,
`list`, `row-name`, `row-menu`, `row-menu-popover`,
`row-archive`, `row-unarchive`, `row-delete`, `collapsed-cluster`, `loading`,
`empty`, `error`, `retry-button`, `footer`, `memories`, `launcher`,
`confirm-dialog`, `confirm-delete`, `confirm-cancel`.

The header's icon row exposes `collapse-toggle`, which collapses the drawer to a
floating `collapsed-cluster` (with expand and new-conversation icons). The
Active/All filter lives behind the `filter-toggle` funnel, whose
...
- **Header**: the header is a compact icon row with a single **collapse/sidebar** toggle (unless `collapsible={false}`). "New Conversation" is its own row below the header. Collapsing the drawer leaves a floating cluster (`collapsed-cluster`) with expand and new-conversation icons, and emits `collapse-change` / `onCollapseChange` so the host can reclaim the reserved layout space.
