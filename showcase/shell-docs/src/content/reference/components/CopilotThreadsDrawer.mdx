...
<PropertyReference name="onSearch" type="(query: string) => void">
  Optional callback fired as the user types in the drawer's client-side search
  box. The element emits a `search` CustomEvent (`detail: { query: string }`)
  that the wrapper routes here. Search filters the rendered thread list on the
  client.
</PropertyReference>
...
| Slot | Replaces |
| --- | --- |
| `header` | The drawer header region (empty by default; the redesigned header is an icon row of toggles). |
...
Style structural elements with `::part()`: `root`, `header`, `search-toggle`,
`collapse-toggle`, `new-thread-button`, `section-heading`, `filters`,
`filter-toggle`, `filter-indicator`, `filter-active`, `filter-all`,
`search-input`, `list`, `row-name`, `row-menu`, `row-menu-popover`,
...
The header's icon row exposes `search-toggle` (toggles the `search-input`) and
`collapse-toggle` (collapses the drawer to a floating `collapsed-cluster`). The
...
- **Header**: the header is a compact icon row — a **search** toggle and a **collapse/sidebar** toggle. "New Conversation" is its own row below the header. Collapsing the drawer leaves a floating cluster (`collapsed-cluster`) with expand and new-conversation icons.
- **Search**: the search toggle reveals a client-side search box that filters the rendered list as you type (emitting the `search` event / `onSearch`).
