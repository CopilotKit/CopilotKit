...
<PropertyReference name="subagent" type="SlotProp<typeof CopilotChatSubagent>">
  Slot for the group that shows the work of one subagent. Defaults to `CopilotChatSubagent`, which starts collapsed to a header with the name and status of the subagent, and opens on a click.
  The slot gets `subagentRunId`, `subagent` (undefined when the agent did not announce it), `messages`, and `children` with the rendered messages of the group.
...
```tsx
<CopilotChatMessageView subagent="bg-muted rounded-lg" />
```
