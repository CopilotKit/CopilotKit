...
A Channel has no public `start()` method. Creating the Node listener starts the
Channel and opens its Realtime Gateway connection. `ready()` is optional and
purely await-and-observe: use it when startup should fail if activation does not
reach a usable status before the timeout.
...
The generic Fetch handler does not own a long-running Node process, so its host
must start and stop Channels through the host's lifecycle. For the Node listener
shown here, `ready()` reports the initial `online` or `setup_required` outcome
once; inspect `status()` before accepting traffic. Observe later drops and
reconnects through `status()`.

| Status           | Meaning                                                          |
| ---------------- | ---------------------------------------------------------------- |
| `connecting`     | Listener-driven activation is in progress.                   |
