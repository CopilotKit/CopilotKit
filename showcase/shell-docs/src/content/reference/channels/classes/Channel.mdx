...
| `agent`          | `AbstractAgent \| (threadId: string) => AbstractAgent` | Default agent instance or factory. Used when `runAgent` omits `agentId`. Every turn clones the configured result.           |
| `agents`         | `Record<string, AbstractAgent \| (threadId: string) => AbstractAgent>` | Named extra agents. `"default"` is the no-name agent when `agent` is omitted. Do not pass both `agent` and `agents.default`. |
