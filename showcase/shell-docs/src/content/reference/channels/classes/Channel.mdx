...
| `onWelcome(handler)`       | `{ thread, user?, platform }` | Runs once for a supported provider installation or conversation activation. No handler means no welcome output.                |
...
`status()` before accepting managed traffic. A configured direct adapter still
starts when managed setup is incomplete, while the Channel remains
`setup_required` until its managed providers are configured. Observe later
drops and reconnects through `status()`.
