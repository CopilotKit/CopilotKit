...
| `agent`          | `AbstractAgent \| (threadId) => AbstractAgent` | Required when calling `thread.runAgent()`. Not inherited from `runtime.agents`. Prefer a factory; each turn clones its configured agent.                                 |
