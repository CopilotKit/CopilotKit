...
| `agent`          | `AbstractAgent \| (threadId) => AbstractAgent` | Default agent. Prefer a factory. Every turn clones its configured agent, including the agent returned by a factory.                   |
| `agents`         | `Record<string, AbstractAgent \| (threadId) => AbstractAgent>` | Named extra agents. `"default"` is the no-name agent when `agent` is omitted. Do not pass both `agent` and `agents.default`. |
