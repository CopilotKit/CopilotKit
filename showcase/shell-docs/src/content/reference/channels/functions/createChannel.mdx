...
| `agent`          | `AbstractAgent \| (threadId) => AbstractAgent` | Required for `runAgent()`. There is no fallback to the runtime's `agents` map; without `agent`, every `runAgent()` call throws. Handlers can still reply through `thread.post()`. Prefer a factory. Every turn clones its configured agent, including the agent returned by a factory. |
