...
| `agent`          | `AbstractAgent \| (threadId) => AbstractAgent` | Required for the Channel to answer. There is no fallback to the runtime's `agents` map; without `agent`, every `runAgent()` call throws. Prefer a factory. Every turn clones its configured agent, including the agent returned by a factory. |
