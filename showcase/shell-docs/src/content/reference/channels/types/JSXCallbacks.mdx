...
<Callout type="warn" title="Managed Teams control boundary">
  Managed Teams currently dispatches `Button` submissions. Its renderer can
  display `Select` and `Input` fields, but the managed ingress does not route
  their callbacks or expose their submitted values. Use `Button.value` for
