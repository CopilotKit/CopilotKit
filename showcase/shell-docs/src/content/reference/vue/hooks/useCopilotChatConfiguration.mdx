...
Because the composable returns a `ComputedRef`, the value stays reactive: when labels, the active agent or thread, the modal state, or the drawer state change, the resolved value updates automatically. This provides a lightweight, reactive context for localized labels, agent/thread binding, modal state, and the shared drawer controls used by the chat UI components.
...
The injected configuration value also carries shared drawer state and thread
control helpers, which are documented in the composable return value below.
...
<PropertyReference name="drawerOpen" type="boolean">
  The current threads drawer state. This is always available when a provider is
  present.
</PropertyReference>

<PropertyReference name="setDrawerOpen" type="(open: boolean) => void">
  Function to control the threads drawer state.
</PropertyReference>

<PropertyReference name="drawerRegistered" type="boolean">
  `true` when a threads drawer is mounted in the current provider chain.
</PropertyReference>

<PropertyReference name="registerDrawer" type="() => () => void">
  Registers a threads drawer and returns a cleanup function that unregisters
  it.
</PropertyReference>

<PropertyReference name="setActiveThreadId" type="(threadId: string, options?: { explicit?: boolean }) => void">
  Switches the active thread from code. When `options.explicit` is `true`, the
  new thread is treated as an explicit caller choice.
</PropertyReference>

<PropertyReference name="startNewThread" type="() => void">
  Resets the current chat to a fresh, non-explicit thread.
</PropertyReference>
...
Use the shared drawer state when you need to open or close the threads drawer
...
function toggleDrawer() {
  ...
  if (!value?.setDrawerOpen) return;
  value.setDrawerOpen(!value.drawerOpen);
}
</script>

<template>
  <button v-if="config" @click="toggleDrawer">
    {{ config.drawerOpen ? "Close drawer" : "Open drawer" }}
  </button>
</template>
```
...
- **Drawer state ownership**: Drawer state (`drawerOpen` / `setDrawerOpen`) is shared through the provider chain, and `drawerRegistered` / `registerDrawer` track whether a drawer is mounted so mobile launchers can appear.
- **Thread overrides**: `setActiveThreadId` and `startNewThread` are imperative helpers used by the threads drawer and other host UI. `hasExplicitThreadId` still determines whether the resolved thread is treated as caller-controlled.
