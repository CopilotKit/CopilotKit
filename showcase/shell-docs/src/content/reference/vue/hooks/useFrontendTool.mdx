...
- **WebMCP registration.** With `webmcp` set, the tool is also registered on `document.modelContext` and removed with the tool. The tool needs a `description` for this (WebMCP rejects tools without one). To import page tools into CopilotKit, use [`useWebmcpTools`](/reference/vue/hooks/useWebmcpTools).
