...
<PropertyReference name="subagent" type="{ subagentRunId: string; subagent: Subagent | undefined; messages: Message[]; body: Component }">
  Replaces the group that shows the work of one subagent. `subagent` is
  undefined when the agent did not announce the subagent. Render
  `<component :is="body" />` to show the messages of the group. When the slot
  is omitted, the default group starts collapsed to a header with the name and
  status of the subagent, and opens on a click. See
