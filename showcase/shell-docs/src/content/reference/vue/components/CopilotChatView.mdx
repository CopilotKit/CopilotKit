...
<Callout type="info">
  `CopilotChatView` is a presentational component. You supply messages and agent
  state and handle its emitted events. Input can be controlled with `inputValue`;
  when that prop is omitted, the component manages the input value locally.
</Callout>
