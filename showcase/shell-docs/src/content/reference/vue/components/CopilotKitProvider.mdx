...
<PropertyReference name="enableInspector" type="boolean">
  Controls the CopilotKit Inspector. `true` shows it on every browser host,
  `false` hides it, and an omitted value shows it only on `localhost`,
  `127.0.0.1`, or `0.0.0.0`. The Inspector is not loaded during SSR. In a Vue
  template, pass this as `:enable-inspector="false"`.
</PropertyReference>

<PropertyReference name="showDevConsole" type="boolean | &quot;auto&quot;">
  **Deprecated:** use `enableInspector`. This compatibility alias retains its
  existing behavior when `enableInspector` is omitted. `enableInspector` wins
  when both props are supplied.
</PropertyReference>
