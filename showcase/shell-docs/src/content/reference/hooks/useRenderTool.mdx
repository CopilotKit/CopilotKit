...
You can target a specific tool name (with a typed Zod `parameters` schema) or use a wildcard (`"*"`) fallback renderer. The `parameters` schema is optional — omit it for render-only entries (for example, to opt a tool out of the default rendering with `() => <></>`).
...
### Named render-only overload (no schema)

When you don't need typed `parameters` — for example, to opt a specific tool out of the default rendering — omit the schema. `parameters` defaults to a pass-through (`z.any()`).
...
Omit `parameters` and return an empty fragment to suppress the default UI — for a single tool by name, or for all tool calls with the wildcard.
