...
Because the hook falls back to the built-in `DefaultToolCallRenderer` when no renderer matches, tool calls paint UI out-of-the-box. To suppress that -- render nothing for a tool call -- register a renderer whose `render` returns `null`. A registered renderer (even one returning `null`) takes priority over the built-in fallback, so no fallback UI is shown.
...
Opt out of all otherwise-unhandled tools with a wildcard catch-all renderer. This replaces the built-in fallback globally while still letting any tool with its own named renderer paint:
...
- **Name-based lookup**: The renderer searches registered render configurations for an exact tool name match first, then falls back to a wildcard (`"*"`) catch-all renderer, and finally to the framework's built-in `DefaultToolCallRenderer`.
...
- **Built-in fallback for unmatched tools**: If no renderer is registered for the tool name and no wildcard renderer exists, the function falls back to the built-in `DefaultToolCallRenderer` rather than returning `null`, so unhandled tool calls still paint out-of-the-box.
