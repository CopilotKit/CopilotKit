...
Because the hook falls back to the built-in `DefaultToolCallRenderer` when no renderer matches, tool calls paint UI out-of-the-box. To suppress that — render nothing for a tool call — register a renderer whose `render` produces no UI (return `null`, or an empty fragment `<></>`). A registered renderer takes priority over the default, so no fallback UI is shown.
...
Opt out of **all unhandled tools** by registering a wildcard (`"*"`) renderer with [`useRenderTool`](/reference/hooks/useRenderTool). Its wildcard overload takes no `parameters` schema, so this is the simplest no-op path — no Zod schema required. This disables the default renderer globally while still letting any tool with its own named renderer paint:

```tsx
function App() {
  // Suppresses the built-in DefaultToolCallRenderer for any tool call
  // without an exact-name renderer.
