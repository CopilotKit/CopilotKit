...
To disable rendering for a single tool, register a render-only renderer under that tool's name — same call as the wildcard, just a specific name (still no schema needed):

```tsx
function App() {
  useRenderTool({ name: "searchDatabase", render: () => <></> }, []);
