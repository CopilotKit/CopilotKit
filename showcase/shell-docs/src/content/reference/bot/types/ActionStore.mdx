...
<Callout type="info">
`ActionStore` and `InMemoryActionStore` are **deprecated**. Configure `store.adapter` on `createBot` instead — any [`StateStore`](/reference/bot/types/StateStore) backend (including `createRedisStore` and `createPostgresStore`) backs action snapshots automatically via the same pluggable interface. See the [Persistence guide](/bots/persistence) for a full walkthrough.
</Callout>

Action snapshots are now stored via `store.adapter` — the [StateStore](/reference/bot/types/StateStore) you pass to `createBot({ store: { adapter: … } })`. When `store.adapter` is a Redis or Postgres backend, actions survive process restarts and are shared across horizontally-scaled instances:
...
- [createBot](/reference/bot/functions/createBot) — the `store.adapter` option (replaces `actionStore`)
- [StateStore](/reference/bot/types/StateStore) — the pluggable persistence interface
