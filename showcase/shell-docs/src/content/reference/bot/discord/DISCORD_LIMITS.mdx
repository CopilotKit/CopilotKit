...
`DISCORD_LIMITS` is the typed constant the renderer ([`renderComponents`](/reference/bot/discord/renderComponents)) uses to enforce every Discord API ceiling in one place. Import it when you need to validate or pre-truncate data yourself before passing it to the renderer.
...
- [renderComponents](/reference/bot/discord/renderComponents) — the renderer that enforces these limits
