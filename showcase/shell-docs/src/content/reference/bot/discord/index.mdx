...
- `GuildMembers` — required for user lookup; it powers [`Thread`](/reference/bot/classes/Thread)'s `lookupUser(query)`, which resolves members via `guild.members.search`.
