...
<Callout type="info">
  Using an **OpenAI-compatible** provider such as [Atlas
  Cloud](https://www.atlascloud.ai)? Point the `OpenAIAdapter` at its `baseURL`
  — no custom adapter required. See the [OpenAIAdapter
  reference](/reference/v1/classes/llm-adapters/OpenAIAdapter) for an Atlas
  Cloud example (DeepSeek, Qwen, GLM, Kimi, MiniMax, …).
</Callout>
