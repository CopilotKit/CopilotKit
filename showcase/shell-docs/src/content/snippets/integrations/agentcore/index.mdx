...
- **Shared state** — bidirectional sync between agent state and your frontend UI
...
<Callout type="info" title="AWS CLI required">
  Both paths below require AWS credentials configured locally. If you haven't
  done this yet, follow the [AWS CLI getting started
  guide](https://docs.aws.amazon.com/cli/latest/userguide/cli-chap-getting-started.html)
  before continuing.
</Callout>
  ...
  >
    ...
    >
      <Step>
        ...
        ```npm
        npm install @copilotkit/react-core @copilotkit/runtime @ag-ui/client rxjs hono
        ```
      </Step>
...
<Cards>
  <Card
    title="Generative UI"
    description="Render custom frontend components directly from your agent's tool calls."
    href="/generative-ui/tool-rendering"
    icon={<PaintbrushIcon />}
  />
  <Card
    title="Shared State"
    description="Sync structured state between your agent and your frontend UI bidirectionally."
