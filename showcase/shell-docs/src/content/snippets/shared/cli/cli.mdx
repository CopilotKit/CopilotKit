...
The CopilotKit CLI helps you create CopilotKit apps connected to cloud-hosted Enterprise Intelligence. It handles browser sign-in, project selection, project-scoped runtime API keys, historical thread import, and local project configuration so your app can use Rich Threads and durable conversation history.
...
<OpsPlatformCTA
  ...
  body="Create a free account, then run the init command below to scaffold a connected app."
...
<Callout type="info" title="Creating vs. adding to an existing app">
  `init` (`create` is an exact alias) scaffolds a brand-new project in its own directory — it prompts for an app name and does not detect or bootstrap an app you already have. To add CopilotKit to an existing app, follow the manual installation in the [Quickstart](/quickstart) instead.
</Callout>
...
| `npx copilotkit@latest skills onboard` | Installs skills, then starts agent-assisted CopilotKit onboarding for an existing app. It does not provision or connect Enterprise Intelligence. |
...
- **Use Rich Threads:** the generated starter already includes the Drawer; see [Threads Drawer](/prebuilt-components/copilot-threads-drawer) to customize it, or [Headless Threads](/headless-threads) to replace it with your own thread UI
...
- **Production self-hosting:** [Self-Hosting Enterprise Intelligence](/premium/self-hosting) — review the enterprise architecture and plan a deployment with CopilotKit Engineering
