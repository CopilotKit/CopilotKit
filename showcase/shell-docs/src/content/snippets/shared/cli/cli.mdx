...
If you need to import into a different project, select it before running the dry run:

```bash title="Terminal"
npx copilotkit@latest project select
```

This changes the import target for the current directory and provisions a project-scoped runtime key for the selected project.

For the full migration flow, see [Import thread history](/threads-import). Source-specific setup lives in [Import ADK threads](/google-adk/threads-import) and [Import LangGraph threads](/langgraph-python/threads-import).
...
- **Import thread history:** [Import thread history](/threads-import) — migrate existing ADK or LangGraph conversations into Enterprise Intelligence
