...
<Steps>
  ...
  <Step>
    ...
    and project-scoped runtime API key to `.env`. Every Threads-enabled starter
    also receives a compatibility token.
    ...
    Keep `INTELLIGENCE_API_KEY` and `COPILOTKIT_LICENSE_TOKEN` on the server
    side. The project key connects the Runtime to Intelligence. Current starter
    SDK pins use the token for legacy entitlement checks; it does not replace
    the project key.

    Most starters read the project key from `INTELLIGENCE_API_KEY`. The
    AgentCore AWS deployment path reads the same value from
    `CPK_INTELLIGENCE_API_KEY`. Before you deploy an AgentCore starter, copy the
    generated value to that variable:
