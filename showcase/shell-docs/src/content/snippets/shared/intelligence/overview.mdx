...
|---|---|---|
| Rich Threads | Resumable conversations that survive reloads, devices, and browser sessions. | [Rich Threads](/threads) and [Threads & Persistence Architecture](/intelligence/threads-explained) |
| Existing conversation history | Import supported LangGraph threads or Google ADK sessions as Rich Threads, then synchronize future CopilotKit-mediated runs. | [LangGraph history](/langgraph-python/threads-import) and [Google ADK history](/google-adk/threads-import) |
...
| User Memories | Durable facts and preferences that can be recalled across conversations. | [User Memories](/intelligence/memories) |
...
| Automatic Learning | Agents improve from real usage. BuiltInAgent and framework adapters can load published skills automatically. | [Automatic Learning](/learning) and [Automatic skill delivery](/intelligence/learned-skills) |
...
| Self-hosting | Run Intelligence in your own Kubernetes cluster or on AWS ECS/Fargate. | [Kubernetes and Helm](/intelligence/self-hosting) or [AWS ECS/Fargate](/intelligence/self-hosting-ecs) |
...
- [Import LangGraph threads](/langgraph-python/threads-import) from LangGraph Server, LangGraph Platform, or LangSmith Deployment thread and run APIs. Standalone LangChain message stores, LangSmith traces, and embedded checkpointers without those APIs are not supported import sources.
- [Import Google ADK sessions](/google-adk/threads-import) from database session stores or Vertex/Agent Engine session history. In-memory sessions cannot be exported.
...
| [Self-host on AWS ECS/Fargate](/intelligence/self-hosting-ecs) | Your team runs workloads on ECS and needs Intelligence in its AWS account. | The versioned CloudFormation bundle, Fargate services, ALB, PostgreSQL, Redis, OIDC, secrets, upgrades, and monitoring. |
...
| Run the platform on AWS ECS/Fargate | [Self-host on AWS ECS/Fargate](/intelligence/self-hosting-ecs) |
| Understand the runtime/platform architecture | [CopilotKit Intelligence architecture](/intelligence/intelligence-platform) |
| Add persistent conversations to an app | [Rich Threads](/threads) |
| Let users open existing LangGraph conversations in a CopilotKit app | [Synchronize LangGraph Threads](/langgraph-python/threads-import) |
| Bring existing ADK session history into a CopilotKit app | [Synchronize ADK Threads](/google-adk/threads-import) |
...
| Give an agent durable context across conversations | [User Memories](/intelligence/memories) |
| Turn real usage into reusable agent behavior | [Automatic Learning](/learning) and [Automatic skill delivery](/intelligence/learned-skills) |
