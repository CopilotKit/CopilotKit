...
Headless UI uses the `useCopilotChatHeadless_c` hook. You build the chat UI. CopilotKit still runs threads, generative UI, suggestions, and interrupts.

<Callout type="warn">
  **This hook needs a public license key.**
  Self-hosted and open-source projects use that key. A cloud-hosted project uses
  `CPK_INTELLIGENCE_API_KEY` and does not include this hook. If the CopilotKit CLI
  created your project, build the UI with the v2 primitives in
  [Headless UI](/built-in-agent/custom-look-and-feel/headless-ui).
</Callout>
...
<Cards className="gap-6">
  <Card
    ...
    description="Build a Headless UI chat from the ground up to get started."
...
**cloud-hosted** project, which receives `CPK_INTELLIGENCE_API_KEY` and no license
key, so do not follow this section for a CLI-scaffolded project. Use
[Headless UI](/built-in-agent/custom-look-and-feel/headless-ui) instead.

<Steps>
<Step>
### Create a new application

Scaffold a self-hosted or open-source CopilotKit project:

```bash
npx copilotkit@latest create
```

Then [self-host the Runtime](/intelligence/self-hosting) and get a free public
license key for the provider.
</Step>
