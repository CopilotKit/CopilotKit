...
It requires the Enterprise Intelligence Platform because Threads are stored and
synced server-side. The React setup below is the same for cloud-hosted and self-hosted deployments; only the platform endpoint and Runtime credential change.
...
<SignupLink surface="docs_drawer">Get a free developer account</SignupLink> to
...
`<CopilotThreadsDrawer>` works with zero props. Common optional props:
...
| `limit` | Overrides the page size. The default page size is 50 threads. Cursor pagination stays active when you omit `limit`: `nextCursor` controls whether the drawer shows "Load more". |
...
Use the React wrapper's supported props to customize its content:

```tsx title="app/threads-drawer.tsx"
...
- **Labels and behavior**: use the supported wrapper props above, such as `label`, `recentLabel`, and the selection callbacks, instead of passing raw custom-element children.
...
Threads require CopilotKit Intelligence. A cloud-hosted app gets its entitlement
from the Intelligence project connected to the Runtime; it does not put a
license token in browser code. Self-hosted and offline enterprise setups may use
an explicit license token.

## Related

- [Rich Threads overview](/threads): compare thread UI and deployment paths
- [Connect an Existing App to Hosted Intelligence](/premium/existing-app-hosted-intelligence): configure the Runtime and verify per-user persistence
