...
CopilotKit CLI starters with Rich Threads already include the Drawer and its
shared chat configuration. Use this guide to understand that generated setup,
customize the Drawer, or look up its component options; you do not need to add it
again to a CLI-created starter.

Start with the [Rich Threads overview](/threads) to compare the Drawer with a custom
headless UI and understand the shared persistence infrastructure beneath both.
...
server-side). Follow the managed CLI starter flow in the [Rich Threads overview](/threads)
to create an app connected to Enterprise Intelligence.
...
## Understand the included setup

The generated starter places `<CopilotThreadsDrawer>` next to `<CopilotChat>` and
wraps **both** in a shared `<CopilotChatConfigurationProvider>`. That shared
...
example shows the essential structure to preserve while customizing your
starter.
...
That's the whole integration included by the starter. A hand-rolled sidebar would have you track the
...
- [Rich Threads overview](/threads): understand the product and choose an implementation path
