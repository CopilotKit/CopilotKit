...
**Self-hosted and OSS.** Set `COPILOTKIT_LICENSE_TOKEN` in the Runtime
environment. Without it the drawer shows a locked view with an Upgrade link in
place of the list, and it issues no thread requests.
