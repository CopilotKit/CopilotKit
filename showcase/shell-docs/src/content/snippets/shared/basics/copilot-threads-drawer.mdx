...
<Callout type="warn">
  **These component names exist in two packages.** `@copilotkit/react-ui` also
  exports `CopilotChat` and `CopilotSidebar`, and those are the [deprecated v1
  components](/migrate/v2). This guide documents the v2 pairing: take the chat,
  the Drawer, and the provider from `@copilotkit/react-core/v2`, as the example
  below does.
</Callout>
