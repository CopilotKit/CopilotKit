The Enterprise Intelligence Platform provides production-grade error observability with the observability solution of your choice via the `onError` hook. This feature requires a `publicLicenseKey` or `publicApiKey` and provides rich, structured error events you can forward to your monitoring and analytics systems.
...
<Callout type="info">
  **Key Configuration**: - Use `publicApiKey` when using Copilot Cloud - Use
  `publicLicenseKey` when self-hosting CopilotKit - Both keys provide the same
