...
2. **Run** — messages and tool calls stream under that `threadId`. If a server-side store is configured (the Enterprise Intelligence Platform, or a persisting `AgentRunner`), they're persisted as they happen so the thread can be replayed later; a runtime with no persistence layer keeps nothing server-side.
