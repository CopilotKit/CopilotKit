...
<Steps>
      <Step>
        ...
        After a new thread record is created, the Runtime starts a best-effort naming task from the first run's input messages without waiting for that run to finish. It accepts a generated title of one to eight words and falls back to `Untitled` if every attempt fails. Add `generateThreadNames: false` to your existing Runtime options to turn that off.
      ...
      <Step>
        ...
        `useThreads` manages stored rows; it does not create an empty row. The new UUID switches the chat immediately, and Enterprise Intelligence creates the stored row lazily on the next agent run.
