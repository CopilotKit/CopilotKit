...
The honest answer is that you can't, because there is no such extension point. CopilotKit does not define a "bring your own thread backend" endpoint spec that you can implement against — [Threads](/threads) (`useThreads`, the [Threads Drawer](/prebuilt-components/copilot-threads-drawer), cross-device sync, replayable event history) are a capability of [CopilotKit Intelligence](/intelligence/overview), not an interface with a swappable implementation.

What you *can* do without the platform is persist conversations yourself. That path is real and it works — it just looks different from Threads, and it's worth being clear about what you get and what you give up before you build it.
...
- [Open source vs Intelligence](/concepts/oss-vs-enterprise) — where the capability boundary sits
- [Threads](/threads) — what the platform-backed path looks like
