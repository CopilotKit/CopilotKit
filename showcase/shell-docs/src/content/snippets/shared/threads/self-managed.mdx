...
<FrontendOnly frontend="vue">
`useThreads` is platform-backed and will not return your rows, so the conversation list is application code: a table of `(thread_id, user_id, title, updated_at)` that you query and render, with the selected id handed to `<CopilotChat :thread-id="..." />`.
</FrontendOnly>
