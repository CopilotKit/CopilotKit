...
New to Rich Threads? Read the [Rich Threads overview](/threads) first. CLI-created starters already include the Threads Drawer; use [Headless Threads](/headless-threads) only when you need to build a custom thread UI.
...
<Steps>
  ...
  <Step>
    ...
    `COPILOTKIT_API_KEY` is also accepted for the key. To use a different cloud-hosted project, run `npx copilotkit@latest project select` first, then export the updated values written to `.env`. You can pass the same values directly with `--api-url` and `--api-key` instead.
  </Step>
