...
`useRenderTool` is the one exception. React Native exports it as a **temporary, deprecated compatibility shim** over react-core's two render-tool hooks, because RN used to export a *different* hook under that name — one that registered a tool as well as a renderer. The shim routes a call carrying `description` or `handler` the way the old hook did (`name: "*"` always stays renderer-only), warns in development, and is scheduled for removal in the next minor, after which the name becomes react-core's hook. It owns no registry of its own: every path delegates to react-core. See [Migrating from the old React Native `useRenderTool`](/reference/react-native/hooks/useRenderTool#migrating-from-the-old-react-native-userendertool).
...
<Callout type="warn" title="Choose the hook by who owns the tool, not by where the UI goes">
  ...
  On React Native, `useRenderTool` is currently a **deprecated compatibility shim** over both core hooks: a call carrying `description` or `handler` is routed the way React Native's older local hook did and warns in development, and the shim is removed in the next minor. [Migration notes](/reference/react-native/hooks/useRenderTool#migrating-from-the-old-react-native-userendertool).
