...
- **`useRenderTool` is React Native's own hook**, not react-core's. It registers the tool *and* its renderer, requires `parameters`, accepts a `handler`, and must return `ReactElement | null` (React Native's `FlatList` cannot render bare strings). It writes into react-core's canonical renderer registry, so wildcard resolution behaves here exactly as it does on the web: a renderer registered under the name `"*"` is the fallback for any tool call with no exact-name renderer. Because this hook always registers a tool alongside the renderer, a `"*"` entry still needs `parameters` — the web hook has a dedicated `"*"` overload that takes a renderer alone.
