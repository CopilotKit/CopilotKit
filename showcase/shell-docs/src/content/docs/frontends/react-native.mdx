...
<Callout type="warn" title="Choose the hook by who owns the tool, not by where the UI goes">
  ...
  `useRenderTool`'s `render` must return `ReactElement | null`, so returning `null` draws nothing. `useFrontendTool`'s `render` is a component type that also accepts a bare string, which typechecks and then throws inside a `FlatList` — return an element or `null` there too.
