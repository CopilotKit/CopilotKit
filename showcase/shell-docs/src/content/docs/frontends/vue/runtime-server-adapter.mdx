...
const handler = createCopilotRuntimeHandler({
  ...
  onBeforeHandler: async ({ route }) => {
      const agent = "agentId" in route ? ` for agent ${route.agentId}` : "";
      console.log(`Handling ${route.method}${agent}`);
    },
...
Once your runtime is running, point `CopilotKitProvider` at it. For a separately
hosted runtime, use its full URL:

<VueDocExample file="quickstart/App.vue" region="provider-chat-app" />

For a same-origin multi-route deployment (including a Vue frontend served
separately behind the same origin), use a relative base path:

<VueDocExample file="backend/agentcore.vue" region="agentcore-provider-chat" />

When the server uses `mode: "single-route"`, the Vue transport must opt into the
matching envelope:
