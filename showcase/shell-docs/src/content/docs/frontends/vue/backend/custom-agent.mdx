...
The frontend setup is the same as the [Vue quickstart](/vue). Wrap your app with
`CopilotKitProvider` and add a chat component:
...
[AgentRunner & persistence](/vue/backend/agent-runner).
...
With `type: "custom"`, you yield AG-UI events directly. See the [AG-UI runtime guide](/vue/backend/ag-ui) for the event flow.
</Tab>
</Tabs>
...
const agent = new BuiltInAgent({
  type: "aisdk",
  factory: ({ input, abortSignal }) => {
    const props = isRecord(input.forwardedProps) ? input.forwardedProps : {};
...
const agent = new BuiltInAgent({
  ...
  factory: ({ input, abortController }) => {
    const props = isRecord(input.forwardedProps) ? input.forwardedProps : {};
    const { messages, systemPrompts } = convertInputToTanStackAI(input);
    const model = typeof props.model === "string" ? props.model : "gpt-4o";
...
Forward props from Vue with the `CopilotKitProvider` `properties` binding. This
compiled provider example sends non-secret request properties alongside an
authentication header:

<VueDocExample file="auth/provider-headers.vue" region="provider-auth-headers" />

Use the same binding with `{ model: "anthropic/claude-sonnet-4", temperature:
0.3 }` when those are the factory inputs you accept. Treat all forwarded
properties as untrusted request input and validate allowed models and values on
the server.
