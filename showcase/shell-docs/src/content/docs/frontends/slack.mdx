...
<Steps>
  ...
  <Step>
    ...
    - `CHANNEL_CODE` is written for you when the CLI declares the Channel
      during onboarding; it knows the Code it just created.

    So the CLI puts both Intelligence values in `.env` and you never copy either
    one by hand. Setting them yourself still works: see
