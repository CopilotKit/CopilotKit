...
<Steps>
  ...
  <Step>
    ...
    separate hosts, so do not derive the WebSocket URL from the API URL. Pass
    each as a bare base URL without `/api`, `/socket`, `/runner`, or `/client`.
    For a self-hosted deployment, replace both bases together. Create the
    project-scoped runtime key from **API Keys** in the Intelligence project
    sidebar.
