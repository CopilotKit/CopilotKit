...
description: Run any supported agent framework in Teams through a cloud-hosted Intelligence connection.
...
<Steps>
  ...
  <Step>
    ...
    The Channel answers only through its own `agent` option: it does not fall
    back to the runtime's `agents` map, so `agents: {}` above is intentional,
    and a Channel declared without `agent` fails every `runAgent()` call.
    ...
    because `next dev`, used by a Next.js agent backend or web app, already
    listens on 3000.
    ...
    <Callout type="warn" title="Also serving a web chat from this runtime?">
      ...
      `CopilotRuntime`; without it, an Intelligence runtime hides every agent
      from `/info` and the web chat loses its agents. See the
  ...
  <Step>
    ...
    [configure the runtime handoff](/channels/intelligence#configure-the-runtime-handoff)
    ...
    <Callout type="info" title="If tsx cannot resolve fast-json-patch">
      `fast-json-patch`, a dependency of `@ag-ui/client`, ships a stray
      `index.ts` beside its compiled `index.js`. Some tsx installs load that
      TypeScript file and then fail on its missing `./src/core` import. If
      startup fails that way, compile the runner and run the output with Node:

      ```bash title="Terminal"
      npx tsc --noEmit false --outDir dist
      node --env-file=.env dist/channel.js
      ```
    </Callout>
