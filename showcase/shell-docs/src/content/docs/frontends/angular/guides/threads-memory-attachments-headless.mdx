...
For a custom thread list, render the list and `CopilotChat` under the same
`provideCopilotChatConfiguration` scope. Use `injectThreads` for list state and
`injectChatConfiguration` to switch the active chat. The `injectThreads` inputs
accept plain values or Angular signals.
...
@Component({
  ...
  template: `
    <button type="button" (click)="startNewThread()">
      New conversation
    </button>
    ...
    @if (threads.fetchMoreError(); as error) {
    ...
    @if (threads.hasMoreThreads()) {
      ...
      >
        {{ threads.isFetchingMoreThreads() ? "Loading..." : "Load more" }}
      </button>
    }
  `,
})
export class ThreadListComponent {
  ...
  readonly chat = injectChatConfiguration();

  protected select(threadId: string): void {
    this.chat.setActiveThreadId(threadId);
  }
...
promises. Deletion is irreversible to the app user: the platform soft-deletes
the thread and retains its stored row. Ask the user before calling
`deleteThread` because the current UI and API expose no restore action.
...
<AngularSnippet region="headless-chat-controller" />
