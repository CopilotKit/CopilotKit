...
- [Node.js](https://nodejs.org/en/) 22.18 or later (tsdown 0.22 needs this to build packages)
- [pnpm](https://pnpm.io/) v10.x installed globally (`npm i -g pnpm@^10`)

Published packages type-check and emit declarations with TypeScript 7 (`tsc` is the Go compiler). Angular stays on TypeScript 5.9.3. Vue stays on TypeScript 5.8.2 and `vue-tsc`. Building packages needs Node 22.18 or later because tsdown 0.22 requires that.
...
<FrontendOnly frontend="angular">
For Angular debugging workflows, use the [Angular troubleshooting guide](/guides/troubleshooting) and the AG-UI Event Inspector.
</FrontendOnly>
