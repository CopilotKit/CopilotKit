...
<Frame description="CopilotThreadsDrawer beside a CopilotChat — selecting a thread replays its conversation, with no active-thread wiring.">
  <img
    ...
    alt="CopilotThreadsDrawer listing conversations next to an active CopilotChat conversation"
...
- Thread switching, new conversations, rename, archive, and delete with zero wiring
...
Drop `<CopilotThreadsDrawer>` next to `<CopilotChat>` under your `CopilotKitProvider`.
With no callbacks, the drawer drives the active thread itself: selecting a row
connects the chat to that thread and replays its history; the "New Conversation"
row resets the chat to a fresh welcome screen.
...
| `collapsible` | Whether the drawer shows a collapse toggle. Defaults to `true`; set `false` to keep it always expanded. |
| `onCollapseChange` | Called when the user collapses or expands the drawer (`(collapsed: boolean) => void`) — use it to reclaim the drawer's reserved layout space. |
...
See the [CopilotThreadsDrawer reference](/reference/components/CopilotThreadsDrawer) for the full prop and type list.
...
- [CopilotThreadsDrawer reference (React)](/reference/components/CopilotThreadsDrawer)
