...
Want the agent to turn data into a chart on its own? Wrap a chart component in
a tool. See [charting from a tool](/channels/charts#let-the-agent-chart-on-its-own).
