...
See [interactive messages](/channels/interactive) and
[JSX callbacks](/channels/reference/callbacks).
...
See [Tools and context](/channels/tools).

## Runtime lifecycle

A Channel has no public `start()` method. The long-running runtime listener owns
activation:

```ts title="channel.ts"
const listener = createCopilotNodeListener({ runtime });

if (!listener.channels) throw new Error("Channels were not configured.");
...
`ready()` settles for `online` and `setup_required`; inspect `status()` before
accepting traffic.

| Status           | Meaning                                                          |
| ---------------- | ---------------------------------------------------------------- |
| `connecting`     | Activation is still in progress.                                 |
| `online`         | The managed session is currently sendable.                       |
