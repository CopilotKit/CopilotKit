...
Methods that are not available on a surface return a failed result or neutral
fallback. Check the return value when the capability is required.
...
| ------------ | ------------------------------- | ---------------------------------------------------------------------------------------------- |
| `prompt`     | `string \| AgentContentPart[]`  | Explicit user turn. Pass the inbound managed message because it is not in fetched history yet. |
...
`awaitChoice`; see [interactive messages](/channels/interactive).
...
default MemoryStore keeps it only in the current process. See
