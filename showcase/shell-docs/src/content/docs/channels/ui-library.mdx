...
Want to post a component from your own app, a styled dashboard card or a shadcn
tile, that is not in this vocabulary? Pass it to `thread.post` and it renders to
an image. See [Posting JSX as images](/channels/posting-jsx-as-images) and
