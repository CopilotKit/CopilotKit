...
history for the agent. The current in-flight message is not in that history, so
pass it explicitly:
...
[`StateStore` contract](/channels/reference/channel#statestore-contract).
...
<Callout type="warn" title="Do not infer durability from Online status">
  **Online** confirms that the managed listener can receive and send. It does
  not prove your application workflow state survives a process restart. Test a
...
See the [Thread API](/channels/reference/thread) for methods and
