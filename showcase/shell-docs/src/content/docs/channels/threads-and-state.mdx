...
<Callout type="info" title="A Channels thread is not an Intelligence thread">
  A Channels `Thread` is one platform conversation inside the SDK. [Threads](/threads)
  is the application UI for browsing and restoring agent conversations. They share
