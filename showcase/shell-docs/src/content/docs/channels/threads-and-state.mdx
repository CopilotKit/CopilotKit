...
thread identifiers from its value. The SDK passes the checkpoint id to your
`agent(threadId)` factory (or sets it on a cloned singleton). For the default
agent that id is the conversation thread. Extra named agents get
`threadId::agentId`. See [Multiple agents](/channels/multiple-agents).
...
<Callout type="info" title="Keep conversation and user identity separate">
  ...
  [Identity and Memory](/channels/identity-and-memory) before attaching
