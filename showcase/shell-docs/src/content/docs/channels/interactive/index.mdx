...
<Callout type="warn" title="Enable Slack Interactivity before testing buttons">
  ...
  configuration. The generated manifest currently may leave Interactivity
  disabled, so verify that setting before expecting button callbacks.
</Callout>
...
- Incoming reactions can be handled when Intelligence delivers them, but
  adding or removing reactions from the SDK is not part of the managed
  baseline.
...
Use the [JSX callback reference](/channels/reference/callbacks) for handler
arguments and the [Thread API](/channels/reference/thread) for post, update, and
resume signatures.
