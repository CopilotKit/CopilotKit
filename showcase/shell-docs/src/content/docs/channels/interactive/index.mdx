...
<Callout type="info" title="Use the current Slack manifest">
  ...
  managed callback URL. If the app was created from an older manifest, apply
  the current manifest and reinstall the app before testing buttons. After the
...
<Callout type="info" title="Framework interrupt support is not universal">
  The DeepAgent tool-driven approval flow has been validated with this
  post-and-resume pattern. Some legacy LangGraph interrupt flows expose only
  partial behavior, and this guide does not promise full native AG-UI interrupt
  compatibility across every framework. The default `sanitizeAgentEvents`
  behavior repairs a known event-validation issue; it does not add framework
  interrupt support by itself.
</Callout>
...
- Slack can deliver buttons, selects, and inputs through Block Kit. Current
  generated manifests enable Interactivity; older apps must apply the current
  manifest and be reinstalled.
