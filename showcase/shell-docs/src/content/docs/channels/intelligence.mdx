...
<Steps>
  ...
  <Step>
    ...
    Both Intelligence base URLs are required. The REST and realtime planes use
    separate hosts, so do not derive the WebSocket URL by changing the API
    URL's scheme. Pass bare base URLs: the client appends `/api/...`, `/runner`,
    or `/client` itself. Do not append `/api`, `/socket`, `/runner`, or
    `/client`. For self-hosted deployments, replace both bases together.
