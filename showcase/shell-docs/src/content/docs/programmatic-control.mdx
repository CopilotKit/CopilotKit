...
For integrations whose pinned showcase uses the promise-based path, the demo
uses `useFrontendTool` with a Promise-based handler instead. This can be a
runtime limitation or a temporary compatibility fallback.
