...
`injectAgentStore` is the signal-based interface to the AG-UI agent powering
your Angular copilot. Select an agent by id, read reactive state from the
returned signal, and use the underlying `AbstractAgent` for protocol methods:
...
The `store().agent` value is a standard AG-UI `AbstractAgent`. Subscribe to
protocol events when you need a lower-level stream:
...
The callbacks map directly to the [AG-UI event
types](https://docs.ag-ui.com/concepts/events). Prefer Angular signals for
templates; use `subscribe` for protocol-level diagnostics or integrations.
</FrontendOnly>
...
const agent = store().agent;  // An AbstractAgent
store().messages();           // Reactive conversation history
store().state();              // Reactive agent state
agent.subscribe({ /* … */ }); // Protocol event stream
```

```ts title="What actually happens"
// injectAgentStore() → registry checks /info → resolves a proxy agent
// core.runAgent({ agent }) → runtime POST → agent execution → SSE events
```
</FrontendOnly>

This indirection is what enables the runtime to provide authentication, middleware, agent routing, and Enterprise Intelligence Platform features — without changing how you interact with agents on the frontend.
