...
The examples below take `intelligence` and `identifyUser` as given. `intelligence` is a `CopilotKitIntelligence` instance, not a plain object — see [Connect your runtime to Intelligence](/premium/connect-your-runtime) for the constructor and where the project API key comes from.
