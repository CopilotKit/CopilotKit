...
import AgentNameBinding from "@/snippets/agent-name-binding.mdx";
...
<FrontendOnly frontend="vue">
<Callout type="warn" title="Keep the transport automatic when possible">
  `CopilotKitProvider` discovers the Runtime transport when
  `use-single-endpoint` is omitted. If you must pin the mode, use
  `:use-single-endpoint="true"` for a single-route handler or
  `:use-single-endpoint="false"` for a multi-route handler.
</Callout>
</FrontendOnly>
