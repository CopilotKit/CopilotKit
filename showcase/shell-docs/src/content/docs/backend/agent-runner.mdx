...
| `SqliteAgentRunner` | `@copilotkit/sqlite-runner` | First-party, file-backed durable runner. Persists thread runs to a SQLite file so history survives restarts on a single instance. Requires the `better-sqlite3` `^12.10.0` peer dependency and a real (non-`:memory:`) `dbPath`. |
...
`better-sqlite3` `^12.10.0` peer dependency and give it a real, non-`:memory:` `dbPath`:
