...
— see [Authorizing threads](/auth#authorizing-threads) — or move to a runner
that scopes threads to a user. `SqliteAgentRunner` and `IntelligenceAgentRunner`
are both fine here: the thread routes are served from a runner's local store
only when the runner opts in, and only `InMemoryAgentRunner` does — along with
anything extending it, such as `AgentCoreRunner`.
</Callout>
...
existing runtime, see [Connect your runtime to Intelligence](/intelligence/connect-your-runtime).
</Callout>
