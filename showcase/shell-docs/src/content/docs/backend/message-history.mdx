...
It pulls back only the calls that a forwarded result answers. An assistant
message can hold several tool calls, and forwarding a call whose result you
dropped recreates the error you are trying to avoid.
...
export function lastTurnOnly(messages: Message[]): Message[] {
  ...
  for (const message of messages.slice(0, start)) {
    ...
    const toolCalls = (message.toolCalls ?? []).filter((call) =>
      neededCallIds.has(call.id),
    );
    if (toolCalls.length > 0) rescued.push({ ...message, toolCalls });
  }
...
// Every forwarded call has its result in the same list.
const forwardedCallIds = trimmed.flatMap((message) =>
  message.role === "assistant" ? (message.toolCalls ?? []).map((c) => c.id) : [],
);
const forwardedResultIds = trimmed.flatMap((message) =>
  message.role === "tool" ? [message.toolCallId] : [],
);
assert.deepEqual(forwardedCallIds, ["cB"]);
assert.ok(forwardedCallIds.every((id) => forwardedResultIds.includes(id)));
...
console.log("trim-history: forwarded only the answered call");
```
...
<Callout type="info">
CopilotKit constructs the agents it discovers from `runtimeUrl`, and no
configuration option attaches middleware to them. The instances also belong to
CopilotKit. A change to the runtime URL or the transport replaces one, and the
replacement carries over the thread, the state and the messages, but not
anything you attached yourself. Trim inside the runtime instead. Follow
[CopilotKit#1482](https://github.com/CopilotKit/CopilotKit/issues/1482) for a
supported browser-side option.
</Callout>
