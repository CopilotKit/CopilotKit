...
<Callout type="warn" title="`credentials: true` needs an explicit origin">
Setting `credentials: true` without an `origin` (or with `origin: "*"`) is
...
authorization headers to your runtime, so name the origins you trust instead:
