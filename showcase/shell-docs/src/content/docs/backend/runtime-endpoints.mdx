...
| `POST /api/copilotkit/threads/clear` | Wipe the in-memory thread history. A no-op on an Intelligence runtime, whose history lives in a database. |
...
On an Intelligence runtime these routes resolve the caller through your
`identifyUser` and only reach that user's threads.

On the default `InMemoryAgentRunner` — and on anything extending it, such as
`AgentCoreRunner` — they read the runner's local store and resolve no user at
all: any caller that reaches the route reads the thread it names, `GET
/threads` lists every thread in the process, and `POST /threads/clear` wipes
them all. A runner that serves neither, such as `SqliteAgentRunner`, answers
these routes with a 422 instead.
...
[Authorizing threads](/auth#authorizing-threads).
</Callout>
...
`threads/list` and `threads/clear` carry no thread, so the only decision
available for those two is whether to allow them at all.

See [Auth](/auth) for the full authentication guide and
[Authorizing threads](/auth#authorizing-threads) for the ownership map this
example assumes.
