...
Vue uses `CopilotKitProvider`. With `use-single-endpoint` omitted, it discovers
the transport from `/info`; set it to `true` for a single-route handler or
`false` for a multi-route handler.
...
<FrontendOnly frontend="vue">
Keep `CopilotKitProvider` pointed at the Runtime base URL. It discovers the
transport when `use-single-endpoint` is omitted; set
`:use-single-endpoint="true"` only when you need to pin a single-route handler.
</FrontendOnly>
</Step>
...
<FrontendOnly frontend="vue">
The Vue SDK also discovers the transport from Runtime info. Point
`CopilotKitProvider` at the base endpoint; no second route configuration is
needed:
...
<Callout type="warn">
Runtime info must remain reachable so the Vue client can discover the transport
