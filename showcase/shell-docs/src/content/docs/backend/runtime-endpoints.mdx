...
<Callout type="info" title="The framework wrappers support Intelligence resources">
  ...
  `mode: "single-route"`. A current client and Runtime carry Threads,
...
## Enable Threads routes

If the Inspector shows the Threads setup view, the client did not find the
...
Use this prompt to configure Intelligence and verify that your Runtime exposes Threads.
...
Single-route and multi-route handlers both expose Threads. Use single-route
...
See [Scope Threads to the signed-in user](/threads-lifecycle#scope-threads-to-the-signed-in-user)
...
An Intelligence-backed web Runtime that is ready for Threads includes:
