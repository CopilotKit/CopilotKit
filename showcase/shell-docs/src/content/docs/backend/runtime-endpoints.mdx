...
| `GET /api/copilotkit/inspector-metadata` | Optional trusted project, plan, license, and action context for the Inspector. Intelligence-backed runtimes advertise this route with `inspectorMetadata: true` in the runtime-info response. |
...
does not change the runtime connection or agent state.
