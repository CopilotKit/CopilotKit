...
- **Schema-loading** (langgraph-python, langgraph-typescript,
  ...
  ms-agent-python, google-adk, strands), the schema is saved as a `.json`
  file next to the agent and loaded once at startup.
- **Schema-inline** (built-in-agent, spring-ai, ms-agent-dotnet), the schema is
  declared inline as a typed literal in source rather than loaded from a
  JSON file. Spring AI and .NET don't ship a `load_schema` JSON loader, and
  the Built-in Agent inlines it so the schema ships in the Next.js route
  bundle without runtime file access.
- **LLM-driven** (mastra, strands-typescript), the agent runs a secondary LLM
...
<Callout type="info" title="The flight card is an illustrative domain">
  ...
  against it, and an operations container (`a2ui.render(...)` in the Python
  SDK) with `createSurface` + `updateComponents` + `updateDataModel`. Keep your own application's domain and substitute your own
...
3. The tool returns an `a2ui_operations` container (built with
   `a2ui.render(...)` in the Python SDK, or by hand) with
   `createSurface` + `updateComponents` + `updateDataModel` operations.
...
wrapper) parses the schema file once at module-import time. Where the
integration ships a sibling `booked_schema.json`, it is loaded too and
kept ready for the button-click "booked" optimistic swap (see the note on
...
The agent tool returns the operations container: `a2ui.render(operations=[…])`
with the CopilotKit Python SDK, or the same `{ "a2ui_operations": [...] }`
envelope built by hand where the integration has no such helper. Either
way the operations are *nested* (`{ createSurface: {...} }`). The A2UI
middleware detects the container in the tool result and forwards it to the
frontend renderer. The LLM only generates the four
...
those imports are for — and the setup steps below show that construction. If you added
...
The component tree is declared inline as a typed literal in source,
equivalent to deserialising a `flight_schema.json` but compiled into
the agent code. Spring AI and .NET don't ship a `load_schema` JSON
helper, and the Built-in Agent keeps the schema in its route bundle so
it needs no runtime file access. The structure is identical to the
JSON form; only the surface syntax changes:
...
Mastra and Strands TypeScript take a different route: the agent tool runs a
