...
3. The tool call streams through the agent as `TOOL_CALL_ARGS` events.
...
Owning the tool is also where you control what happens when the
secondary LLM returns a surface that fails validation. See
[A2UI Error Recovery](./error-recovery) for the `recovery` option and the
retry and failure states the renderer shows.
</Step>
</Steps>

## Progressive streaming

The secondary LLM's `render_a2ui` tool call streams through the agent
