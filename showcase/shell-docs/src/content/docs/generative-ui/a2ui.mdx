...
- **[Fixed Schema](./a2ui/fixed-schema)** — Pre-defined schema loaded from JSON. Agent provides data only. Fastest — no LLM schema generation.
- **[Fixed Schema (Streaming)](./a2ui/fixed-schema-streaming)** — Same pre-defined schema, but cards appear one-by-one as the LLM streams data. Great for lists.
- **[Dynamic Schema](./a2ui/dynamic-schema)** — A secondary LLM generates both the schema and data. Fully flexible — any UI from any prompt.
