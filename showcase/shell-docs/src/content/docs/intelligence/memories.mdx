...
`@copilotkit/vue/v2` does not yet export a reactive memory controller equivalent
to React's `useMemories()`. Use the REST routes below from a trusted backend, or
let an agent use the MCP tools; do not put a project API key in the browser.
</FrontendOnly>
