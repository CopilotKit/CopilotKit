...
When the first run creates a new thread, the runtime starts generating a short name (2–5 words) from the input messages using the LLM. This runs asynchronously, so it doesn't block thread creation or the agent's response. The generated name appears through the frontend thread API via realtime sync.
...
When a client opens an existing thread, the runtime obtains connection
credentials and the client joins the thread's WebSocket channel. Both completed
and active threads replay their recorded history over this connection. If a run
is active, the same channel also carries new events as they arrive.

The transition from replayed history to live updates is seamless. If a tool call from a previous thread completes while the client is switching away, its result is discarded rather than inserted into the new thread, so stale output never leaks between conversations.
...
The frontend thread store updates the list immediately when you rename,
archive, unarchive, or delete a thread. The mutation promise resolves or rejects
with the server request's outcome; it does not wait for WebSocket confirmation.

A failed delete restores the removed row. Failed rename, archive, and unarchive
operations report an error but do not automatically roll back the local change.
Handle the returned promise and use the thread client's error state; refetch the
list when you need to reconcile it with the server. Realtime metadata updates
also keep connected clients in sync.
...
The thread client exposes the most recent list or mutation error. A successful
mutation does not automatically clear an earlier error. Refetch the list to
reset its error state and reconcile the local rows with the server.
...
- **Ephemeral interactions.** If your users don't need conversation history (e.g., a one-shot Q&A widget), threads add unnecessary complexity. Choose an SSE runtime with an in-memory [AgentRunner](/backend/agent-runner). Omitting `threadId` from `CopilotChat` does not disable persistence: the chat creates an ID, and an Intelligence-connected runtime still stores the conversation.
