...
For runs through CopilotKit, Intelligence records the Rich Thread history while the agent continues using its configured durable LangGraph checkpointer, LangGraph deployment, or ADK session service. Keep a stable mapping between the CopilotKit `threadId` and the native thread or session identifier so subsequent runs use the same conversation in both systems.
