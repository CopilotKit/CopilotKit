...
# Mastra and LangGraph examples use OpenAI. ADK uses a Google key below.
...
```bash title="Terminal"
python -m pip install copilotkit-intelligence-langgraph langchain-openai
```
...
async def main():
    skills = create_skill_registry_middleware()  # Uses the environment above.
    try:
...
```bash title="Terminal"
npm install @copilotkit/intelligence-langgraph langchain @langchain/core @langchain/langgraph @langchain/openai zod
...
const registry = new SkillRegistry(); // Uses the environment above.
...
This example uses Google instead of OpenAI. Save it as `agent.py`, then run `python agent.py`:
