...
3. Set the following variables in the agent's server environment.
...
Leave `CPK_INTELLIGENCE_SKILLS_REVISION` unset to receive later published Skills. See [container configuration](#configure-one-container) for exact revision pins.
...
[Environment variables](#configure-one-container) can supply the container and revision. Explicit values override them.
...
const agent = new BuiltInAgent({
  ...
  learnedSkills: {
    // Optional: omit containerId and revision to use environment variables.
...
```bash title="Terminal"
python -m pip install copilotkit-intelligence-langgraph
```

Save this example as `agent.py`. Run it with `python agent.py`:
...
async def main():
    skills = create_skill_registry_middleware(
        # Optional: omit container_id and revision to use environment variables.
...
```bash title="Terminal"
python -m pip install copilotkit-intelligence-adk
```
...
1. Initialize the adapter with a published Skill in its container.
...
The registry's status exposes the loaded revision and any stale state. A valid empty container can initialize successfully, so initialization alone does not prove that a Skill exists.
