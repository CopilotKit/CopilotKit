...
Every attribute name starts with `copilotkit.`. Each event has an `event_id` in the form `<event>:<organization>:<project>:<resource>`, so you can remove duplicates after a retry.
...
Every service writes one JSON object per line to stdout.

- **Level:** set it with `config.logLevel` on Helm.
- **Request lines:** each HTTP request ends with an `http.server.request.completed` line that has `duration_ms`, `http.method`, `http.route`, `http.status_code`, and `outcome`.
...
- **Error rate:** error spans, and `http.server.request.completed` lines with `http.status_code` 500 or higher.
