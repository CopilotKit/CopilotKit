...
<Step>
    ...
    <SignupLink surface="docs_crewai_flows_quickstart_step1">Sign up for a free developer account</SignupLink> on our Enterprise Intelligence Platform if you want managed persistent threads and the inspector. A Copilot Cloud account is not required to run a local CrewAI Flow with CopilotKit.
</Step>
