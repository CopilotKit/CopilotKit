...
The foundation of this system is built on AG2 **conversation variables** (`Context.variables`) and AG-UI `StateSnapshotEvent`s, which `AGUIStream` emits automatically at the start of the run and again at the end of the run when a tool has changed the variables.
