...
<Steps>
    <Step>
        ...
        <SignupLink surface="docs_ag2_quickstart_step1">Sign up for a free developer account</SignupLink> on our Enterprise Intelligence Platform to get a license key. You'll use it later to enable persistent threads and the inspector.
    </Step>
...
The samples repository is a ready-made example. To expose any AG2 agent over AG-UI, install AG2 (1.0+) with the AG-UI and OpenAI extras and wrap the agent in `AGUIStream`:
...
Point the `HttpAgent` in your Copilot Runtime at `http://localhost:8000/chat` and your agent is live in the UI. See the <a href="https://docs.ag2.ai/docs/user-guide/ag-ui/" target="_blank">AG2 AG-UI integration docs</a> for supported protocol events, human-in-the-loop checkpoints, and shared-state details.
