...
<Callout type="info">
  CopilotKit consumes AG-UI protocol events streamed by AG2 over <code>/chat</code>. See the <a href="https://docs.ag2.ai/docs/user-guide/ag-ui/" target="_blank">AG2 AG-UI integration docs</a>.
</Callout>
...
>
            ...
            <Step>
                ...
                <Tabs groupId="language_ag2_readables_state" items={['Python']} default="Python" persist>
                    <Tab value="Python">
                        ...
                        def get_readable(message: RunAgentInput, description: str):
                            raw = next(
                            ...
                            if raw is None:
                                return None
                            # [!code highlight:5]
                            # `useAgentContext` JSON-encodes objects and arrays, but passes a
                            # plain string through untouched — so decoding is best-effort.
                            try:
                ...
                <Tabs groupId="language_ag2_readables_backend" items={['Python']} default="Python" persist>
                    <Tab value="Python">
                        ...
                        async def run_agent(
                            ...
                            colleagues = get_readable(message, "The current user's colleagues") or []
    ...
    >
        <Steps>
            ...
            <Step>
                ### Consume the data in your AG2 backend
                For a minimal backend, inject all readables directly into the agent's prompt for the run — no tools required. `prompt=` on `dispatch` **replaces** the agent's own prompt rather than adding to it, so pass your instructions along with the readables:
                ...
                <Tabs groupId="language_ag2_readables_minimal" items={['Python']} default="Python" persist>
                    <Tab value="Python">
                        ...
                        async def run_agent(
                            ...
                            # [!code highlight:3]
                            # `prompt=` overrides the agent's prompt, so send it along with the readables.
                            return StreamingResponse(
                                stream.dispatch(message, prompt=[SYSTEM_PROMPT, *readables], accept=accept),
                                media_type=accept or "text/event-stream",
                            )
                        ```
                    </Tab>
