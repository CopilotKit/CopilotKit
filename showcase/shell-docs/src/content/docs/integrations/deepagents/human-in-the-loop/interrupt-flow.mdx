...
<Step>
    ...
    <Callout type="info">
        Your agent will not be aware of the `interrupt` interaction by default in LangGraph. To make state available to your agent, use `CopilotKitMiddleware(expose_state=[...])` as shown in the Python example below.
    </Callout>
