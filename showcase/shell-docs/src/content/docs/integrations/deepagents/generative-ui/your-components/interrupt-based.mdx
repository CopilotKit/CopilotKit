...
By default, LangGraph interrupts happen outside of the agent's awareness—the agent execution pauses, the user responds, and execution resumes without the agent knowing about the interaction. However, you often want the agent to use the information collected during the interrupt.

In our example, we stored the user's chosen name in `state.agentName`. Here's how to make the agent aware of it:
...
<Tabs groupId="agent_language" items={['Python', 'TypeScript']} persist>
    <Tab value="Python">
        ...
        class AgentNameMiddleware(AgentMiddleware[AgentState, Any]):
            ...
            def before_model(self, state: AgentState, runtime: Runtime[Any]) -> dict[str, Any] | None:
                # The agent name is now available in state after the interrupt resolves
                agent_name = state.get("agent_name", "Assistant")
                
                # Inject it into the system prompt so the agent knows its name
                if agent_name:
                    runtime.system_prompt = f"You are {agent_name}, a helpful AI assistant."
                
                return None
        ```

        The agent will now introduce itself using the name provided by the user.
    </Tab>
    <Tab value="TypeScript">
        ```ts title="agent.ts"
        import { createMiddleware } from "langchain";
        ...
        export const agentNameMiddleware = createMiddleware({
            ...
            beforeModel: async (state, runtime) => {
                // The agent name is now available in state after the interrupt resolves
                const agentName = state.agentName || "Assistant";
                
                // Inject it into the system prompt so the agent knows its name
                if (agentName) {
                    runtime.systemPrompt = `You are ${agentName}, a helpful AI assistant.`;
                }
                
                // Handle the interrupt on first call (before name is set)
                if (!state.agentName) {
...
<Callout type="tip">
    You can access any custom state properties collected during interrupts the same way. This pattern works for approval workflows, form data, user preferences, and any other information gathered through the interrupt flow.
</Callout>
