...
## Backend — owning the tool

To own `generate_a2ui` yourself — for example to make the validate→retry recovery loop visible — wrap the agent with `enable_a2ui` and set `injectA2UITool: false` so the runtime does not inject a second copy:

```python title="src/agents/recovery_agent.py"
from agent_framework import Agent
from agent_framework_ag_ui import AgentFrameworkAgent, enable_a2ui

base_agent = Agent(client=chat_client, name="a2ui_recovery_agent", instructions=SYSTEM_PROMPT)
...
`enable_a2ui(agent, subagent_client, params)` takes an `A2UIToolParams` object — the same params the adapter uses under the hood:
