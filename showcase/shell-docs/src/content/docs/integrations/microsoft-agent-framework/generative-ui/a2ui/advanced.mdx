...
When using [Dynamic Schema A2UI](./dynamic-schema), a render sub-agent generates the UI schema and data. This takes a few seconds — during which CopilotKit shows a built-in progress indicator.
...
export const A2UIProgress = memo(function A2UIProgress({
  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-gray-50 p-4">
      ...
      {componentCount > 0 && (
        <p className="mt-2 text-xs text-gray-500">{componentCount} components</p>
      )}
    </div>
  );
});
...
As the render sub-agent generates the A2UI surface, the `parameters` object accumulates:
...
| `data` | `object` | Data values — arrive after the schema |

## Action Handlers

Fixed-schema buttons declare an `action` in the schema (see [Fixed Schema — Button actions](./fixed-schema#button-actions)). By default a click is forwarded to the agent; the React `createA2UIMessageRenderer` factory lets you intercept it in the browser instead.

### `onAction` interceptor

Pass `onAction` to the renderer factory. Return `null` after handling the action locally; return `undefined` to forward it unchanged to the agent:
...
An interceptor can return a modified `A2UIUserAction`; the renderer forwards that action to the agent:
...
const A2UIMessageRenderer = createA2UIMessageRenderer({
  ...
  onAction: (action) => {
    ...
    return {
      ...action,
      context: { ...(action.context ?? {}), source: "a2ui" },
    };
  },
});
```
...
| `createA2UIMessageRenderer` | Factory that accepts `theme` and optional `onAction` |
