...
<Steps>
    ...
    <Step>
        ...
        Frontend tools registered with `useHumanInTheLoop` arrive in each AG-UI run request. You do not register them again on the server. Mount your agent with the AG-UI endpoint helper, and the adapter forwards the request's frontend-tool definitions to the underlying agent for that run.
        ...
        <Tabs groupId="language_microsoft-agent-framework_agent" items={['.NET', 'Python']} persist>
          ...
          <Tab value="Python">
            ...
            # This endpoint receives frontend tools with each AG-UI run request.
