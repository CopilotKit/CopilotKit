...
Use this path when the operation is implemented on the server and Microsoft Agent Framework must pause before executing it. `agent-framework-ag-ui>=1.0.0rc8` maps tool approvals and workflow `request_info(...)` pauses to the standard AG-UI lifecycle:
...
<Steps>
    <Step>
        ...
        ```bash
        uv add "agent-framework-ag-ui>=1.0.0rc8" agent-framework-openai python-dotenv uvicorn
        ```
