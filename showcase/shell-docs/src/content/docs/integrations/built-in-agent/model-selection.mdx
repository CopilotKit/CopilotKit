...
[Groq](https://groq.com), [Novita](https://novita.ai), [Cheaper Inference](https://cheaperinference.com),
or your own fine-tuned endpoint, works through the same
...
Set `baseURL` to `https://api.cheaperinference.com/v1`, read the key from
`process.env.CHEAPER_INFERENCE_API_KEY`, and use a bare model id such as `gpt-5.4-mini`.
