...
[OrcaRouter](https://www.orcarouter.ai), a self-hosted gateway, an internal LLM proxy,
[Ollama](https://ollama.com), [Together](https://together.ai),
...
Gateways that ship a dedicated AI SDK provider plug in the same way, since any
AI SDK `LanguageModel` works. For example, with OrcaRouter's provider package
(`orcarouter-ai-sdk-provider`):

```typescript
import { BuiltInAgent } from "@copilotkit/runtime/v2";
import { orcarouter } from "orcarouter-ai-sdk-provider"; // [!code highlight]

const agent = new BuiltInAgent({
  // orcarouter/auto picks an upstream model per request
  model: orcarouter("orcarouter/auto"), // [!code highlight]
});
```
