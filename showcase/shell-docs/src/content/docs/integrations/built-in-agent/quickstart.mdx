...
<Steps>
    <Step>
        ...
        <Callout type="info" title="Already have an app?">
          Keep your current app and follow the [Angular quickstart](/angular) to configure its provider and chat. The CopilotKit CLI (`npx copilotkit@latest create`, aliased as `init`) creates a separate project; it does not modify an existing app.
        </Callout>
        </FrontendOnly>
    </Step>
    <Step>
    ...
    <Step>
        ...
        <FrontendOnly frontend="angular">
        <Callout type="info" title="Choose an Angular runtime URL">
          Configure the provider and runtime URL from the [Angular quickstart](/angular). Use a relative URL only when the Angular app and runtime share an origin; otherwise use the absolute runtime URL that your app can reach.
        </Callout>
        </FrontendOnly>
    </Step>
    <Step>
    ...
    <Step>
        ...
        <Accordions className="mb-4">
            <Accordion title="Troubleshooting">
                - If `localhost` does not resolve on your machine, use `127.0.0.1`. `0.0.0.0` is a server bind address, not a client destination.
