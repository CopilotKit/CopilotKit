    ...
    <Step>
        ...
        >
            ...
            <Step>
                ...
                Your agent server listens on port 4111 by default. The frontend you build
                next reaches it over HTTP and reads its address from `MASTRA_BASE_URL`, so
                set that too — and point it at your deployed Mastra service in production:

                ```bash
                export MASTRA_BASE_URL=http://127.0.0.1:4111
                ```
            </Step>
            <Step>
                ### Start your agent
            ...
            <Step>
                ...
                <Callout type="warn" title="Don't point `next dev` at your Mastra project root">
                  Next.js rewrites the `tsconfig.json` at its own root on first run,
                  overwriting settings like `"moduleResolution": "NodeNext"` that your agent
                  depends on. A sibling package leaves your agent's TypeScript config
                  untouched — and it is why this guide reaches the agent over HTTP rather
                  than importing it.
                </Callout>
            </Step>
            <Step>
            ...
            <Step>
                ...
                const runtime = new CopilotRuntime({
                  // getRemoteAgents is async; `agents` accepts the promise directly.
                  agents: MastraAgent.getRemoteAgents({
                    mastraClient,
                    resourceId: "user-1", // your app's user id — scopes Mastra memory
                  }),
