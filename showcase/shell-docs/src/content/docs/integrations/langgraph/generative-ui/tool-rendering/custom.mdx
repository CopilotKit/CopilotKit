...
<Callout type="info">
  Named renderers compose with a catch-all. The next optional step shows that
  pattern in this same demo; see
  [Catch-all Tool Rendering](/langgraph-python/generative-ui/tool-rendering/catch-all)
  for the built-in catch-all, branded catch-all, and render-nothing variants.
</Callout>
...
hook can provide a uniform catch-all UI. This is exactly how the demo
