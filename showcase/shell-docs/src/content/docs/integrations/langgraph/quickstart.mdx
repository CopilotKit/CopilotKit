...
`@copilotkit/sdk-js` 1.73.3 requires Zod 3 as a peer dependency. When adding it to a TypeScript agent, install Zod 3 alongside it:

```bash
npm install @copilotkit/sdk-js@1.73.3 zod@3
```

Pin `zod@3` explicitly. A separate, unversioned `npm install zod` selects Zod 4, which does not satisfy this SDK release's peer dependency; npm only warns that it is overriding it. Use `zod@3` with pnpm or Yarn too. If your agent already requires Zod 4, npm rejects the SDK with an `ERESOLVE` conflict: check SDK compatibility before changing its dependencies, and do not bypass the conflict with `--force` or `--legacy-peer-deps`.
</Callout>
    ...
    <Step>
        ...
        >
          ...
          <Step>
              ...
              <Tabs groupId="deployment_method" items={['LangSmith', 'FastAPI']}>
                <Tab value="LangSmith">
                  ...
                  const runtime = new CopilotRuntime({
                    ...
                    // Local single-user setup. Replace this before deploying to multiple users.
                    identifyUser: () => ({
                      id: "local-demo-user",
              ...
              <Callout type="warn" title="Use verified identity before production">
                The fixed identity above is runnable only for a local, single-user demo. It does not isolate
                multiple users. Before deployment, resolve a server-verified session or token and add the
                required request and ownership guards from
          ...
          <Tab value="TypeScript">
            ...
            <Snippet
              ...
              cell="cli-start"
              file="src/agent/package.json"
              title="src/agent/package.json — selected TypeScript agent dependencies and dev command"
            />
            ...
            <Snippet
              ...
              title="src/agent/graph.ts — compile and export the selected graph"
            />

            With the agent available at `http://localhost:8123`, connect your
            React app through a V2 runtime route. The selected demo's route
            maps its public agent names to the graph ids above; use the same
            public name in your provider and runtime. See [Provider and handler
            pairs](/backend/runtime-endpoints#provider-and-handler-pairs) for
            the route shape and [the React quickstart](/built-in-agent/quickstart) for the frontend
            setup.
          </Tab>
          </Tabs>
        </TailoredContentOption>
