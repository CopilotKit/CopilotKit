...
<Callout type="info" title="Import is one-time; threads keep future runs synced">
  Import moves existing LangGraph history. After import, keep new conversations synced by connecting your app to Enterprise Intelligence and using CopilotKit threads normally. See [Threads](/threads) for the runtime and UI setup.
</Callout>
...
Historical import does not keep polling LangGraph. For future conversations, wire your app to Enterprise Intelligence:
...
3. If your backend maps CopilotKit thread IDs to LangGraph thread IDs, keep that mapping stable so resumed conversations reach the expected LangGraph thread.
