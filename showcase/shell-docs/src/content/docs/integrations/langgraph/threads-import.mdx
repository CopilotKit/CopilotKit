...
<Callout type="info" title="Import is one-time; threads keep future runs synced">
  Import moves existing LangGraph history. Run it from a CopilotKit app created with the CLI and Enterprise Intelligence enabled. Use the [Threads Drawer](/prebuilt-components/copilot-threads-drawer) for a drop-in thread UI, or [Headless Threads](/threads) to build your own.
</Callout>
