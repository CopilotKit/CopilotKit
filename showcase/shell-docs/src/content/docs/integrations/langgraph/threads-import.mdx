...
<Callout type="info" title="Import is one-time; threads keep future runs synced">
  Import moves existing LangGraph history. Run it from a CopilotKit app created with the CLI and Enterprise Intelligence enabled. See [Headless Threads](/threads) for the thread UI setup.
</Callout>
