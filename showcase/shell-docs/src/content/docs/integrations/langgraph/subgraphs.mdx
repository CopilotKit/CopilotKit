...
export class AgentStatusComponent {
  readonly store = injectAgentStore("sample_agent");

  // Read this.store().state() in the template or another computed signal.
  // Subgraph streaming updates it automatically.
}
```
</FrontendOnly>
