...
| --- | --- |
| UI preferences the model should see | Publish them with `useAgentContext`; see [Agent Config](/langgraph-python/agent-config). |
...
non-secret values that should influence the model, use
[`useAgentContext`](/reference/v2/hooks/useAgentContext). The
[Agent Config guide](/langgraph-python/agent-config) shows the complete React
and Angular patterns and how CopilotKit middleware exposes the latest values on
every turn.
...
<Tabs groupId="language_langgraph_agent" items={['Python', 'TypeScript']} default="Python" persist>
  <Tab value="Python">
    ...
    [self-hosted authentication example](/langgraph-python/auth#self-hosted-deployment).
  </Tab>
