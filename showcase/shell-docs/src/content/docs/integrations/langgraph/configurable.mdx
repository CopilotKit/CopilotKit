...
Call `runAgent` from an event handler or another intentional lifecycle point. Calling it during component initialization without a guard can start duplicate runs.
...
@Component({
  selector: "app-root",
  template: `<button (click)="runWithConfig()">Start agent</button>`,
})
export class AppComponent {
  private readonly copilotKit = inject(CopilotKit);
  private readonly store = injectAgentStore("sample_agent");

  async runWithConfig(): Promise<void> {
