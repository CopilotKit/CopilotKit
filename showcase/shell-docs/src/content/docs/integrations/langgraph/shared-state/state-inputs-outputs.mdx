...
- A LangGraph agent built as a custom `StateGraph` and served through the CopilotKit runtime. If you don't have one yet, start with the [quickstart](/quickstart).
- The CopilotKit SDK for your agent's language: `copilotkit` for Python or `@copilotkit/sdk-js` for TypeScript.
- `@copilotkit/react-core` in your frontend, for the `useAgent` hook from `@copilotkit/react-core/v2`.
