...
<Callout type="tip" title="Why this pattern?">
  The `CopilotKitMiddleware` stores context entries in `state["copilotkit"]["context"]`, making them accessible in your agent nodes for programmatic use (like authentication). The middleware also injects the context as a SystemMessage for the LLM to read.
</Callout>
