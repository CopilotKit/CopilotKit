...
<Callout type="warn">
    Router mode requires that you set up an LLM adapter. See how in ["Set up a copilot runtime"](/quickstart?copilot-hosting=self-hosted#set-up-a-copilot-runtime-endpoint) section of the docs.
</Callout>
