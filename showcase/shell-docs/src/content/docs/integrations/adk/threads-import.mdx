...
description: "Import historical Google ADK sessions into CopilotKit Intelligence, then add Threads while continuing to use ADK persistence."
icon: "lucide/Repeat"
doc_type: how-to
---
...
The ADK importer reads already-persisted Google ADK sessions and writes them into CopilotKit Intelligence as Threads. It supports ADK database session stores and Vertex/Agent Engine session history.
...
CLI-created starters already include the Threads Drawer for opening and switching Threads. Use [Headless Threads](/headless-threads) only when you need a custom thread UI.
