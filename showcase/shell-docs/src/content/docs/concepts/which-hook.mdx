...
| [`useWebmcpTools`](/reference/hooks/useWebmcpTools) | Import page WebMCP tools from `document.modelContext` so the CopilotKit agent can call them. |
...
- **"The agent should *run* something in my app."** Use [`useFrontendTool`](/reference/hooks/useFrontendTool).
  ...
  to also show inline UI for the call. If the page already registered those
  tools on `document.modelContext`, use [`useWebmcpTools`](/reference/hooks/useWebmcpTools)
  so you do not rewrite each handler.
...
- [Import page WebMCP tools](/webmcp-tools): let the agent call tools already on `document.modelContext`.
