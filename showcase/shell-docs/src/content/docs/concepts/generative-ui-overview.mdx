...
<Callout type="info">
  **Prefer to learn by building?** Follow [Frontend tools and generative UI](/angular/guides/frontend-tools-generative-ui) for a complete Angular example with typed tools, standalone renderer components, A2UI, Open Generative UI, and MCP Apps.
</Callout>
</FrontendOnly>
...
Use `useComponent` to register a Vue component as a frontend tool. Use
`useRenderTool` when the tool runs on the backend and Vue controls how the call
appears.

<Callout type="info">
  **Prefer to learn by building?** Start with [Components as Tools](/generative-ui/tool-based)
  for a Vue example that registers a component during setup.
</Callout>
</FrontendOnly>
