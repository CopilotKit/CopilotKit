---
title: OSS vs Intelligence Platform
description: What ships in CopilotKit's open-source core versus what's part of the Intelligence Platform layer — Threads, Persistence, Observability, Self-Hosting.
icon: "lucide/Cloud"
doc_type: explanation
---

CopilotKit ships in two layers: an **open-source frontend stack** (npm packages, no external service required) and an **Intelligence Platform** that adds durable conversations, realtime sync, observability, and an admin console. Intelligence extends OSS — same SDK, same runtime — so the boundary is about *which capabilities you unlock*, not which code you write.
...
- **Intelligence Platform adds** durable threads, persistence across reloads and devices, realtime WebSocket sync, observability connectors, and a hosted admin console.
- **Intelligence Platform runs two ways** — fully hosted as [Copilot Cloud](https://dashboard.operations.copilotkit.ai), or self-hosted via the `copilot-intelligence` Helm chart on your own Kubernetes.
- **Your app code doesn't change** between OSS-only, Cloud, and Self-Hosted. The boundary is a runtime config switch.

| | OSS only | Needs Intelligence Platform |
|---|---|---|
...
| Intelligence Platform observability connectors | | Yes |
...
## What's in the Intelligence Platform
...
Same Intelligence Platform — you pick where it runs:
...
- **Self-Hosted** installs the `copilot-intelligence` Helm chart into your own Kubernetes cluster. Pick this for data residency, on-prem requirements, or compliance control. You operate the workload (Postgres, Redis, ingress, OIDC, secret rotation); the chart and the same Intelligence Platform image power the install.
...
- **Intelligence Platform overview** — [Intelligence Platform](/premium/overview) lists every platform capability and the per-feature setup.
- **Hosted setup** — [Copilot Cloud](https://dashboard.operations.copilotkit.ai) for the hosted Intelligence Platform.
