...
7. Threads, persistence, and realtime sync (when configured) are mediated by [CopilotKit Intelligence](/premium/overview) — the platform backend that sits beside the runtime.
...
- **CopilotKit Intelligence overview** — [CopilotKit Intelligence](/premium/overview) covers Threads, Persistence, hosted inspection, and the cloud-hosted-vs-self-hosted decision.
