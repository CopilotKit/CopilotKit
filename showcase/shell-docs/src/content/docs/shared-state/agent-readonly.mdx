...
import InspectorPaneContext from "@/snippets/shared/inspector/open-inspector-pane-context.mdx";

<InlineDemo demo="readonly-state-agent-context" />

<InspectorPaneContext components={props.components} />
...
<FrameworkSetup concept="agent-context-setup" />
...
- Read by the framework's backend bridge on every turn. The setup section for
  your integration shows where that bridge receives the entries.
