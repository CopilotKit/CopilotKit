...
A LangChain agent's state updates discontinuosly; only across node transitions in the graph.
...
<TailoredContent id="agent-type">
  ...
  >
    ...
    <Step>
        ...
        <Snippet region="frontend-use-coagent-state" title="ui/app/page.tsx — useAgent subscription" />
    </Step>
    <Step>
        ### Give it a try!
  ...
  >
    <Steps>
      ...
      <Step>
        ...
        <Tabs groupId="language_langgraph_agent" items={["Python", "TypeScript"]} persist>
          <Tab value="Python">
            <Snippet region="state-streaming-middleware" title="agent.py" />
          </Tab>
          <Tab value="TypeScript">
            ```ts title="agent-js/src/agent.ts"
