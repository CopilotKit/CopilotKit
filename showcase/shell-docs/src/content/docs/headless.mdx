...
CopilotKit's headless hooks let you build the chat UI with your own
components while keeping the agent connection, streaming, and tool calls.
Bring your own design system instead of using CopilotKit's prebuilt chat
components.
...
[**Complete**](#complete-headless-complete) to see a complete message view
(tool calls, reasoning, activity messages, and custom before/after slots)
built from the low-level hooks.
...
- [`useAgent`](/reference/hooks/useAgent) with `agentId` exposes the current conversation (`messages`,
  `isRunning`) and the run-state object.
- [`useCopilotKit`](/reference/hooks/useCopilotKit) returns the runtime handle you call
  `runAgent({ agent })` on (the same entry point `<CopilotChat />`
  uses internally).
- [`useComponent`](/reference/hooks/useComponent) (sugar over `useFrontendTool`) lets you register a
  React component the agent can render by invoking a named tool call.
  [`useRenderToolCall`](/reference/hooks/useRenderToolCall) then returns a function that paints any tool
  call inline.
...
a complete message view (text, tool calls
...
- [`useRenderToolCall`](/reference/hooks/useRenderToolCall) — returns the renderer for any registered tool
