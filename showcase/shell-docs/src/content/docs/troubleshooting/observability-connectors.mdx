...
The Intelligence Platform provides production-grade error observability with the observability solution of your choice via the `onError` hook on `CopilotKit`. This feature requires a `publicLicenseKey` or `publicApiKey` and provides structured error events you can forward to your monitoring and analytics systems.
