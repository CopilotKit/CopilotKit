...
<Callout type="info">
Need to see the full event pipeline: what events your agent emits, whether they reach the client, and where they are dropped? Open the [Inspector](../inspector). A red launcher pill names the failure. Click it to land on the pane that shows the agent, the tool if one failed, and the error.
</Callout>
