...
The Enterprise Intelligence Platform is the backend that powers threads, shared state, the inspector, observability, and other production capabilities. The same platform architecture runs whether you consume it as a hosted service at [Copilot Cloud](https://dashboard.operations.copilotkit.ai) or install it into your own Kubernetes cluster via the `copilot-intelligence` Helm chart — your application code (and the CopilotKit runtime it talks to) does not change based on which one you point at.

This page covers the logical architecture that applies to both deployment modes, plus the operational concerns specific to running the platform in your own cluster. Self-hosting gives you data residency and compliance control at the cost of running a production Kubernetes workload.
...
Self-hosted Enterprise Intelligence assumes a **platform layer** is already present in your cluster: an ingress controller (nginx or AWS ALB), `cert-manager` (or a cloud certificate service), and — for production — External Secrets Operator. These components are installed once per cluster, outside the Enterprise Intelligence release.

The **application layer** is the `copilot-intelligence` chart itself. It owns only the Enterprise Intelligence workloads and references the platform layer by class name (for example, `ingress.className: nginx`) and by secret store reference. Mixing the layers into a single install makes upgrades fragile; keeping them separate lets you upgrade the platform and the application on independent cadences.
...
**Why Helm.** Helm is the lingua franca of Kubernetes application delivery — every large platform team already operates a Helm workflow. Shipping Enterprise Intelligence as a Helm chart lets customers integrate it into their existing GitOps, CI/CD, and environment-promotion pipelines without adopting a new deployment primitive.
...
- **Step-by-step guide:** [Self-Hosting Enterprise Intelligence](/premium/self-hosting) — install the `copilot-intelligence` Helm chart, configure external dependencies, and verify the deployment
