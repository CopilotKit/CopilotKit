...
|-------------|-------------|
| `useThreads` hook | Lists, renames, archives, restores, and deletes threads. Pagination via `hasMoreThreads` / `fetchMoreThreads`. Stays in sync across tabs and devices via WebSocket. |
...
The `useThreads` hook maintains a WebSocket subscription for thread metadata changes. When any client creates, renames, archives, unarchives, or deletes a thread, the update is pushed to all connected clients automatically. This is how a thread created on one tab appears in the sidebar on another tab without polling.
...
general replication link between databases: `useThreads` operations such as
rename, archive, unarchive, and delete change the Enterprise Intelligence thread
and do not mutate records in LangGraph, ADK, or another native store.
