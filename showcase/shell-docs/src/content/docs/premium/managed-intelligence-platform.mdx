...
Use cloud-hosted CopilotKit Intelligence when you want the fastest path to production. Use [Self-host CopilotKit Intelligence](/premium/self-hosting) when your organization needs the platform inside its own VPC, cluster, or data boundary.
...
Project API keys connect your runtime to the cloud-hosted project. During `init` or its `create` alias, and during `project select`, the CLI provisions a project-scoped key, writes it to `.env` as `INTELLIGENCE_API_KEY`, and records the selected project in `.copilotkit/project.json`.

Every Threads-enabled starter created with `create` or `init` also receives `COPILOTKIT_LICENSE_TOKEN`. Current starter SDK pins use the token for legacy entitlement checks. The token does not replace the project API key, and both values must stay on the server.

Most starters read the project key from `INTELLIGENCE_API_KEY`. The AgentCore AWS deployment path reads the same key from `CPK_INTELLIGENCE_API_KEY`. Before you deploy an AgentCore starter, copy the generated `INTELLIGENCE_API_KEY` value to `CPK_INTELLIGENCE_API_KEY`.
...
- **Self-host the platform:** [Self-host CopilotKit Intelligence](/premium/self-hosting) — install and operate the Helm chart in your own cluster
