...
Project API keys connect your runtime to the cloud-hosted project. During `project select`, the CLI provisions a project-scoped key, writes it to `.env` as `CPK_INTELLIGENCE_API_KEY`, and records the selected project in `.copilotkit/project.json`. A `create` flow does the same only for a starter with Threads support. `CPK_INTELLIGENCE_API_KEY` remains required for managed connectivity. When telemetry is enabled and identity creation succeeds, the CLI may write `CPK_TELEMETRY_ID` for analytics attribution. The telemetry ID is not a secret. A selected project and its project-scoped key are a valid managed setup without `CPK_TELEMETRY_ID`. Managed setup does not issue or write `COPILOTKIT_LICENSE_TOKEN`.
...
If telemetry-identity setup fails, the CLI still commits the project and API
key, removes stale telemetry identity, and warns that Runtime telemetry will
stay anonymous. If key creation or local env writing fails, project selection
does not replace the prior local project and credentials.
...
- Archive is reversible: it marks the thread archived and hides it from the default list without deleting its history. Pass `includeArchived: true` to show archived threads, then unarchive one to return it to the active list.
- Delete is irreversible to the app user: the current UI and API expose no restore action. The platform soft-deletes the thread and retains its stored row.
...
- **Connect an existing app:** [Connect an Existing App to Hosted Intelligence](/premium/existing-app-hosted-intelligence) — add one hosted project key and verified app-user identity to Runtime v2
...
- **Self-host the platform:** [Self-Hosting Enterprise Intelligence](/premium/self-hosting) — install and operate the Helm chart in your own cluster
