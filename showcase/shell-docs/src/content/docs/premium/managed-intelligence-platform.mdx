...
Project API keys connect your runtime to the cloud-hosted project. The CLI provisions a project-scoped key during `create` and `project select`, writes it to `.env` as `CPK_INTELLIGENCE_API_KEY`, and records the selected project in `.copilotkit/project.json`. It also writes the non-secret `CPK_TELEMETRY_ID` for analytics attribution. Managed setup does not issue or write `COPILOTKIT_LICENSE_TOKEN`.
