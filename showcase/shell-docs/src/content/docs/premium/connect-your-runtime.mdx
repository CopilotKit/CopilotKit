...
description: "Wire an existing CopilotKit runtime to the Enterprise Intelligence Platform — construct CopilotKitIntelligence with a project API key, identify users, and confirm the credential is actually being used."
icon: "lucide/PlugZap"
doc_type: how-to
premium: true
---
...
Intelligence — SSE mode does the same. Confirm consumption instead:

```bash title="Terminal"
npx copilotkit verify
```

`verify` makes an authenticated call to the platform with the key your project
resolves and reports whether the runtime is wired for Intelligence at all. A
green round trip in the browser with `intelligence` unset is a false pass.

You can also confirm from the product side: open your project in the
[cloud-hosted dashboard](/premium/managed-intelligence-platform) and check that a
thread appears after you send a message. If no thread appears, the runtime is in
SSE mode.
...
See [Self-Hosting Enterprise Intelligence](/premium/self-hosting) for the full
deployment path.
