...
On the backend, a tool pauses the run with the framework's interrupt
primitive and hands its payload to the client. A headless resolver needs
nothing more from the agent than the in-chat `useInterrupt` flow does:
