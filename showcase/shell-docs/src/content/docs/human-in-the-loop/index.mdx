...
<WhenFrameworkHas flag="interrupt_pattern" absent>
  <Callout type="info" title="Built-in Agent support">
    Built-in Agent supports the tool-based `useHumanInTheLoop` pattern shown
    above. Graph-paused `useInterrupt` and headless interrupts require a backend
    graph runtime that can suspend and resume execution, so they are not
    supported by Built-in Agent.
  </Callout>
</WhenFrameworkHas>
