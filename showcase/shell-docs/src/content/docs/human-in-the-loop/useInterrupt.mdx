...
> primitive (LangGraph, AWS Strands) or a Promise-resolving frontend tool
> path. For
...
<Snippet region="backend-interrupt-tool" title="backend: schedule_meeting interrupt tool" />
...
- The payload (`{"topic": topic, "attendee": attendee}`) is what the
  ...
  while a standard AG-UI interrupt carries it on the interrupt itself.
