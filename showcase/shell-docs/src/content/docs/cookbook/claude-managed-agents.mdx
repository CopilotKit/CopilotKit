...
<Callout type="warning" title="Protect the runtime before going public">
  ...
  Metadata. Those browser checks are not authentication because custom clients can forge the headers. The public
  demo therefore adds route-specific per-IP and process-wide in-memory limits. The counters reset on restart and
  are not shared across replicas, and the 2,000-start allowance is not a fixed dollar ceiling. Scope the API key to
  a dedicated, non-default
