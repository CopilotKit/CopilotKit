...
Claude calculates the projection, calls `show_growth_projection`, and keeps its prose short while the chart
...
and MCP servers; the agent disables `web_fetch` while retaining its built-in workspace tools and `web_search`:
