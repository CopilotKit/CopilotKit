...
The helper keeps the public demo bounded: it sets `turnTimeoutMs` to 90 seconds and rejects request bodies over
256 KB before they reach the runtime. The adapter already serializes runs per thread; this recipe does not add a
global cross-thread limiter.
...
<Callout type="warning" title="Protect the runtime before going public">
  ...
  When `ALLOWED_ORIGINS` is set, the server rejects requests without the matching origin and same-origin Fetch
  Metadata. Those browser checks are not authentication because custom clients can forge the headers. Put the
  runtime behind your application session check before using this pattern in production.
</Callout>
