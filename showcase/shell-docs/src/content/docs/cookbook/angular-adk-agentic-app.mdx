...
In Angular, the run body is populated from CopilotKit `properties`, which the agent receives as `forwardedProps`:
...
export const appConfig = {
  providers: [
    provideCopilotKit({
      ...
      properties: { userId: currentUserId }, // arrives on the agent as forwardedProps
    }),
  ],
};
```

On the agent side, the AG-UI adapter copies the run's `state` onto the ADK session, so a tool reads the user id straight off `tool_context.state`:

```python title="agent/main.py"
def recall_for_user(tool_context) -> dict:
    user_id = tool_context.state.get("userId")
    # ...scope every read and write to user_id
    return {"ok": True}
```

On the BFF, **prefer the run-body value over any header** when scoping, so a stale header can never win.
...
2. Scope the user via the run body, not a header: set CopilotKit `properties: { userId }` on the
   client (it arrives on the agent as forwardedProps), and read it in ADK tools from
   `tool_context.state["userId"]`. On the BFF, prefer the run-body value over any header.
