...
A **hosted demo** is live at **[app-production-5569.up.railway.app](https://app-production-5569.up.railway.app)** — open it and send any prompt from the [governance matrix](#try-it) to watch OpenBox govern each action in real time. To run and modify it yourself, follow the steps below (`agent/` on port 8123 and `frontend/` on port 3000).

## Prerequisites

- **Node.js ≥ 20** and npm
- An **OpenAI-compatible API key** (`OPENAI_API_KEY`); the agent defaults to `gpt-4o`, override with `OPENAI_MODEL`. The governed business results are LLM-generated, so a working model key is required — not optional.
