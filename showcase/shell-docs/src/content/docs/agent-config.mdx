...
Start with the [Quickstart](/quickstart) so you have a running CopilotKit
provider, runtime route, and chat. This guide adds user-controlled preferences
to that working path; it does not replace the runtime setup. For the current
`properties` contract, see [`CopilotKitCoreConfig`](/reference/core/types/CopilotKitCoreConfig).
For the backend boundary and validation rules for browser-supplied settings, see
...
In the demo above, choose **casual**, **expert**, and **detailed**, then send
`Introduce yourself in the style I selected.` The selected Showcase agent
begins its reply by reporting the active tone, expertise, and response-length
settings, so you can verify that the current provider properties reached the
factory for that run.
