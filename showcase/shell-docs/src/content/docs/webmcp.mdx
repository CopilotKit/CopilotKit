...
See the Vue [`useFrontendTool` reference](/reference/vue/hooks/useFrontendTool) for the complete API.

</FrontendOnly>

<FrontendOnly frontend="angular">

### Angular frontend tool

Add `webmcp` to the tool you register in an Angular injection context.
...
See the Angular [`registerFrontendTool` reference](/reference/angular/functions/registerFrontendTool) for the complete API.
...
## Import page tools into CopilotKit

The path above publishes CopilotKit tools onto the page for browser agents.

Use `useWebmcpTools` (React and Vue) or `registerWebmcpTools` (Angular) to import page tools into CopilotKit. Then a CopilotKit agent can call tools that the page already registered on `document.modelContext`.

```tsx
import { useWebmcpTools } from "@copilotkit/react-core/v2";

useWebmcpTools({
  filter: (tool) => tool.name.startsWith("search"),
});
```

`filter` receives the page tool and returns true to import it. Filter order is allow, then deny, then `filter`.

Full steps: [Import page WebMCP tools](/webmcp-tools).
...
- [React `useWebmcpTools` reference](/reference/hooks/useWebmcpTools)
</FrontendOnly>
<FrontendOnly frontend="vue">
- [Vue `useFrontendTool` reference](/reference/vue/hooks/useFrontendTool)
- [Vue `useWebmcpTools` reference](/reference/vue/hooks/useWebmcpTools)
</FrontendOnly>
<FrontendOnly frontend="angular">
- [Angular `registerFrontendTool` reference](/reference/angular/functions/registerFrontendTool)
- [Angular `registerWebmcpTools` reference](/reference/angular/functions/registerWebmcpTools)
</FrontendOnly>
