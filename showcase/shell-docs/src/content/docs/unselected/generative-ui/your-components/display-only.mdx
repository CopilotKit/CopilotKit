...
The component itself is ordinary React — it reads only its props and can stream in as the agent fills the payload. The example above uses [Recharts](https://recharts.org) for the bar chart and a hand-rolled SVG donut for the pie chart; neither knows anything about CopilotKit.
