...
Pass an object to override specific props on the default component. Useful for adding `className`, event handlers, data attributes, or any other prop the default component accepts.
...
Root-level slot props available on all chat components:
