---
title: Migrate to V2
description: Upgrade the CopilotKit frontend from v1 to v2 — a single-package consolidation. Backend does not change.
...
If you only use CopilotKit's React packages, `@ag-ui/client` types are already re-exported from `@copilotkit/react-core/v2` and you don't need a separate install.
</Step>
</Steps>
...
Most v1 hooks have direct v2 equivalents, usually with a slightly cleaner API:

| v1 | v2 | Notes |
|----|----|-------|
| `useCopilotReadable` | `useAgentContext` | Renamed — semantically identical. |
| `useCopilotAction` | `useFrontendTool` + `useComponent` / `useHumanInTheLoop` | Split into narrower hooks for each use case. |
| `useCopilotChat` | `useCopilotChatHeadless_c` | Full headless UI replacement with generative UI + suggestions + interrupts. See [headless UI](../custom-look-and-feel/headless-ui). |

Everything else keeps the same name, same shape, different import path.
