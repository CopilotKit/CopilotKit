...
const agent = new BuiltInAgent({
  model: "openai:gpt-5.4-mini",
  prompt:
    "You are a customer support agent for Acme Corp. Be concise and helpful. Always check the knowledge base before answering.", // [!code highlight]
});
```
...
<Callout type="info">
  Not all parameters are supported by every provider. For example, `topK` is
  supported by Google but not OpenAI. Unsupported parameters are ignored.
</Callout>
