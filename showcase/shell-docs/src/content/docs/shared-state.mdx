...
The LangGraph adapter sends a run only the state keys in the graph's input
schema, and the model sees a key only if you add it to the model request.
The cell's graph declares `recipe` on its state annotation, and its chat
node builds a system message from `state.recipe` and puts it ahead of the
conversation on every model call:

<Snippet region="shared-state-read-agent" cell="shared-state-read" title="src/agent/shared-state-read.ts: read the recipe" />
