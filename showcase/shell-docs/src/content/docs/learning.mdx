...
Learning is available automatically when your Runtime has Intelligence and
`identifyUser` configured, just like Threads. No debug mode or separate handler
opt-in is required. Keep your application's authentication middleware on the
Runtime handler; the platform checks project credentials and Learning access.
...
| --- | --- |
| Inspector Learning is unavailable or the Runtime returns 404 | Confirm the Runtime has Intelligence and `identifyUser` configured, and the Intelligence deployment supports the Learning endpoint. |
