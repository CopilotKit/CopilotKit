...
Authentication answers *who is calling*. It does not, on its own, stop that
caller reaching someone else's conversation. The runtime's
[thread routes](/backend/runtime-endpoints#thread-routes) serve message
history, event streams and state snapshots, and outside CopilotKit
Intelligence they resolve no user.

Two facts decide what you have to write:

- **CopilotKit Intelligence scopes threads for you.** It resolves the caller
  through your `identifyUser` and passes that user into every thread
  operation, so one user's request cannot read another user's thread.
- **The default `InMemoryAgentRunner` does not.** It keeps threads in process
  memory keyed by `threadId` with no owner recorded, so those routes answer for
  any thread they are given. The same applies to anything extending it, such as
  `AgentCoreRunner`. Ownership is yours to track and enforce.

Enforce it in the `onBeforeHandler` hook, which runs after routing and
...
const handler = createCopilotRuntimeHandler({
  ...
  hooks: {
    ...
    onBeforeHandler: async ({ request, route }) => {
      // Authorization: may this caller touch this thread?
      if (!("threadId" in route)) return;
      const user = await verifyToken(request.headers.get("authorization"));
      if (!(await userOwnsThread(user.id, route.threadId))) {
        throw new Response("Forbidden", { status: 403 });
      }
...
`userOwnsThread` is yours to write. Record the owner when your frontend opens
a thread — the `threadId` is chosen by the client, so your application already
knows it — and read that record back here.

<Callout type="warn" title="Two routes carry no thread id">
`GET /threads` and `POST /threads/clear` are not about one thread: the first
lists them all and the second wipes them all. `route` has no `threadId` for
either, so the only decision `onBeforeHandler` can make is whether to allow
the call. On a deployment with more than one user, reject both unless the
...
- **Authorize the thread routes.** Verifying the token does not stop a caller naming someone else's `threadId`. See [Authorizing threads](#authorizing-threads).
