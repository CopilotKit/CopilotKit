...
<WhenFrameworkHas flag="auth_pattern" equals="ag2-dependencies">
...
The backend has two responsibilities: validate the token before the agent dispatches, and thread the resolved user identity into the run as a **dependency** so tools can read it later. Dependencies stay server-side — unlike conversation variables, they are never streamed to the client as state.
...
Once the token is validated, AG2's tools can request the user identity with the `Inject` annotation. This is how you make individual tool calls aware of who's asking, without having to thread the user object manually through every helper — and without exposing it to the LLM or the client:
...
Finally, register the tool on the agent so the LLM can call it:

```python title="backend/server.py"
from backend.tools import get_account_data # [!code highlight]

agent = Agent(
    name="assistant",
    prompt="You are a helpful assistant.",
    config=OpenAIConfig(model="gpt-5.4-mini"),
    tools=[get_account_data], # [!code highlight]
)
```
