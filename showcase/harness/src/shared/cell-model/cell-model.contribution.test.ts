...
describe("functional result admission", () => {
  it("does not certify a fresh legacy functional green", () => {
    const legacy = {
      ...row("green"),
      key: "d6:langgraph-python/gen-ui-declarative",
      dimension: "d6",
    };
    expect(classifyRung(raw("D6", [legacy]), NOW).contribution).toBe("NO_DATA");
  });
  it("keeps health green diagnostic and real functional failures visible", () => {
    expect(classifyRung(raw("D1", [row("green")]), NOW).contribution).toBe(
      "GREEN_FRESH",
    );
    const failed = {
      ...row("red"),
    ...
    expect(classifyRung(raw("D6", [failed]), NOW).contribution).toBe(
      "FAIL_FRESH",
    );
  });
});

describe("public pill proof admission", () => {
  const key = "d6:langgraph-python/gen-ui-declarative";
  function candidate() {
    return {
      ...row("green"),
      key,
      dimension: "d6",
      signal: structuredClone(unitPillSignal(key, FRESH)),
    };
  }
  it("admits complete public proof and leaves diagnostic health independent", () => {
    expect(classifyRung(raw("D6", [candidate()]), NOW).contribution).toBe(
      "GREEN_FRESH",
    );
  });
  it.each([
    [
      ...
      (proof: Record<string, unknown>) => {
        proof.surface = "direct-diagnostic";
      },
    ],
    [
      "diagnostic",
      (proof: Record<string, unknown>) => {
        proof.mode = "diagnostic";
      },
    ],
    [
      "empty",
      (proof: Record<string, unknown>) => {
        proof.requiredActions = [];
        proof.actions = [];
      },
    ],
    [
  ...
  ])("rejects %s proof", (_name, mutate) => {
    const value = candidate();
    const signal = value.signal as Record<string, unknown>;
    mutate(signal.pillExecution as Record<string, unknown>);
    expect(classifyRung(raw("D6", [value]), NOW).contribution).toBe("NO_DATA");
  });
  it("does not inherit proof when an old writer refreshes the observation", () => {
    const value = candidate();
    value.observed_at = new Date(NOW).toISOString();
    expect(classifyRung(raw("D6", [value]), NOW).contribution).toBe("NO_DATA");
  });
});

it("keeps a missing test oracle unverified without hiding a real failure", () => {
  const key = "d6:langgraph-python/gen-ui-declarative";
  const unknown = {
    ...row("red"),
    key,
  ...
  expect(classifyRung(raw("D6", [unknown]), NOW).contribution).toBe("NO_DATA");
  expect(classifyRung(raw("D6", [unknown, failed]), NOW).contribution).toBe(
    "FAIL_FRESH",
  );
});

it("does not accept another frontend in a React status key", () => {
  const key = "d6:langgraph-python/gen-ui-declarative";
  const signal = structuredClone(unitPillSignal(key, FRESH)) as Record<
    string,
    unknown
  >;
  signal.frontend = "angular";
  const proof = signal.pillExecution as Record<string, unknown>;
  (proof.identity as Record<string, unknown>).frontend = "angular";
  expect(
    classifyRung(raw("D6", [{ ...row("green"), key, signal }]), NOW)
      .contribution,
  ).toBe("NO_DATA");
});
