...
export function buildFrontendMatrix(
  ...
  const runnable = catalog.cells.filter((cell) => cell.runnable);
  if (runnable.length !== catalog.metadata.runnable) {
    throw new Error(
      `frontend catalog runnable metadata says ${catalog.metadata.runnable}; found ${runnable.length} cells`,
    );
  }
...
export function urlForFrontendCell(
  ...
  if (cell.frontend === "angular") {
    return `${baseWithoutTrailingSlash(bases.angularBaseUrl)}/${cell.integration}/${cell.feature}`;
  }
