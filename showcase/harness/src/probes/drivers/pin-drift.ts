 ...
 *      which isn't shipped into the harness runtime container, and `tsx` is
 ...
 * the harness runtime container. Operators wire a real runner via
  ...
  async run() {
    throw new Error(
      "pin-drift probe runner is not wired: inject a runner via " +
        ...
        "requires the full monorepo tree which isn't available in harness.",
    );
  },
};
...
function resolveRepoRoot(ctx: ProbeContext): string {
  ...
  // relative layout: `showcase/harness/src/probes/drivers/pin-drift.ts`
  // → `../../../../..` → repo root.
  return path.resolve(
