        ...
        return {
          async newPage(): Promise<E2eDeepPage> {
            ...
            const wrapped: E2eDeepPage = {
              ...
              goto: (url, gotoOpts) =>
                page.goto(url, gotoOpts as Parameters<typeof page.goto>[1]),
              close: () => page.close(),
...
export function createE2eDeepDriver(
  ...
  return {
    ...
    async run(
      ...
      // D5 only runs one representative fixture per feature category.
      // Filter to features that have a representative entry in the map.
      const d5Features = requestedFeatures.filter(
        (ft) => D5_REPRESENTATIVES[ft] !== undefined,
      );

      if (d5Features.length === 0) {
        return {
          key: input.key,
          state: "green",
          signal: {
          ...
          if (ageMs >= 0 && ageMs < DEPLOY_CHURN_GRACE_MS) {
            ...
            return {
              ...
              signal: {
                ...
                slug,
                backendUrl,
                total: d5Features.length,
                passed: 0,
                failed: [],
                skipped: d5Features.map(String),
      ...
      ctx.logger.info("probe.e2e-deep.service-start", {
        slug,
        featureCount: d5Features.length,
        backendUrl,
      });
...
> {
  ...
  try {
    context = await browser.newContext({
      extraHTTPHeaders: {
        "X-AIMock-Context": buildCtx.integrationSlug,
        "X-Test-Id": `d5-${buildCtx.integrationSlug}`,
      },
    });
