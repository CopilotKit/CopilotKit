 ...
 * `worker_id` null and prunes the null-scoped (i.e. legacy) partition, byte-for-
 * byte identical to the pre-fleet behavior.
 */
...
export interface ResourceSnapshotWriterOptions {
   ...
   * rows are written with `worker_id` null and the prune is scoped to the null
   * (legacy) partition, identical to the pre-fleet single-writer behavior.
   */
  workerId?: string;
    ...
    async write(event, gauges, stats, perBrowser): Promise<void> {
      ...
      try {
        await createWithTimeout({
          ...
          event,
          // Per-replica attribution (migration 1779990000). null on the legacy
          // single-process boot() path (workerId omitted) so a query separates
          // fleet-replica rows from the legacy partition.
