...
describe("resource-snapshot-writer", () => {
  ...
  it("WORKER_ID: legacy boot() path (no workerId) stamps worker_id null", async () => {
    // The legacy single-process boot() path constructs the writer with NO
    // workerId; it must write rows with worker_id null so a query can separate
    // fleet-replica rows from the legacy single-process partition.
    const { pb, created } = makeFakePb();
    const { logger } = makeLogger();
    const writer = createResourceSnapshotWriter({ pb, logger });
    ...
    expect(created).toHaveLength(1);
    expect(created[0].worker_id).toBeNull();
  });
