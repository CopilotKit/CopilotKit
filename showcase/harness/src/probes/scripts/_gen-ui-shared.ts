...
/** Require the complete authored pill inventory before dispatching a turn. */
export async function assertCanonicalPillInventory(
  page: Page,
  expectedNames: readonly string[],
  selector = '[data-testid="copilot-suggestion"]',
): Promise<void> {
  await page.waitForSelector(selector, { state: "visible", timeout: 5_000 });
  const names = await page.evaluate(
    new Function(
      `return Array.from(document.querySelectorAll(${JSON.stringify(selector)})).filter(e => e.getClientRects().length && getComputedStyle(e).visibility !== 'hidden').map(e => e.innerText);`,
    ) as () => string[],
  );
  if (JSON.stringify(names) !== JSON.stringify(expectedNames)) {
    throw new Error(
      `canonical pill inventory differs: expected ${JSON.stringify(expectedNames)}, observed ${JSON.stringify(names)}`,
    );
  }
}
...
/** Inspect each actual chart mark through its visible hover tooltip. */
export async function assertRechartsValues(
  page: Page,
  spec: CanonicalChartSpec,
): Promise<void> {
  if (!page.hover) throw new Error("real chart hover is unavailable");
  const mark =
    spec.kind === "pie" ? ".recharts-pie-sector" : ".recharts-bar-rectangle";
  const count = await page.evaluate(
    new Function(
      `const chart=document.querySelectorAll(${JSON.stringify(spec.selector)})[${spec.baseline}]; return chart && chart.getClientRects().length && getComputedStyle(chart).visibility !== 'hidden' ? chart.querySelectorAll(${JSON.stringify(mark)}).length : 0;`,
    ) as () => number,
  );
  if (!spec.expected.length || count !== spec.expected.length) {
    throw new Error(
      `canonical ${spec.kind} series length differs: expected ${spec.expected.length}, observed ${count}`,
    );
  }
  for (let index = 0; index < spec.expected.length; index++) {
    await page.hover(
      `:nth-match(${spec.selector}, ${spec.baseline + 1}) ${mark} >> nth=${index}`,
      { timeout: 5_000 },
    );
    let matches = false;
    ...
    do {
      const tip = await page.evaluate(
        new Function(
          `const tip=document.querySelectorAll(${JSON.stringify(spec.selector)})[${spec.baseline}]?.querySelector('.recharts-tooltip-wrapper'); if(!tip || !tip.getClientRects().length || getComputedStyle(tip).visibility==='hidden') return null; return {label:tip.querySelector('.recharts-tooltip-label')?.textContent ?? '',name:tip.querySelector('.recharts-tooltip-item-name')?.textContent ?? '',value:tip.querySelector('.recharts-tooltip-item-value')?.textContent ?? ''};`,
        ) as () => { label: string; name: string; value: string } | null,
      );
      const expected = spec.expected[index]!;
      matches =
        !!tip &&
        (spec.kind === "pie" ? tip.name : tip.label) === expected.label &&
        tip.value === String(expected.value);
      if (!matches) await new Promise((resolve) => setTimeout(resolve, 50));
    } while (!matches && Date.now() < deadline);
    if (!matches)
      throw new Error(
        `canonical ${spec.kind} label/value differs: ${spec.expected[index]!.label}`,
      );
  }
}
