...
export async function probeDefaultCatchall(
  ...
  return await page.evaluate(() => {
    ...
    return {
      ...
      observedToolNames,
      observedStatuses,
      ...(customLeakPhrasePresent ? { customLeakPhrasePresent } : {}),
    };
  });
}
...
export function validateDefaultCatchall(
  ...
  if (snap.statusAttributePresent === false) {
    const observedStatuses = snap.observedStatuses ?? [];
    return (
      "tool-rendering-default-catchall: container present but no " +
      "data-status attribute; observed statuses: " +
      (observedStatuses.length === 0 ? "(none)" : observedStatuses.join(", "))
    );
  }
  return null;
}
