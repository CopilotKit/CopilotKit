...
import { buildTurns } from "./d5-shared-state-read.js";
...
describe("shared-state-read canonical pill contract", () => {
  ...
  it("uses identical canonical controls across integrations", () => {
    const canonical = buildTurns(context).map(({ input, action }) => ({
      input,
      action,
    }));
    for (const integrationSlug of [
      ...
      expect(
        buildTurns({ ...context, integrationSlug }).map(
...
describe("strict visible-result regressions", () => {
  const contracts: readonly StateToolsContract[] =
    STATE_TOOLS_CONTRACTS["shared-state-read"];
  it("registers the functional builder with all required results", () => {
    expect(getD5Script("shared-state-read")).toBeDefined();
  });
