...
import { buildTurns as canonicalBuildTurns } from "./d5-shared-state.js";
...
describe("shared-state canonical pill contract", () => {
...
describe("strict visible-result regressions", () => {
  const contracts: readonly StateToolsContract[] =
    STATE_TOOLS_CONTRACTS["shared-state-write"];
  it("registers the functional builder with all required results", () => {
    expect(getD5Script("shared-state-write")).toBeDefined();
  });
