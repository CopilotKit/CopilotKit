...
import { buildTurns as canonicalBuildTurns } from "./d5-shared-state-streaming.js";
...
describe("shared-state-streaming canonical pill contract", () => {
...
describe("strict visible-result regressions", () => {
  const contracts: readonly StateToolsContract[] =
    STATE_TOOLS_CONTRACTS["shared-state-streaming"];
  it("registers the functional builder with all required results", () => {
    expect(getD5Script("shared-state-streaming")).toBeDefined();
  });
