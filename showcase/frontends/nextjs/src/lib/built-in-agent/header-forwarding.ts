 ...
 * tanstack-factory constructs `openaiText("gpt-5.4", { fetch: forwardingFetch })`
...
export function withForwardedHeaders<T>(
  ...
  //
  // GATING RULE (the same one `forwardingFetch` below states and applies):
  // only emit CVDIAG when a diagnostic header is present (x-diag-run-id OR
  // x-aimock-context). This log used to run on EVERY request, which is one
  // noisy line per inbound request in production while the outbound half of
  // the same rule stayed quiet — the two halves of one rule disagreeing.
