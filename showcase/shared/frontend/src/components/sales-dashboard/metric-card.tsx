...
const TREND_STYLES: Record<"up" | "down" | "neutral", { color: string; arrow: string }> = {
...
export function MetricCard({ label, value, trend }: MetricCardProps) {
  return (
    <div data-testid="metric-card" className="rounded-lg border border-[var(--border)] bg-[var(--card)] p-4">
      ...
      {trend && (
        <p data-testid="trend-indicator" className={`text-sm font-medium mt-1 ${TREND_STYLES[trend.direction].color}`}>
