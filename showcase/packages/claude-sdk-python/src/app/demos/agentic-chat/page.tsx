...
function Chat() {
  ...
  useFrontendTool({
    ...
    description:
      "Change the background color of the chat. ONLY call this tool when the user explicitly asks to change the background. Never call it proactively. Can be anything the CSS background attribute accepts. Prefer gradients.",
    parameters: z.object({
      background: z
        .string()
  ...
  useRenderTool({
    ...
    render: ({ args, result, status }: any) => {
      ...
      const temp = result?.temperature ?? args.temperature ?? 22;
      const cond = result?.conditions || args.conditions || "Clear";
      const hum = result?.humidity ?? args.humidity ?? 55;
      const wind = result?.wind_speed ?? args.wind_speed ?? 12;
      const feels = result?.feels_like ?? args.feels_like ?? temp;
  ...
  useConfigureSuggestions({
    suggestions: [
      {
        title: "Add a proverb",
        message: "Add a proverb about perseverance.",
      },
      {
        title: "Weather check",
