...
function SidebarWithSuggestions() {
    useConfigureSuggestions({
        suggestions: [
            { title: "AI Proverb", message: "Create a proverb about artificial intelligence." },
            { title: "Nature Proverb", message: "Create a proverb about nature and patience." },
            { title: "Wisdom Proverb", message: "Generate a proverb about the value of wisdom." },
        ],
        available: "always",
    });
    ...
    useFrontendTool(
        {
            ...
            parameters: z.object({
                text: z
                    .string()
                    .describe("The proverb text"),
                gradient: z
                    .string()
                    .describe("CSS Gradient color for the background"),
            }),
            followUp: false,
            handler: async ({
                text,
                ...
                setProverbs((prev) => [
                    newProverb,
                    ...prev.filter(
                        (p) => p.text !== "CopilotKit may be new, but it's the best thing since sliced bread."
                    ),
                ]);
...
function ProverbCard({ proverb }: { proverb: Partial<Proverb> }) {
    return (
        <div
            ...
            style={{ position: "relative", borderRadius: "16px", margin: "24px 0", padding: "32px", maxWidth: "42rem", border: "1px solid #e2e8f0", overflow: "hidden", background: proverb.gradient || "linear-gradient(to bottom right, #f8fafc, #eff6ff)" }}
        >
