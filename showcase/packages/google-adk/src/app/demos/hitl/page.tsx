...
function StepSelector({
    ...
    const [localSteps, setLocalSteps] = useState<Step[]>(() =>
        steps.map((s) => ({
            ...
            status: (typeof s === "object" && s.status === "disabled" ? "disabled" : "enabled") as Step["status"],
        }))
    );
            ...
            <div className="space-y-1 mb-6">
                {localSteps.map((step, i) => (
                    <label key={i} data-testid="step-item" style={{ display: "flex", alignItems: "center", gap: "12px", padding: "10px 16px", borderRadius: "8px", cursor: "pointer" }}>
                        ...
                        />
                        <span className={step.status !== "enabled" ? "line-through text-gray-400" : "text-gray-800"} data-testid="step-text">
                            {step.description}
                        </span>
                    </label>
                ))}
...
function StepsFeedback({ args, respond, status }: { args: any; respond: any; status: any }) {
    ...
    return (
        <div data-testid="select-steps" style={{ borderRadius: "16px", border: "1px solid #e5e5e0", background: "#fff", padding: "32px", boxShadow: "0 8px 30px rgba(0,0,0,0.08)", width: "520px" }}>
            ...
            {decided !== null && (
                <div className={`text-center py-2 rounded-lg font-medium ${decided ? "bg-green-50 text-green-700" : "bg-red-50 text-red-700"}`}>
