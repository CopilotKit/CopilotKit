...
// E2E smoke for the agent-config demo (AWS Strands showcase).
//
...
test("agent-config loads without errors", async ({ page }) => {
  const errors: string[] = [];
  ...
  expect(errors, `page errors on /demos/agent-config: ${errors.join(" | ")}`).toEqual([]);
});
