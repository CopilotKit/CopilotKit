...
// E2E smoke for the declarative-gen-ui demo (AWS Strands showcase).
//
...
test("declarative-gen-ui loads without errors", async ({ page }) => {
  const errors: string[] = [];
  ...
  expect(errors, `page errors on /demos/declarative-gen-ui: ${errors.join(" | ")}`).toEqual([]);
});
