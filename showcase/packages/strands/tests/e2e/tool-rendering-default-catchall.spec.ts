...
test("tool-rendering-default-catchall loads without errors", async ({ page }) => {
  const errors: string[] = [];
  ...
  expect(errors, `page errors on /demos/tool-rendering-default-catchall: ${errors.join(" | ")}`).toEqual([]);
});
