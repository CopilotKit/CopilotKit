...
// E2E smoke for the voice demo (AWS Strands showcase).
//
...
test("voice loads without errors", async ({ page }) => {
  const errors: string[] = [];
  ...
  expect(errors, `page errors on /demos/voice: ${errors.join(" | ")}`).toEqual(
    [],
  );
});
