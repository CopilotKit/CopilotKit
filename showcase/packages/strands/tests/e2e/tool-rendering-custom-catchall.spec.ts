...
// E2E smoke for the tool-rendering-custom-catchall demo (AWS Strands showcase).
//
...
test("tool-rendering-custom-catchall loads without errors", async ({ page }) => {
  const errors: string[] = [];
  ...
  expect(errors, `page errors on /demos/tool-rendering-custom-catchall: ${errors.join(" | ")}`).toEqual([]);
});
