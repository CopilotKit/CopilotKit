...
// E2E smoke for the chat-customization-css demo (AWS Strands showcase).
//
...
test("chat-customization-css loads without errors", async ({ page }) => {
  const errors: string[] = [];
  ...
  expect(errors, `page errors on /demos/chat-customization-css: ${errors.join(" | ")}`).toEqual([]);
});
