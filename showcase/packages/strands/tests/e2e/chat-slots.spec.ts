...
// E2E smoke for the chat-slots demo (AWS Strands showcase).
//
...
test("chat-slots loads without errors", async ({ page }) => {
  const errors: string[] = [];
  ...
  expect(errors, `page errors on /demos/chat-slots: ${errors.join(" | ")}`).toEqual([]);
});
