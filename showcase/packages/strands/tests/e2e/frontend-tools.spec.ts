...
// E2E smoke for the frontend-tools demo (AWS Strands showcase).
//
...
test("frontend-tools loads without errors", async ({ page }) => {
  const errors: string[] = [];
  ...
  expect(errors, `page errors on /demos/frontend-tools: ${errors.join(" | ")}`).toEqual([]);
});
