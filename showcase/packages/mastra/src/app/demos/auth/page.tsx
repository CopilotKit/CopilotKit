...
// Auth demo (Mastra) — framework-native request authentication via the V2
// runtime's `onRequest` hook. The banner toggles an in-memory React auth
// flag; when `authenticated === true`, <CopilotKit headers={...}> injects
// the `Authorization: Bearer <demo-token>` header on every request. The
// runtime route (/api/copilotkit-auth) rejects any request without the
// header.
//
// Default UX: the page loads authenticated so the initial `/info`
// handshake succeeds and the chat is immediately usable. Clicking "Sign
// out" flips to the unauthenticated state — the next chat submission (and
// any re-fetch of `/info`) will 401, which we surface via the page-level
// error banner. This inverts the historical unauth-first flow, which
// crashed the page on load when `/info` returned 401 before `onError`
// handlers could attach.
//
...
// <CopilotKit onError> prop and renders a persistent error banner below
// the chat. A local ErrorBoundary guards against any uncaught render-time
// error from chat internals in the unauthenticated state so the page
// never white-screens.
