...
import { CopilotChat, useHumanInTheLoop, useConfigureSuggestions } from "@copilotkit/react-core/v2";
import { z } from "zod";
...
function DemoContent() {
    ...
    render: ({ event, resolve }) => (
      <StepSelector
        ...
        onConfirm={(selectedSteps) => {
          resolve("The user selected the following steps: " + selectedSteps.map((s) => s.description).join(", "));
        }}
      />
    ),
  });
  ...
  useHumanInTheLoop({
    ...
    render: ({ args, respond, status }: any) => <StepsFeedback args={args} respond={respond} status={status} />,
  });
      ...
      <div className="space-y-2 mb-4">
        {localSteps.map((step, i) => (
          ...
          >
            ...
            />
            <span
              className={step.status !== "enabled" ? "line-through text-gray-400" : "text-gray-800"}
