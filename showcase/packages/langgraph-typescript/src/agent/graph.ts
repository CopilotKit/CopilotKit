...
const getWeather = tool(async ({ location }) => JSON.stringify(getWeatherImpl(location)), {
  name: "get_weather",
...
const queryData = tool(async ({ query }) => JSON.stringify(queryDataImpl(query)), {
  name: "query_data",
...
const manageSalesTodos = tool(async ({ todos }) => JSON.stringify(manageSalesTodosImpl(todos)), {
...
const getSalesTodos = tool(async ({ currentTodos }) => JSON.stringify(getSalesTodosImpl(currentTodos)), {
...
const scheduleMeeting = tool(
  async ({ reason, durationMinutes }) => JSON.stringify(scheduleMeetingImpl(reason, durationMinutes)),
  {
...
const searchFlights = tool(async ({ flights }) => JSON.stringify(searchFlightsImpl(flights)), {
  name: "search_flights",
...
const tools = [getWeather, queryData, manageSalesTodos, getSalesTodos, scheduleMeeting, searchFlights, generateA2ui];
