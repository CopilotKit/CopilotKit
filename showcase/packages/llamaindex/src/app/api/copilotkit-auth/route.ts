...
// agent.
//
// This uses `createCopilotRuntimeHandler` from `@copilotkit/runtime/v2`
// directly — the framework-agnostic fetch handler — because the V1 Next.js
// adapter does NOT forward the `hooks` option.
...
const authDemoAgent = new HttpAgent({ url: `${AGENT_URL}/run` });

const runtime = new CopilotRuntime({
  // @ts-ignore -- see main route.ts
  agents: {
    "auth-demo": authDemoAgent as AbstractAgent,
    default: authDemoAgent as AbstractAgent,
  },
});
