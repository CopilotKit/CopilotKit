...
/**
 * Declarative Generative UI (A2UI — Dynamic Schema) — hardcoded catalog variant.
 *
 * The agent (see `src/agents/a2ui_dynamic_hardcoded.py`) emits an
 * `a2ui_operations` payload at runtime — it *designs* the component tree on
 * the fly using a secondary LLM with
 * `bind_tools([render_a2ui], tool_choice="render_a2ui")`. The runtime's A2UI
 * middleware (`a2ui: { injectA2UITool: true }` in the shared
 * `src/app/api/copilotkit/route.ts`) detects the operations and forwards
 * them to the frontend, where they render against the `demoCatalog` below.
 *
...
export default function DeclarativeGenUIHardcodedDemo() {
  return (
    <CopilotKit
