...
export function Callout({ type = "info", children }: { type?: "info" | "warn" | "error"; children: React.ReactNode }) {
  ...
  return (
    <div className={`my-4 rounded-md border-l-4 ${s.border} ${s.bg} p-4 text-sm text-[var(--text-secondary)]`}>
...
export function Accordion({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <details className="group rounded-lg border border-[var(--border)] bg-[var(--bg-surface)]">
      ...
      <div className="px-4 pb-4 text-sm text-[var(--text-secondary)]">{children}</div>
    </details>
  );
}
