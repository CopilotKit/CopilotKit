...
<Callout type="warning">
  **Deprecated.** `useCopilotChatHeadless_c` is still supported during the
  deprecation window, but new fully custom chat UIs should use
  [`useAgent`](/reference/hooks/useAgent) with
  `useCopilotKit().copilotkit.runAgent` from the v2 API. See the
  [migration guide](/migration-guides/migrate-use-copilot-chat-headless-c).
</Callout>

CopilotKit previously offered _fully headless UI_ through the `useCopilotChatHeadless_c` hook. By using this hook, you can build your own chat interfaces
...
### With registered tool renderers

When you build a fully headless UI, use `useToolRenderingResolver` to render tool calls with the same renderers you register through `useRenderTool`, `useFrontendTool`, or `useHumanInTheLoop`.
...
export const Chat = () => {
  const { agent } = useAgent({ agentId: "weather-agent" });
  const { copilotkit } = useCopilotKit();
  const messages = agent.messages as Message[];
  const resolveToolRendering = useToolRenderingResolver(); // [!code highlight]
        ...
        <div key={message.id}>
          ...
          {message.role === "assistant" &&
            message.toolCalls?.map((toolCall) => {
              ...
              return <div key={toolCall.id}>{renderedToolElement}</div>;
            })}
        </div>
      ))}
      <button onClick={() => void sendMessage("What's the weather in Lisbon?")}>
        Ask
      </button>
    </div>
  );
