...
Enable A2UI in your CopilotRuntime and the middleware will handle the
...
const runtime = new CopilotRuntime({
  ...
  // `injectA2UITool: true` auto-injects the `render_a2ui` tool and
  // catalog schema into the agent (only needed for dynamic schemas).
