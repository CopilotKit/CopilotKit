...
For simple components that don't need typed parameters, skip the Zod
schema — the render function still receives whatever props the agent
passes through:
...
## Scoping to a specific agent

In multi-agent setups, pin a component to a single agent so it only
appears in that agent's thread:
