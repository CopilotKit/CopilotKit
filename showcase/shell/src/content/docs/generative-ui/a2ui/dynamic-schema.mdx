...
<Callout type="info">
Prefer a single-file prototype? The showcase also ships the
`declarative-gen-ui-hardcoded` cell — the same Card / Title / Metric /
PrimaryButton vocabulary, but definitions + renderers + catalog all in
one `catalog.tsx`. Useful for a quick demo; graduate to the 3-file
split the moment more than one surface needs the catalog.
</Callout>
...
## Customising the generation prompt

The `a2ui_prompt()` helper builds the system prompt handed to the
secondary LLM — it bundles the full A2UI JSON schema reference plus
default generation and design guidelines. Pass overrides to tighten
the output to your product's style:
...
prompt = a2ui_prompt(
    ...
    design_guidelines="Use a minimal, monochrome aesthetic.",
)
```

The showcase cell ships with the defaults; swap in your own guidelines
to lock the generated UI to a specific component vocabulary or visual
language.
