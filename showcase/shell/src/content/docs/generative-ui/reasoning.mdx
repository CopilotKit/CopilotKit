...
## Tweaking just one piece — slot sub-components

The reasoning card is composed of three sub-slots. Pass an object (not
a component) to `reasoningMessage` to override just the parts you want
and keep the rest of the built-in card:
...
`CustomHeader` receives `{ isOpen, label, hasContent, isStreaming,
onClick }`; `CustomContent` receives `{ isStreaming, hasContent,
children }` (the raw reasoning text). Swap only what you need.
