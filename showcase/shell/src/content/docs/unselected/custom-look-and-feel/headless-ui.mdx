...
A headless UI gives you full control over the chat experience — you bring your own components, layout, and styling while CopilotKit handles agent communication, message management, and streaming. This is built on top of the same primitives (`useAgent` and `useCopilotKit`) covered in [Programmatic Control](../programmatic-control).

## When should I use this?

Use headless UI when the [slot system](./slots) isn't enough — for example, when you need a completely different layout, want to embed the chat into an existing UI, or are building a non-chat interface that still communicates with an agent.
...
If your LangChain agent shares state with the frontend, access it via `agent.state`:
...
- [Programmatic Control](../programmatic-control) — Full `useAgent` reference and advanced patterns
- [Component Slots](./slots) — Customize the built-in UI without going fully headless
