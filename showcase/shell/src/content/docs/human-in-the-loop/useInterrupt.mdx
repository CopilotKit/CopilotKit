...
<Callout type="info">
  Your agent won't be aware of the `interrupt` interaction by default — to
  LangGraph, `interrupt()` returns the resume value and the model sees only
  the tool's final return. If you want the model to see the human exchange
  as part of its conversation, have the tool append the resolution into a
  message it returns.
</Callout>
