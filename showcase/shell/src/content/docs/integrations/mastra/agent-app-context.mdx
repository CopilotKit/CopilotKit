...
<Steps>
    <Step>
        ...
        ```tsx title="YourComponent.tsx" showLineNumbers {1, 15-18}
    ...
    <Step>
        ...
        export const colleaguesContactAgent = new Agent({
            ...
            instructions: ({ runtimeContext }) => {
              ...
              const aguiContext = runtimeContext.get('ag-ui') as { context: Array<{ description: string; value: unknown }> };
              const colleaguesContextItem = aguiContext.context.find((contextItem: { description: string; }) => contextItem.description === "The current user's colleagues")
              return `
