...
supervisor-and-its-tools:

- **Router mode** (default) — omit the `agent` prop on `<CopilotKit>`
  and the runtime's LLM routes each user turn to the most appropriate
  registered agent.
  ```tsx
  <CopilotKit runtimeUrl="<copilot-runtime-url>">
    {/* the LLM picks between all registered agents per turn */}
  </CopilotKit>
  ```
- **Agent-lock mode** — pass `agent="<name>"` and every turn is routed
  to that single agent, no matter what the LLM would pick.
  ```tsx
  <CopilotKit runtimeUrl="<copilot-runtime-url>" agent="research">
    {/* every turn goes to the `research` agent */}
  </CopilotKit>
  ```
