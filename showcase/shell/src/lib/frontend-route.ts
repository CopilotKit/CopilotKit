...
/** Return runnable frontend identities in registry order. */
...
/** Build the frontend-aware route for one Showcase cell. */
...
/** Map a legacy React demo URL to its frontend-aware route. */
...
function unavailableReason(cell: FrontendCatalogCell): string {
  ...
  if (cell.frontend_status === "docs-only") {
    return "This feature has docs but no runnable demo.";
  }
...
/** Resolve a frontend, integration, and feature to a URL or named stop state. */
