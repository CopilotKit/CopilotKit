...
const NAV_DEFINITION: NavTab[] = [
  {
    ...
    sections: [
          ...
          "agentic-protocols",
          {
            ...
            children: [
              "quickstart/applications",
              {
                ...
                children: ["quickstart/introduction", "quickstart/server", "quickstart/middleware"],
              },
              "quickstart/clients",
            ],
          },
      ...
      {
        section: "Development",
        entries: ["development/updates", "development/roadmap", "development/contributing"],
      },
    ],
  },
  {
  ...
  return (
    <div className="mx-auto max-w-3xl px-6 py-16 text-center">
      ...
      <p className="text-base text-[var(--text-secondary)] leading-relaxed mb-10">
        AG-UI is an open protocol for connecting AI agents to frontend applications. It defines a standard event-based
        interface for streaming agent state, tool calls, and generative UI to any client.
      </p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 text-left mb-10">
        ...
        >
          <h3 className="text-sm font-semibold text-[var(--text)] group-hover:text-[var(--violet)] mb-1">Concepts</h3>
          <p className="text-xs text-[var(--text-muted)]">Architecture, events, agents, state, tools, middleware</p>
        </Link>
        <Link
        ...
        >
          ...
          <p className="text-xs text-[var(--text-muted)]">Build your first AG-UI integration step by step</p>
        </Link>
        <Link
        ...
        >
          ...
          <p className="text-xs text-[var(--text-muted)]">@ag-ui/core, @ag-ui/client, @ag-ui/encoder</p>
        </Link>
        <Link
          href="/ag-ui/sdk/python/core/overview"
          className="group p-5 rounded-xl border border-[var(--border)] bg-[var(--bg-surface)] hover:border-[var(--violet)] transition-all"
        >
          <h3 className="text-sm font-semibold text-[var(--text)] group-hover:text-[var(--violet)] mb-1">Python SDK</h3>
          <p className="text-xs text-[var(--text-muted)]">ag_ui.core, ag_ui.encoder</p>
        </Link>
        <Link
          href="/ag-ui/tutorials/cursor"
          className="group p-5 rounded-xl border border-[var(--border)] bg-[var(--bg-surface)] hover:border-[var(--violet)] transition-all"
        >
          <h3 className="text-sm font-semibold text-[var(--text)] group-hover:text-[var(--violet)] mb-1">Tutorials</h3>
          <p className="text-xs text-[var(--text-muted)]">Hands-on guides and debugging walkthroughs</p>
        </Link>
        <a
          ...
          className="group p-5 rounded-xl border border-[var(--border)] bg-[var(--bg-surface)] hover:border-[var(--violet)] transition-all"
        >
          <h3 className="text-sm font-semibold text-[var(--text)] group-hover:text-[var(--violet)] mb-1">GitHub</h3>
          <p className="text-xs text-[var(--text-muted)]">Open source · Apache 2.0 · ag-ui-protocol/ag-ui</p>
        </a>
      </div>

      <p className="text-xs text-[var(--text-faint)]">2 SDKs · 15+ framework adapters · Open protocol</p>
    </div>
  );
}

export default async function AgUiDocPage({ params }: { params: Promise<{ slug?: string[] }> }) {
  ...
  function renderNavItem(item: ResolvedNavItem, depth: number = 0): React.ReactNode {
      ...
      return (
        <Link
          ...
          className={`block py-[5px] text-[14px] transition-colors ${
            isActive ? "text-[var(--text)] font-medium" : "text-[var(--text-secondary)] hover:text-[var(--text)]"
          }`}
    ...
    return (
      <div key={item.name} className="mt-2">
        <div className="py-[5px] text-[14px] text-[var(--text-muted)]" style={{ paddingLeft: `${indent}px` }}>
          {item.name}
        </div>
  ...
  return (
    <div className="flex" style={{ height: "calc(100vh - 52px)" }}>
      ...
      <SidebarNav className="w-[220px] shrink-0 border-r border-[var(--border)] bg-[var(--bg)] overflow-y-auto p-4">
        <Link href="/ag-ui" className="block text-xs font-mono uppercase tracking-widest text-[var(--violet)] mb-4">
          AG-UI Protocol
        </Link>
        {navTabs.map((tab, i) => (
          <div key={tab.tab} className={i > 0 ? "mt-6 pt-5 border-t border-[var(--border)]" : ""}>
            {tab.sections.map(({ section, items }) => (
              <div key={section} className="mb-5">
                <div className="text-[13px] font-semibold text-[var(--text)] mb-2">{section}</div>
