...
<Variant for="langgraph adk a2a ag2 agent-spec agno aws-strands crewai-flows llamaindex mastra microsoft-agent-framework pydantic-ai">
  <Note>
    This page covers `defineTool` and `BuiltInAgent.tools` — Built-in Agent's server-side tool API. Your integration defines tools natively in its own framework (LangChain nodes, ADK `tools=[...]`, Mastra `agent.tool(...)`, Strands tools, etc.). Use your framework's tool API directly — those tools are automatically forwarded to the LLM through AG-UI without extra CopilotKit wiring.
  </Note>
</Variant>
...
<Info>
  If you're using a [custom backend](/custom-agent) instead of `BuiltInAgent`'s simple mode, see the "With Tools" examples for how to wire tools with AI SDK, TanStack AI, or custom backends.
</Info>
</Variant>
