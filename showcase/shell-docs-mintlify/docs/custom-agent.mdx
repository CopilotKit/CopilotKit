...
When you need full control over the LLM call — a custom backend, a non-default SDK, or a model the simple `provider:model` string can't reach — `BuiltInAgent` accepts a factory function. You provide the factory; CopilotKit handles converting the stream to AG-UI events, managing lifecycle, and wiring everything into the runtime.
...
const copilotEndpoint = createCopilotHonoHandler({
  runtime,
...
With `type: "custom"`, you yield AG-UI events directly. See the [AG-UI event reference](/ag-ui) for all available event types.
</Tab>
</Tabs>
...
const agent = new BuiltInAgent({
  ...
  factory: ({ input, abortSignal }) => {
    ...
    // Add shared application state (useAgent / agent.setState)
...
Factory mode does not inject state management tools automatically. If your app uses shared state (`useAgent` with `agent.setState`), add the tools yourself:
