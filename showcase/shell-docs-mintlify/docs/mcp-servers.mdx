...
<Variant for="langgraph adk a2a ag2 agent-spec agno aws-strands crewai-flows llamaindex mastra microsoft-agent-framework pydantic-ai">
  <Note>
    This page covers `BuiltInAgent.mcpServers` and `mcpClients` — Built-in Agent's MCP integration. To wire MCP servers into your integration, use your framework's native MCP support (e.g. LangChain's MCP client, Mastra's MCP integration, ADK's MCP toolset). The resulting tools flow to the LLM through your framework just like any other tool, and CopilotKit forwards their UI hooks via AG-UI as usual.
  </Note>

  <Info>
    If you want to apply MCP across **all** registered agents from a single place, the `mcpApps` option on `CopilotRuntime` works for any integration — see [CopilotRuntime](/copilot-runtime#mcpapps).
  </Info>
</Variant>
