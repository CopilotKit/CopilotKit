...
Agents expose bidirectional state through `agent.state` and `agent.setState()`. Your components re-render automatically when the agent updates state, and state you write from the frontend is available to the agent on its next run. For a full explanation and live examples, see [Shared State](/shared-state).
