...
<Variant for="langgraph adk a2a ag2 agent-spec agno aws-strands crewai-flows llamaindex mastra microsoft-agent-framework pydantic-ai">
  <Note>
    This page covers `BuiltInAgent`-specific options like `maxSteps`, `temperature`, `providerOptions`, and `overridableProperties`. Your integration tunes these in its own framework — for example, LangChain configures generation parameters on the `ChatOpenAI` / `ChatAnthropic` instance, ADK on the `LlmAgent`, Mastra on the `Agent` config. Refer to your framework's docs for the equivalent knobs.
  </Note>

  <Info>
    Runtime-level features that DO apply across integrations — MCP apps, before/after hooks — are covered in [Copilot Runtime](/copilot-runtime).
  </Info>
</Variant>
...
<Info>
  If `BuiltInAgent`'s configuration options don't cover your needs — for example, if you want to use TanStack AI, a custom LLM backend, or async initialization — see [Bring your own model](/custom-agent).
</Info>
</Variant>
