...
const slugs = fs
  ...
  .filter((d) => {
    ...
    return (
      fs.statSync(p).isDirectory() && fs.existsSync(path.join(p, "report.json"))
    );
  })
  .sort();
...
function parseSections(content) {
  ...
  for (let i = 0; i < SECTION_MARKERS.length; i++) {
    ...
    const contentEnd = end
      ? content.indexOf(end, contentStart)
      : content.length;
    sections[SECTION_MARKERS[i]] = content
...
const ports = slugs.map((slug) => {
  ...
  const report = JSON.parse(
    fs.readFileSync(path.join(portDir, "report.json"), "utf-8"),
  );
...
console.log(
  `Aggregating ${ports.length} ports: ${ports.map((p) => p.slug).join(", ")}`,
);
...
for (const [section, heading] of Object.entries(SECTION_TO_HEADING)) {
  ...
  if (newBlocks) {
    ...
    console.log(
      `  + ${section}: spliced ${newBlocks.match(/<Variant for=/g)?.length ?? 0} blocks`,
    );
  }
}
...
let ioBody = im[2];
for (const port of ports) {
  if (!port.integrationOnlyPages || port.integrationOnlyPages.length === 0)
    continue;
