...
/** Measure raw, gzip, and Brotli bytes in the static initial graph. */
export function measureInitialGraph(rootDirectory = projectDirectory) {
  const outputDirectory = join(rootDirectory, "dist/showcase-angular/browser");
  const stats = JSON.parse(
    readFileSync(
      join(rootDirectory, "dist/showcase-angular/stats.json"),
      "utf8",
    ),
  );
...
function run() {
  ...
  const { files, ...sizes } = measureInitialGraph();
