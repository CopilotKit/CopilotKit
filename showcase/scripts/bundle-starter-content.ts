...
function discoverStarterFiles(starterPath: string): StarterFile[] {
  ...
  if (isTurborepo) {
    ...
    if (fs.existsSync(agentDir)) {
      ...
      const agentSubdirs = fs.readdirSync(agentDir, { withFileTypes: true }).filter((e) => e.isDirectory());
      for (const subdir of agentSubdirs) {
        const subFiles = collectAgentFiles(path.join(agentDir, subdir.name), `agent/${subdir.name}`);
        files.push(...subFiles);
      }
    }
    ...
    const componentDirs = ["src/components/sales-dashboard", "src/components/renderers", "src/components/charts"];
