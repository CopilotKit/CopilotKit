    ...
    feature_support: {
      ...
      "cli-start": {
        ...
        angular: {
          ...
          reason: "Angular CLI scaffolding is outside this delivery.",
...
it("rejects contradictory docs-only declarations", () => {
  ...
  expect(() => normalizeFrontendRegistry(registry, features)).toThrow(
    'docs-only feature "cli-start" cannot be supported for "angular"',
  );
});
...
it("rejects an expired quarantine", () => {
  ...
  expect(() =>
    normalizeFrontendRegistry(registry, features, { today: "2026-07-22" }),
  ).toThrow(/quarantine.*expired.*2026-07-21/i);
});
