...
async function captureDemo(
    ...
    try {
        ...
        // Find the video file
        const videoFiles = fs.readdirSync(videoSubDir)
        ...
        if (videoFiles.length === 0) {
            return { integrationSlug: target.integrationSlug, demoId: target.demoId, success: false, error: "No video file produced" };
        }
        ...
        try { fs.rmSync(videoSubDir, { recursive: true, force: true }); } catch { /* ignore */ }

        if (!gifOk) {
            return { integrationSlug: target.integrationSlug, demoId: target.demoId, success: false, error: "ffmpeg conversion failed" };
        }
        ...
        // Clean up video subdir
        try { fs.rmSync(videoSubDir, { recursive: true, force: true }); } catch { /* ignore */ }
        return { integrationSlug: target.integrationSlug, demoId: target.demoId, success: false, error: msg };
    }
}
