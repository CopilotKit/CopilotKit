    ...
    while (i < lines.length) {
        ...
        if (checkboxMatch) {
            blocks.push({
                ...
                to_do: {
                    rich_text: [
                        { text: { content: checkboxMatch[2].trim() } },
                    ],
...
async function syncToNotion(qaFiles: QAFile[]): Promise<void> {
    ...
    for (const [slug, { name, files }] of byIntegration) {
        ...
        if (!integrationPageId) {
            ...
            integrationPageId = await createPage(
                QA_PARENT_PAGE_ID,
                name,
                "📦"
            );
        ...
        for (const file of files) {
            ...
            let featurePageId = await findChildPage(
                integrationPageId,
                pageTitle
            );
            ...
            if (!featurePageId) {
                ...
                featurePageId = await createPage(
                    integrationPageId,
                    pageTitle,
                    "✅"
                );
...
async function main() {
    if (!NOTION_API_KEY) {
        ...
        console.error(
            "and share the QA parent page with it."
        );
        process.exit(1);
    }
