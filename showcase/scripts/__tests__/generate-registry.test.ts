...
function runGenerator(tmpDir: string): { code: number; stdout: string; stderr: string } {
...
describe("Registry Generator", () => {
    it("generates registry.json from existing packages", async () => {
        ...
        expect(langgraph.features.length).toBe(9);
        expect(langgraph.demos.length).toBe(9);
    });
