...
describe("pr-tour-video-plan", () => {
    ...
    demos: [
      {
        ...
        route: "/demos/tool-rendering-suppress-catchall",
        highlight: [],
      },
      {
  ...
  const report: PrTourReport = {
    ...
    columns: [langGraphFastApiColumn],
    cells: [
      {
        row: {
          id: "tool-rendering-suppress-catchall",
  ...
  it("points custom catch-all code tours at the hook registration", () => {
    const customReport: PrTourReport = {
      ...report,
      rows: [
        {
          id: "tool-rendering-custom-catchall",
          name: "Generative UI: Tool Rendering (Custom catch-all)",
        },
      ],
      cells: [
        {
    ...
    const [topic] = buildShowcaseTourPlan(customReport, {
      shellUrl: "http://shell.test",
      dashboardUrl: "http://dashboard.test",
      outputDir: "/tmp/videos",
    });

    expect(topic.cells[0].codeTarget?.file).toContain("page.tsx");
    expect(topic.cells[0].codeTarget?.matchedNeedles).toContain(
  ...
  it("can point showcase previews at a staging backend host pattern", () => {
    const [topic] = buildShowcaseTourPlan(report, {
      shellUrl: "http://shell.test",
      dashboardUrl: "http://dashboard.test",
      outputDir: "/tmp/videos",
      backendHostPattern: "https://showcase-{slug}-staging.up.railway.app",
    });

    expect(topic.cells[0].previewUrl).toBe(
      "https://showcase-langgraph-fastapi-staging.up.railway.app/demos/tool-rendering-suppress-catchall",
    );
    expect(topic.cells[0].codeUrl).toContain("http://shell.test");
  });
