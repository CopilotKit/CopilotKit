...
async function validateEntry(
    ...
    if (!expanded) {
        return { id: entry.id, source: entry.source, expectedDestination: entry.destination, status: "skip", error: "Could not expand wildcard" };
    }
    ...
    try {
        ...
        if (res.status !== 301) {
            return {
                id: entry.id, source, expectedDestination: destination, status: "fail",
        ...
        if (actualPath !== destination) {
            return {
                ...
                actualStatus: 301, actualLocation: actualPath,
            };
        }

        // Step 2: Follow redirect, check destination exists
        const destRes = await fetch(`${baseUrl}${destination}`, { redirect: "follow" });
        if (destRes.status >= 400) {
            return {
                id: entry.id, source, expectedDestination: destination, status: "fail",
                error: `Destination returned ${destRes.status}`,
                actualStatus: 301, actualLocation: actualPath, destinationStatus: destRes.status,
            };
        }

        return {
            id: entry.id, source, expectedDestination: destination, status: "pass",
            actualStatus: 301, actualLocation: actualPath, destinationStatus: destRes.status,
        };
    } catch (err) {
        return {
...
async function main() {
    ...
    for (const entry of entries) {
        ...
        console.log(`  ${icon} ${result.id}: ${result.source} → ${result.expectedDestination}${result.error ? ` (${result.error})` : ""}`);
    }
