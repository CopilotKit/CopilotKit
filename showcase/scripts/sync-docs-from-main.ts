...
function hasShowcaseLocalModifications(
    ...
    try {
        ...
        // File didn't exist at last sync, or git show failed
        return false;
    }
}
...
function getLastSyncSha(): string {
    ...
    // No marker: use a reasonable default (30 days ago)
    return execSync("git rev-parse HEAD~100", { encoding: "utf-8", cwd: ROOT }).trim();
}
...
function main(): SyncResult {
    ...
    const headSha = execSync("git rev-parse HEAD", { encoding: "utf-8", cwd: ROOT }).trim();
