...
- **multimodal needs LFS assets in the image.** The demo's sample attachments (`public/demo-files/sample.png|pdf`) are Git-LFS-tracked and baked into the image (not hot-mounted). If the image is built without `git lfs pull` (e.g. locally on a host lacking git-lfs), the container serves 130-byte LFS *pointers*, the frontend throws before sending, and multimodal reds with `dom-missing` — even though the source is byte-identical to LGP. Build must materialize LFS assets (as LGP's image is). See follow-up below.
...
8. **A single-pass sweep hides flaky cells; and aimock-green can mask a real backend bug.** `bin/showcase test <slug> --d6` runs each cell ONCE, so a flaky cell lands green ~50% of the time and the sweep looks cleaner than reality (OSS-583: a single pass showed 34/39, but `frontend-tools-async` and `a2ui-recovery` were flaky and settled red on re-runs — true stable state was worse). Re-run any suspect cell 3–4× to confirm STABLE green, not a lucky pass. Separately, because aimock replays scripted tool calls regardless of what the backend actually did, a genuinely broken backend can replay green — OSS-583 found three langchain-js streaming-reassembly bugs (see the langgraph-typescript notes above) that were "D6-green" yet broke against a real stream. D6-green is necessary, not sufficient: for graph/tool/streaming-dependent cells, verify the correct dedicated graph actually runs (not a `sample_agent`/`starterAgent` fallback) and re-run for stability.
