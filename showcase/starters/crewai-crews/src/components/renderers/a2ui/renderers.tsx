...
const demonstrationCatalogRenderers: CatalogRenderers<DemonstrationCatalogDefinitions> = {
  Title: ({ props }) => {
    const Tag = (props.level === "h1" ? "h1" : props.level === "h3" ? "h3" : "h2") as keyof React.JSX.IntrinsicElements;
      ...
      >
        {items.map((item: unknown, i: number) => {
          ...
          if (item && typeof item === "object" && "id" in (item as Record<string, unknown>))
            return (
              <div key={`${(item as Record<string, unknown>).id}-${i}`} style={{ flex: "1 1 0", minWidth: 0 }}>
                {(children as (id: string, basePath?: string) => React.ReactNode)(
    ...
    return (
      ...
      >
        {items.map((item: unknown, i: number) => {
          ...
          if (item && typeof item === "object" && "id" in (item as Record<string, unknown>))
            return (
              <React.Fragment key={`${(item as Record<string, unknown>).id}-${i}`}>
                {(children as (id: string, basePath?: string) => React.ReactNode)(
                  (item as Record<string, unknown>).id,
                  (item as Record<string, unknown>).basePath,
                )}
          ...
          <RechartsPie>
            ...
            >
              {data.map((entry: Record<string, unknown>, i: number) => (
                <Cell key={i} fill={(entry.color as string) ?? COLORS[i % COLORS.length]} />
              ))}
            </Pie>
            <Tooltip />
          </RechartsPie>
          ...
          <tbody>
            {rows.map((row: Record<string, unknown>, i: number) => (
              <tr key={i} style={{ borderBottom: `1px solid ${c.divider}` }}>
                {cols.map((col: Record<string, unknown>) => (
                  <td key={col.key as string} style={{ padding: "8px 12px", color: c.cardFg }}>
  ...
  FlightCard: ({ props: rawProps }) => {
    ...
    const dotColor = (props.statusColor as string) ?? statusColors[props.status as string] ?? "#22c55e";
    ...
    return (
      ...
      >
        ...
        >
          ...
          <span style={{ fontWeight: 700, fontSize: "1.15rem" }}>{props.price as string}</span>
        </div>
...
export const demonstrationCatalog = createCatalog(demonstrationCatalogDefinitions, demonstrationCatalogRenderers, {
