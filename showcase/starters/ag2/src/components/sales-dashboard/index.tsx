  ...
  return (
    <div className="h-full overflow-y-auto bg-[var(--background)]">
      <div className="max-w-5xl mx-auto px-8 py-10 h-full">
        {/* Pipeline Summary */}
        <div className="mb-8">
          <h1 className="text-2xl font-bold text-[var(--foreground)] mb-4">Sales Pipeline</h1>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            <div className="rounded-lg border border-[var(--border)] bg-[var(--card)] p-4">
              <p className="text-xs text-[var(--muted-foreground)] uppercase tracking-wider font-medium">
                Total Pipeline
              </p>
              <p className="text-2xl font-bold text-[var(--foreground)] mt-1">${totalValue.toLocaleString()}</p>
            </div>
            {SALES_STAGES.filter((s) => !s.startsWith("closed")).map((stage) => (
              <div key={stage} className="rounded-lg border border-[var(--border)] bg-[var(--card)] p-4">
                <p className="text-xs text-[var(--muted-foreground)] uppercase tracking-wider font-medium">
        ...
        {/* Todo List */}
        <TodoList todos={todos} onUpdate={onUpdate} isAgentRunning={agent.isRunning} />
      </div>
    </div>
  );
}
