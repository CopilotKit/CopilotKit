...
// `boot()` path has no worker identity and writes rows with `worker_id` unset
// (rendered null), so a query can cleanly separate fleet-replica rows
// (worker_id present) from a legacy single-process row (worker_id null) — same
// null-vs-unavailable discipline the gauge columns already use.
//
    ...
    if (!c.schema.getFieldByName("worker_id")) {
      c.schema.addField(
        new SchemaField({
          name: "worker_id",
          type: "text",
          // Nullable: the legacy single-process boot() path writes rows with no
          // worker identity; only fleet replicas stamp this.
          required: false,
        }),
      );
    }
