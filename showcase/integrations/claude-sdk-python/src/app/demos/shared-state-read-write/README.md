...
- **UI → agent**: a sidebar form (name, tone, language, interests) writes into `state.preferences` via `agent.setState(...)`. The backend reads it every turn and injects it into the system prompt.
...
- On the backend (`src/agents/shared_state_read_write_agent.py`), the dedicated `/shared-state-read-write` endpoint reads `preferences` out of the incoming run state and prepends a system message with the user's name, tone, language, and interests. The `set_notes` tool writes back by emitting a `STATE_SNAPSHOT` event with the updated `notes`.
