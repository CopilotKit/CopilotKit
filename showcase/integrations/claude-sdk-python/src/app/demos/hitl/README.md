...
- `useHumanInTheLoop` registers a `render` function that receives `{ args, respond, status }` — the StepSelector keeps local state for which steps are enabled and calls `respond({ accepted, steps })` on Confirm / Reject.
