...
python -u -m uvicorn agent_server:app --host 0.0.0.0 --port "$AGENT_PORT" --no-access-log &> >(while IFS= read -r line; do printf '[agent] %s\n' "$line"; done) &
AGENT_PID=$!
sleep 2
...
# guard above is satisfied (agent idle-alive on :$AGENT_PORT) → indefinite wedge.
  ...
  while sleep 30; do
    ...
    if curl -fsS --max-time 5 "http://127.0.0.1:${AGENT_PORT}/health" > /dev/null 2>&1; then
      ...
      if [ $FAILS -ge 3 ]; then
        ...
        # NOTE: no Slack alert on the :$AGENT_PORT agent branch. The agent event-loop
