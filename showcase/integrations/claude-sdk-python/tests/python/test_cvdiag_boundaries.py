"""Red→green tests for the claude-sdk-python backend CVDIAG boundary instrumentation.
...
async def _drive_abort() -> None:
    ...
    scope = {
        ...
        "method": "POST",
        "path": "/",
        "headers": [(b"x-aimock-context", b"claude-sdk-python")],
        "query_string": b"",
    }
...
def test_all_eleven_backend_boundaries_emit(monkeypatch, capsys):
    ...
    headers = {"x-test-id": VALID_TEST_ID, "x-aimock-context": "claude-sdk-python"}
    ...
    assert all(e["slug"] == "claude-sdk-python" for e in backend)
...
def test_error_caught_scrubs_secret(monkeypatch, capsys):
    ...
    client.post("/boom", headers={"x-aimock-context": "claude-sdk-python"})
...
def test_disabled_by_default_emits_nothing(monkeypatch, capsys):
    ...
    client = _make_client()
    client.post("/", headers={"x-aimock-context": "claude-sdk-python"})
