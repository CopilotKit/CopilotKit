...
// Headless Interrupt demo (unified nextjs port).
//
// Layout: chat on the right, app surface on the left. The user triggers
// the agent from a chat suggestion. When the agent calls `schedule_meeting`,
// a time-picker popup renders IN THE APP SURFACE (left pane) — outside the
// chat. Picking a slot resolves the tool call, the popup vanishes, and
// the agent confirms back in chat.
//
// For backends without a native interrupt primitive (e.g. AWS Strands),
// `schedule_meeting` is registered as a frontend tool whose async handler
// returns a Promise that only resolves when the user interacts with the
// external popup — equivalent UX via a different mechanism.
...
const DEMO_ID = "interrupt-headless";
...
function Layout() {
  ...
  // Resolver for the currently-awaiting `schedule_meeting` tool call.
  ...
  useFrontendTool({
    ...
    // returns a Promise that only resolves once the user interacts.
    handler: async ({
      topic,
      attendee,
    }: {
