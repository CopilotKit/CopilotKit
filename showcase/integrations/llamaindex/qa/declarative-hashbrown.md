...
- The runtime route registers both the named `declarative-hashbrown-demo`
  agent and a `default` fallback, so a frontend that omits the agent id
  still resolves.
