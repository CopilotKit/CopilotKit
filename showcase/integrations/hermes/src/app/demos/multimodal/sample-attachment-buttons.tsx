 ...
 *
 * Hermes needs no converter shim: the modern `image|document` content
 * parts travel over `@ag-ui/client` and the hermes AG-UI adapter renders
 * image parts as OpenAI-style `image_url` for the vision model
 * (translate._content_to_parts). PDF `document` parts have no text body,
 * so the adapter keeps only the accompanying prompt text — the model
 * still answers about the attachment via the fixture-matched reply.
 */
  ...
  const sendSample = useCallback(
    async (spec: SampleSpec): Promise<void> => {
      ...
      try {
        ...
        // the attachment. Hermes sends these modern `image|document` parts
        // over `@ag-ui/client`; the adapter forwards image parts to the
        // vision model natively (no converter shim needed).
        agent.addMessage({
