...
export const useGenerativeUIExamples = () => {
  ...
  // query_data — hermes divergence. langgraph-python backs this with a backend
  // tool that reads db.csv and returns the rows; the model calls it before
  // rendering a chart ("call query_data first, then render with the chart
  // component"). Hermes has no backend, so we register a client-side
  // `useFrontendTool` fake-data handler (same pattern as the green
  // `tool-rendering` cell) that returns a deterministic sample financial
  // dataset. The model receives the rows as the tool result and feeds them
  // into the pieChart / barChart useComponent calls.
  useFrontendTool({
    name: "query_data",
    description:
    ...
    parameters: z.object({
      query: z.string().describe("Natural-language description of the data."),
    }),
    handler: async () => {
      return [
        { category: "Electronics", revenue: 42000, expenses: 15000 },
        { category: "Clothing", revenue: 28000, expenses: 12000 },
        { category: "Food", revenue: 18000, expenses: 5000 },
        { category: "Books", revenue: 12000, expenses: 3000 },
      ];
    },
  });
};
