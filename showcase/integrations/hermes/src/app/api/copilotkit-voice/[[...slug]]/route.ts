...
//
// Hermes serves every run from a single AG-UI endpoint (POST /), so the chat
// path proxies to the same `HttpAgent` as the main route; only the runtime
// endpoint + the registered agent name (`voice-demo`) differ.
