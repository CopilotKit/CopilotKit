 ...
 *
 * Hermes divergence from langgraph-python: there is NO LegacyConverterShim
 * (hermes sends modern AG-UI content parts over @ag-ui/client) and NO
 * PDF-flatten middleware. Item 4 below therefore can't regress on hermes —
 * the assertion is kept as a cheap invariant. The assistant-text checks
 * are keyed to the hermes D6 fixture replies ("image" / "document").
 *
 ...
 * Both fixtures live in showcase/aimock/d6/hermes/multimodal.json.
 */
...
test.describe("Multimodal Attachments", () => {
  ...
  test("sample PDF: auto-sends, user msg shows EXACTLY ONE document chip (not a broken image)", async ({
    ...
    await expect(asstMsg).toContainText(/document/i);
  });
