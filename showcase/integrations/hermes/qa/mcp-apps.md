...
- Agent backend is healthy; `OPENAI_API_KEY` is set; the Hermes AG-UI adapter is reachable (single endpoint on :8000, registered as agent name `mcp-apps` — see `src/app/api/copilotkit-mcp-apps/route.ts`)
...
- [ ] Click "Draw a flowchart"; within 60s verify the agent calls the `create_view` MCP tool exactly ONCE — confirm via DevTools → Network stream or backend logs
...
- [ ] Verify the iframe paints a flow-chart-shaped diagram: at least 3 shape nodes (rectangles, ellipses, or diamonds with text labels) connected by arrows, framed within the viewport (camera-update step)
- [ ] Verify the assistant text below the iframe is a single short sentence describing what was drawn
...
- Chat loads within 3s; plain-text response within 10s; MCP-backed iframe renders within 60s of prompt (one `create_view` call)
