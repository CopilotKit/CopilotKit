...
- Canonical starter for comparison: `showcase/integrations/hermes/` in the CopilotKit repo

## Test Steps

### 1. Dashboard Cell

- [ ] Open the Hermes provider view on the dashboard; locate the "CLI Start Command" cell
- [ ] Verify the cell displays the exact command `npx copilotkit@latest init --framework hermes`
...
- [ ] In a fresh, empty directory, run `npx copilotkit@latest init --framework hermes`
- [ ] Verify the CLI completes without errors and scaffolds a project tree that matches the canonical starter at `showcase/integrations/hermes/` (same top-level files: `package.json`, `next.config.ts`, `tsconfig.json`, `postcss.config.mjs`, `Dockerfile`, `entrypoint.sh`, `src/`)
...
- [ ] Start the Hermes AG-UI adapter per the scaffolded project's instructions (typically `python -m agui_adapter`); verify it binds without error
- [ ] Open the local URL in a browser; verify the starter demo renders and a basic chat round-trip works
...
- `npm install` completes without error; dev server + Hermes adapter boot; starter demo is interactive
