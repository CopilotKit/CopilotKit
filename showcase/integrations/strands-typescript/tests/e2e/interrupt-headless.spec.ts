...
test.describe("canonical HITL pills", { tag: "@canonical-pill" }, () => {
  ...
  test("interrupt-headless: every required pill and result", async ({
    page,
  }) => {
    test.setTimeout(480_000);
    await page.goto(HITL_ROUTES["interrupt-headless"]);
    await page
      .getByTestId("copilot-suggestion")
      .first()
      .waitFor({ state: "visible" });
    await runHitlCanonical(page, "interrupt-headless");
  });
});
