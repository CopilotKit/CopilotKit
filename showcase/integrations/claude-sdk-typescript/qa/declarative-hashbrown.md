# QA: Declarative UI — Hashbrown — Claude Agent SDK (TypeScript)
...
- The hashbrown envelope prompt lives in
  `src/agent/byoc-hashbrown-prompt.ts`; the `byoc-` prefix on the backend
  module and HTTP mount is deliberate and stays.
- Claude typically replies well inside the 45s budget — a first token later
  than ~15s usually means the agent server is cold, not that the render
  path is broken.
