...
test("frontend-tools-async: every canonical pill dispatches its exact prompt and renders its exact result", async ({
  page,
}) => {
  test.setTimeout(240000);
  await page.goto(STATE_TOOLS_ROUTES["frontend-tools-async"]);
  const result = await runConversation(
    page,
    buildStateToolsTurns("frontend-tools-async"),
...
test.describe("diagnostic: existing integration regressions", () => {
  ...
  test.describe("Frontend Tools (async query_notes)", () => {
    ...
    test("project-planning pill → Notes DB card with project-planning notes", async ({
      ...
      ).toHaveText(/Matching\s+["“]project planning["”]/i, { timeout: 30_000 });
    ...
    test("auth pill → Notes DB card with auth-related notes", async ({
      ...
      await expect(
        notesCard.locator('[data-testid="notes-keyword"]'),
      ).toHaveText(/Matching\s+["“]auth["”]/i, { timeout: 30_000 });
      ...
      await expect(page.getByText("I'm your showcase assistant")).toHaveCount(
        0,
      );
    });
    ...
    test("reading pill → Notes DB card with Book recommendations + locked narration", async ({
      ...
      await expect(
        notesCard.locator('[data-testid="notes-keyword"]'),
      ).toHaveText(/Matching\s+["“]reading["”]/i, { timeout: 30_000 });
      await expect(
        notesCard.getByText("1 match", { exact: false }),
      ).toBeVisible({
        timeout: 30_000,
      });
