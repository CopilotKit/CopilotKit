...
The fix is a showcase-local normalizer
(``agents._multimodal_normalize``) that rewrites the user-message
content list so AG-UI image / document / binary parts become OpenAI
Chat Completions ``image_url`` parts, leaving everything else (text,
already-normalised parts, unknown shapes) untouched.
...
# ---------------------------------------------------------------------------
# Middleware-level smoke: the class is exported and an instance is callable
# (we don't spin up uvicorn here — the unit-level invariants above are
# what guard the regression; this is a tripwire that the public surface
# stayed in place).
# ---------------------------------------------------------------------------


def test_middleware_class_is_constructible():
    """The middleware exposes ``__init__(app)`` and ``__call__`` so
    Starlette's ``app.add_middleware`` and a raw ASGI mount both
    accept it."""

    async def _dummy_app(scope, receive, send):  # pragma: no cover
        pass

    mw = MultimodalContentNormalizerMiddleware(_dummy_app)
    assert mw.app is _dummy_app
    assert callable(mw)
