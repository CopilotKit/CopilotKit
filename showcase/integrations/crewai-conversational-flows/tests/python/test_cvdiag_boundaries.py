"""Red→green tests for the crewai-conversational-flows backend CVDIAG boundary instrumentation.
...
async def _drive_abort() -> None:
    ...
    scope = {
        ...
        "method": "POST",
        "path": "/",
        "headers": [(b"x-aimock-context", b"crewai-conversational-flows")],
        "query_string": b"",
    }
...
def test_all_eleven_backend_boundaries_emit(monkeypatch, capsys):
    ...
    headers = {
        "x-test-id": VALID_TEST_ID,
        "x-aimock-context": "crewai-conversational-flows",
    }
    ...
    assert all(e["slug"] == "crewai-conversational-flows" for e in backend)
...
def test_error_caught_scrubs_secret(monkeypatch, capsys):
    ...
    client.post("/boom", headers={"x-aimock-context": "crewai-conversational-flows"})
...
def test_heartbeat_fires_within_window(monkeypatch, capsys):
    ...
    async def run():
        ctx = _RequestCtx(
            test_id=VALID_TEST_ID, slug="crewai-conversational-flows", demo="default"
        )
...
def test_disabled_by_default_emits_nothing(monkeypatch, capsys):
    ...
    client = _make_client()
    client.post("/", headers={"x-aimock-context": "crewai-conversational-flows"})
