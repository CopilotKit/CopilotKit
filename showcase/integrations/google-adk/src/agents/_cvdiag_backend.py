...
# Framework tag — mirrors the ``_header_forwarding`` breadcrumb component
# (``backend-google-adk``) so the structured envelopes and the breadcrumb log
# lines agree on the integration identity. (L1-D: change this single constant
# when copying to a sibling.) google-adk runs Gemini via google-genai, whose
# httpx client hits ``/v1beta/models/gemini-*`` — that is the llm.call site the
# ``LlmCallScope`` below spans.
_CVDIAG_FRAMEWORK = "google-adk"
