...
export function MessageList({
  ...
  return (
    <div
      ...
      className="flex flex-col gap-3 px-2 py-3 min-h-[400px] max-h-[600px] overflow-y-auto"
    >
      {messages.length === 0 && (
      ...
      {isRunning && (
        <div className="self-start text-xs text-slate-500 px-3 py-1.5 rounded-full bg-slate-100 inline-flex items-center gap-2">
...
function MessageRow({ message }: { message: MessageItem }) {
  if (message.role === "user") {
    return (
      <div className="flex justify-end">
        <div className="self-end max-w-[80%] rounded-2xl rounded-br-sm bg-blue-600 px-4 py-2 text-white shadow-sm">
  ...
  if (message.role === "assistant") {
    return (
      <div className="self-start max-w-[88%] space-y-1.5">
        {message.content ? (
