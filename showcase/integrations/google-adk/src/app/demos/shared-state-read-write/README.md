...
- **UI → agent**: a sidebar form (name, tone, language, interests) writes into `state.preferences` via `agent.setState(...)`. A backend before-model callback reads it every turn and injects it into the system prompt.
...
- On the backend (`src/agents/shared_state_read_write_agent.py`), a `before_model_callback` reads `callback_context.state["preferences"]` and prepends a preferences block to the system instruction with the user's name, tone, language, and interests. The `set_notes` tool writes back via `tool_context.state["notes"] = notes`.
