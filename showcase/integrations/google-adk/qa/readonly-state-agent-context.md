# QA: Read-Only Agent Context — Google ADK

## Prerequisites

- Demo is deployed and accessible
- Agent backend is healthy (check /api/health)
- Agent `readonly_state_agent_context` is mounted on the ADK server (see `src/agents/registry.py`)
...
- [ ] Verify the description text references `useAgentContext`: "Edit fields below and watch the data flow into the agent. The agent can read this context, but cannot modify it."
- [ ] Verify the popup chat renders in the corner with placeholder "Ask about your context..."
