...
- Agent backend is healthy (`/api/health`); `GOOGLE_API_KEY` is set on Railway; `AGENT_URL` points at a Google ADK FastAPI service exposing the `beautiful_chat` agent
