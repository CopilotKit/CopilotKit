...
SYSTEM_PROMPT = dedent(
    ...
    `generate_a2ui` to draw it. `generate_a2ui` takes a `context` string
    summarising the user's request and handles the rendering
    automatically. The available components and their shapes are
    described in the A2UI schema context attached to this conversation,
    so rely on that rather than assumptions about specific component
    names or chart types. Keep chat replies to one short sentence; let
