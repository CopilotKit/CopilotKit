...
//
// The aimock fixtures (showcase/aimock/d6/ms-agent-python/a2ui-recovery.json)
// drive the inner render_a2ui sub-agent two ways: HEAL is structurally invalid
...
test.describe("@diagnostic A2UI Error Recovery", () => {
  ...
  test("@diagnostic heal: an invalid first render is healed into a valid surface", async ({
    page,
  }) => {
    await clickPill(page, HEAL_PILL, HEAL_MSG);

    // The first render attempt is invalid; the validate->retry loop heals it
    // into a valid surface that paints. Allow 90s for the sub-agent round-trips.
...
test("@canonical rendering a2ui-recovery: all actual LGP pills and results", async ({
  ...
  const result = await run({
    cell: {
      id: "react/ms-agent-python/a2ui-recovery",
      frontend: "react",
