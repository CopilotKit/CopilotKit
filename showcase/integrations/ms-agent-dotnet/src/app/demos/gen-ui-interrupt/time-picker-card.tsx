 ...
 *
 * Adaptation note — the LangGraph version of this card is driven by
 * `useInterrupt`'s `resolve(...)` callback, which unblocks the paused
 * backend `interrupt()` call. The .NET showcase has no `interrupt()`
 * primitive, so the card is driven by an async frontend-tool handler
 * instead (see page.tsx). The user-visible behavior is identical: pick a
 * slot or cancel, and the agent confirms back in chat.
 */
