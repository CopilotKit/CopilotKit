...
/**
 * Media-preservation + dedupe shim.
 *
...
function mergeSnapshotPreservingUserMedia(
  currentMessages: ReadonlyArray<Readonly<AgentMessage>>,
  snapshotMessages: ReadonlyArray<Readonly<AgentMessage>>,
): AgentMessage[] | null {
  ...
  let mutated = false;
  const next = snapshotMessages.map((message) => {
    if (message.role !== "user" || !message.id) return message as AgentMessage;
    ...
    mutated = true;
    return { ...(message as object), content: current.content } as AgentMessage;
  });
  return mutated ? next : null;
}
  ...
  const subscriber = useMemo(
    () => ({
      ...
      onMessagesSnapshotEvent: ({
        ...
        event: { messages: ReadonlyArray<Readonly<AgentMessage>> };
        messages: ReadonlyArray<Readonly<AgentMessage>>;
      }) => {
        const merged =
          mergeSnapshotPreservingUserMedia(messages, event.messages) ??
          (event.messages as AgentMessage[]);
        const deduped = dedupeUserMessageMedia(merged) ?? merged;
        return { messages: deduped, stopPropagation: true };
      },
      onRunFinalized: ({
        messages,
      }: {
