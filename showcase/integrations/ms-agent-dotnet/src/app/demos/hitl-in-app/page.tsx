...
/**
 * In-App HITL demo (.NET backend variant).
 *
 * The agent is a support-ops copilot. When the operator asks for an
 * action that affects a customer (refund, plan change, escalation, etc.)
 * the agent calls the frontend-provided `request_user_approval` tool.
 * The tool's async handler opens an approval modal rendered OUTSIDE the
 * chat surface and awaits the operator's decision; the resolved value is
 * returned to the agent as the tool result.
 *
 * Backend: `agent/HitlInAppAgent.cs` mounted at `/hitl-in-app` in
 * `agent/Program.cs`. The main `/api/copilotkit` runtime proxies the
 * `hitl-in-app` agent name at that path.
 */
