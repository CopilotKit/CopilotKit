...
test("shared-state-streaming: every canonical pill dispatches its exact prompt and renders its exact result", async ({
  page,
}) => {
  test.setTimeout(240000);
  await page.goto(STATE_TOOLS_ROUTES["shared-state-streaming"]);
  const result = await runConversation(
    page,
    buildStateToolsTurns("shared-state-streaming"),
...
test.describe("diagnostic: existing integration regressions", () => {
  ...
  test.describe("State Streaming", () => {
    ...
    test("page loads with document panel and chat sidebar", async ({
      page,
    }) => {
