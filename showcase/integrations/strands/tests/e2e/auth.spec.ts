...
// E2E smoke for the auth demo (AWS Strands showcase).
//
...
test("auth loads without errors", async ({ page }) => {
  const errors: string[] = [];
  ...
  expect(errors, `page errors on /demos/auth: ${errors.join(" | ")}`).toEqual(
    [],
  );
});
...
test("canonical suggestion pill fires the feature", async ({ page }) => {
  ...
  await pill.click();
  await expect(page.locator('[data-testid="auth-banner"]').first()).toBeVisible(
    { timeout: 60_000 },
  );
});
