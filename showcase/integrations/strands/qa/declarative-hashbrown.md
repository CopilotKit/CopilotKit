# QA: Declarative UI — Hashbrown — AWS Strands (Python)

## Prerequisites

- Demo reachable at `/demos/declarative-hashbrown`
- `agent_server.py` running and healthy; the shared agent at `/` cannot emit
  the hashbrown envelope, so a dedicated prompt-specialized agent is mounted
  at `/byoc-hashbrown/`
...
- The canonical hashbrown envelope prompt lives in
  `src/agents/byoc_hashbrown.py`; the `byoc_` prefix on the backend module
  and the `/byoc-hashbrown/` HTTP mount are deliberate and stay.
- A legacy `src/app/api/copilotkit-byoc-hashbrown/route.ts` is still present
