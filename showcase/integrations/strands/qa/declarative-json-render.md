# QA: Declarative UI — json-render — AWS Strands (Python)
...
- The canonical flat-spec (`{ root, elements }`) prompt lives in
  `src/agents/byoc_json_render.py`; the `byoc_` prefix on the backend module
  and the `/byoc-json-render/` HTTP mount are deliberate and stay.
