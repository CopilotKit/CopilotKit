<Steps>
  <Step>
    ### Choose where the pause lives

    Strands supports two human-in-the-loop shapes. A frontend tool
    (`useHumanInTheLoop`, or `useFrontendTool` with an async handler) needs
    no agent code: on every run the AG-UI Strands adapter adds a proxy tool
    for each tool the browser registered, the model calls it by name, and
    the user's answer returns as the tool result. A native interrupt pauses
    inside a backend tool and renders through `useInterrupt`. The steps
    below set up the native interrupt.
