...
// the inner harness agent's behavior. Missing / unrecognized values fall
// back to the documented defaults — the agent never throws on malformed
// config, so a misbehaving frontend can't kill the demo.
//
// The inner harness agent is built in SalesAgentFactory.CreateAgentConfigAgent
// (W0 contract §1 AsHarnessAgent + shared-client delta); this DelegatingAIAgent
// wrapper is copied verbatim from the Framework column.
