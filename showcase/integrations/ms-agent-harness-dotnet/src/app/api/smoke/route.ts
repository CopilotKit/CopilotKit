...
const INTEGRATION_SLUG = "ms-agent-harness-dotnet";
...
export async function GET() {
  ...
  // Hit our own Beautiful Chat runtime endpoint — tests the full deployed stack.
  const baseUrl =
    process.env.NEXT_PUBLIC_BASE_URL ||
    `http://localhost:${process.env.PORT || 3000}`;

  try {
    const res = await fetch(`${baseUrl}/api/copilotkit-beautiful-chat`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        method: "agent/run",
        params: { agentId: "beautiful-chat" },
        body: {
