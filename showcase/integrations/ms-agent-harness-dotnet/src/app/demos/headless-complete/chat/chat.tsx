...
/**
 * Browser-friendly UUID. `crypto.randomUUID` only exists in a secure
 * context (HTTPS or localhost); the showcase container serves the demo
 * over plain HTTP at a non-localhost hostname, where `crypto.randomUUID`
 * is undefined and calling it throws — which would abort the send before
 * the message is ever dispatched. Fall back to a math-based UUIDv4 there
 * (mirrors `multimodal/sample-attachment-buttons.tsx`).
 */
