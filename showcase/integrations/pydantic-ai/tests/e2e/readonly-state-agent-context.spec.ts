...
test("readonly-state-context: every canonical pill dispatches its exact prompt and renders its exact result", async ({
  page,
}) => {
  test.setTimeout(240000);
  await page.goto(STATE_TOOLS_ROUTES["readonly-state-context"]);
  const result = await runConversation(
    page,
    buildStateToolsTurns("readonly-state-context"),
...
test.describe("diagnostic: existing integration regressions", () => {
  ...
  test.describe("Readonly Agent Context (useAgentContext)", () => {
    ...
    test("activity checkboxes default-checked: pricing page + product demo video", async ({
      ...
      await expect(
        pricingLabel.locator('input[type="checkbox"]'),
      ).toBeChecked();
