...
// Behavioral e2e for the tool-rendering demo (OpenClaw), run against aimock.
//
// This demo registers a single wildcard renderer via `useDefaultRenderTool`
// that paints ANY tool call as a card (custom-catchall-renderer.tsx):
//   data-testid="custom-catchall-card"        (outer, with data-tool-name / data-status)
//   data-testid="custom-catchall-tool-name"
//   data-testid="custom-catchall-args"
//   data-testid="custom-catchall-status"
//   data-testid="custom-catchall-result"      (only once status === complete)
//
...
// renders — not that a specific tool name/arg shape appears. The exact
// server-tool contract must be confirmed against a real OpenClaw request.
test.describe("Tool Rendering (catch-all renderer)", () => {
  ...
  test("'Read a file' pill renders a catch-all tool-call card", async ({
    page,
  }) => {
