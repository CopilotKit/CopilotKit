...
// Branded catch-all renderer for tool calls (custom-catchall variant).
// Registered via `useDefaultRenderTool` in page.tsx, this component handles
// EVERY tool call the OpenClaw agent makes (shell, read, etc.) — there are no
// per-tool renderers in this cell.
//
// Shows the tool name, a status badge, pretty-printed arguments, and the
// result (as JSON). Each cell is self-contained, so this file inlines its own
// JSON parsing rather than importing across cell boundaries. Uses distinct
// `custom-catchall-tr-` data-testids so it never clashes with the sibling
// `tool-rendering` demo.
...
export function CustomCatchallRenderer({
  ...
  return (
    <div
      data-testid="custom-catchall-tr-card"
      data-tool-name={name}
      data-status={status}
    ...
    >
      ...
      <div className="grid gap-3 p-4 text-sm">
        ...
        <Section label="Result">
          {done ? (
            <pre
              data-testid="custom-catchall-tr-result"
