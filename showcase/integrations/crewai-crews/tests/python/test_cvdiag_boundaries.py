...
async def _drive_abort() -> None:
    ...
    scope = {
        ...
        "method": "POST",
        "path": "/",
        "headers": [(b"x-aimock-context", b"crewai-crews")],
        "query_string": b"",
    }
...
def test_all_eleven_backend_boundaries_emit(monkeypatch, capsys):
    ...
    headers = {"x-test-id": VALID_TEST_ID, "x-aimock-context": "crewai-crews"}
    ...
    assert all(e["slug"] == "crewai-crews" for e in backend)
...
def test_error_caught_scrubs_secret(monkeypatch, capsys):
    ...
    client.post("/boom", headers={"x-aimock-context": "crewai-crews"})
