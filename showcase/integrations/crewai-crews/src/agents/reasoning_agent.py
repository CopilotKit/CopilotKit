"""CrewAI (Crews) reasoning agent — emits AG-UI REASONING_MESSAGE_* events.
...
`/reasoning` server mount in `ag2/src/agent_server.py`, adapted to CrewAI.

Why a custom route instead of the stock CrewAI adapter
------------------------------------------------------
CrewAI's stock `ChatWithCrewFlow` (ag_ui_crewai) drives the LLM through
litellm and streams the model's text as TEXT_MESSAGE_CONTENT only — it
emits NO REASONING_MESSAGE_* events, and it consumes only the answer
content from the chat-completions stream, dropping the
`delta.reasoning_content` side-channel entirely (the channel aimock
fixtures populate via their `reasoning` field, and that reasoning models
emit in production). So the stock adapter can never light up CopilotKit's
reasoning slot.

This module mounts a small custom `/reasoning` sub-app that:
  ...
  3. Falls back to parsing <reasoning>...</reasoning> tags out of the text
     when no native reasoning channel is present (defensive parity).
...
crewai-crews-scoped fixture. Because this handler awaits the OpenAI
AsyncClient directly inside the inbound request task's async context (no
threadpool / run_in_executor hop), the request-scoped ContextVar set by
`HeaderForwardingHTTPMiddleware` is already visible to the outbound call —
no executor-contextvar propagation shim is needed here (unlike AG2, whose
autogen path dispatches the LLM call onto a thread pool).
"""
...
async def _run_reasoning_agent(
    ...
    Buffer the full response, split reasoning from answer, emit
    REASONING_MESSAGE_* then TEXT_MESSAGE_*.
    """
    ...
    try:
        ...
        # matches the crewai-crews-scoped fixture. OPENAI_BASE_URL points the
        ...
        # Accumulate both channels. The stock CrewAI/litellm adapter drops
...
class ReasoningEndpoint(HTTPEndpoint):
    ...
    Mounted at the sub-app root (``reasoning_app.mount("/", ...)``).
    ...
    NB: an inner Mount at ``/`` (NOT ``Route("", ...)``) is required —
    ``Route("", ...)`` crashes Starlette at import with an empty-path error.
    """
...
# HTTPEndpoint at "/" means the HttpAgent posts to ``/reasoning/`` — the outer
# Mount strips ``/reasoning`` and this inner Mount at ``/`` resolves the
# endpoint.
reasoning_app = FastAPI(title="CrewAI (Crews) Reasoning Agent")
