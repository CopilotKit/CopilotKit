...
test.describe("canonical HITL pills", { tag: "@canonical-pill" }, () => {
  ...
  test("hitl-approve-deny: every required pill and result", async ({
    page,
  }) => {
    test.setTimeout(480_000);
    await page.goto(HITL_ROUTES["hitl-approve-deny"]);
    await page
      .getByTestId("copilot-suggestion")
      .first()
      .waitFor({ state: "visible" });
    await runHitlCanonical(page, "hitl-approve-deny");
  });
});
...
test.describe(
  ...
  () => {
    ...
    test.describe("HITL In-App (approval dialog portaled to <body>)", () => {
      ...
      // Downgrade is exercised by the canonical suite above, including both decisions.
