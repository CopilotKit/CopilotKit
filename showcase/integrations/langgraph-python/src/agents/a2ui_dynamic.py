...
# The frontend is the single source of truth for which catalog components
# exist; its schema (component names + props) is attached to the conversation
# as an A2UI schema-context entry. The prompt deliberately names NO specific
# component types or chart shapes — doing so would bias the primary LLM
# toward names that may not exist in the live catalog (e.g. after a frontend
# rename). The secondary LLM inside `generate_a2ui` reads the schema-context
# entry directly and is the only place catalog component names should appear.
SYSTEM_PROMPT = (
    ...
    "`generate_a2ui` to draw it. `generate_a2ui` takes no arguments and "
    "handles the rendering automatically; the available components and "
    "their shapes are described in the A2UI schema context attached to "
    "this conversation, so rely on that rather than assumptions about "
    "specific component names or chart types. Keep chat replies to one "
    "short sentence; let the UI do the talking."
)
