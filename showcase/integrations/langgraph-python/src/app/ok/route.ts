 ...
 * third-party `@langchain/langgraph-api` server, which owns its own routing
 * and natively serves `/ok` (not `/health`, and not `/api/health`). Adding a
 * route to it would mean introducing a custom `http.app` in `langgraph.json`
 * — an architecture change, not a no-op.
 *
 ...
 *
 * Body matches `src/app/api/health/route.ts`. The agent's native `/ok` returns
 * a different payload; the contract that matters to Railway is the 200.
 */
