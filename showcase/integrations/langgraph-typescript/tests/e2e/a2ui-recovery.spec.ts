...
//
// The aimock fixtures (showcase/aimock/d6/langgraph-typescript/a2ui-recovery.json)
...
// envelope. Healing + the loop run live in the backend @ag-ui/langgraph
// getA2UITools factory (injectA2UITool=false); the failure UI text ("Couldn't
...
const HEAL_MSG = "Lay out a sales KPI panel and heal a broken first attempt.";
...
test("@canonical rendering a2ui-recovery: all actual LGP pills and results", async ({
  ...
  const result = await run({
    cell: {
      id: "react/langgraph-typescript/a2ui-recovery",
      frontend: "react",
