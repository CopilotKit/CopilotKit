// CopilotKit runtime for the MCP Apps cell (Google Antigravity).
//
...
export const POST = async (req: NextRequest) => {
  try {
    ...
    const mcpAppsAgent: AbstractAgent = new HttpAgent({
      ...
      // every MCP Apps request to 404 at the agent_server FastAPI layer,
      // surfacing as `HTTP 404: {"detail":"Not Found"}` in the chat.
    ...
    // `/headless_complete` is mounted by the agent_server from the
    // registry entry of the same name (agents.headless_complete.headless_complete_agent).
