...
# route logs (component=route-google-antigravity) and the outbound-llm hook below so
# we can see which hop drops ``x-aimock-context`` on the Antigravity chain.
...
class HeaderForwardingHTTPMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next) -> Response:
        ...
        # cross-language chain shows harness -> route-google-antigravity ->
        # backend-google-antigravity. Mutate the captured copy (not request.headers,
        ...
        if "x-diag-run-id" in headers or "x-aimock-context" in headers:
            ...
            headers["x-diag-hops"] = (
                f"{prev_hops},backend-google-antigravity" if prev_hops else "backend-google-antigravity"
            )
        ...
        _cvdiag(
            component="backend-google-antigravity",
...
def install_httpx_hook(client: Any) -> None:
    ...
    if target is None:
        # CVDIAG: this is the PRIME-SUSPECT silent failure on the Antigravity chain.
        # If the provider SDK does not expose an httpx-style event_hooks target
    ...
    if is_async:
        ...
        async def _inject_headers_async(request):
            headers = get_forwarded_headers()
            # CVDIAG: this fires at the actual outbound provider request. The
            # backend hop is already recorded as `backend-google-antigravity` by the
        ...
        def _inject_headers(request):
            ...
            # backend hop is already `backend-google-antigravity`); the x-* forward
...
def install_global_httpx_hook() -> None:
    ...
    def _patched_sync_init(self, *args, **kwargs):
        ...
        try:
            ...
            # silently dies on the Antigravity chain.
            _cvdiag(
                component="backend-google-antigravity",
                boundary="outbound-llm",
                headers=None,
...
def _install_global_aiohttp_hook() -> None:
    ...
    async def _patched_request(self, method, str_or_url, **kwargs):
        ...
        # CVDIAG: some provider SDKs use aiohttp (NOT httpx) for async streaming
        # calls when aiohttp is importable, bypassing the httpx hook
        ...
        # third hop tag (the backend hop is already `backend-google-antigravity`); the
