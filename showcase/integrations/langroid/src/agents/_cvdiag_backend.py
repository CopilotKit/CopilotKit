...
# Framework tag — mirrors ``_header_forwarding._CVDIAG_FRAMEWORK`` (``langroid``)
# so the structured envelopes and the breadcrumb log lines agree on the
# integration identity. (L1-D: change this single constant when copying to a
# sibling.) langroid drives its ChatAgent over the OpenAI chat-completions API,
# whose outbound httpx call is the llm.call site the ``LlmCallScope`` spans.
