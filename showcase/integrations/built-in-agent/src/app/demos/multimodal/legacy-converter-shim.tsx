 ...
 *
 * 1. **Outgoing modern parts are invisible to older converter paths.** Some
 *    runtime conversion paths only understand the legacy
 *    `{ type: "binary", mimeType, data | url }` shape when forwarding
 *    AG-UI messages to the model adapter. Modern
 ...
 *    legacy path sends our `binary` parts out as image-style model content
 *    and pulls incoming media back in as AG-UI `image` regardless of
 *    mimeType, so PDFs come back as `type: "image"` with
 ...
 *
 * Older converter paths only recognize legacy `binary` parts; modern parts
 * are silently filtered out. We APPEND the legacy mirror alongside the modern part
 ...
 *
 * 1. **Type normalization.** The legacy round-trip can emit incoming media
 *    parts as `type: "image"` regardless of the actual mimeType, including
 *    `application/pdf` and other non-image documents. The chat user-message
 *    renderer dispatches by `type` to either
...
export function LegacyConverterShim() {
  ...
  const subscriber = useMemo(
    () => ({
      ...
      // media parts and normalize types that the legacy round-trip mangled.
      // We hook both `onMessagesSnapshotEvent` and
