...
function Chat() {
  ...
  // `useInterrupt` handles backend pause events. The backend surfaces a
  // structured payload — `{ topic, attendee, slots }` — which we render
  // inline in the chat as a message bubble. Calling `resolve(...)` resumes
  // the paused run with the user's selection.
  useInterrupt({
