...
3. The legacy per-package aimock Playwright suite still requires a
   `/test-aimock` comment. The generated frontend matrix, reusable Angular UI
   browser suite, and shell-readiness gate run automatically while a pull
   request is draft so their browser/server ports never open on a ready PR.
...
| `.github/workflows/test_showcase-frontend-matrix.yml` | test / showcase frontend matrix   | push (main), pull_request (main), workflow_dispatch                                         | n/a; browser/server jobs are gated to draft PRs or non-PR runs                                                                       | No        | All 1,308 generated runnable frontend cells, five reusable Angular UI browser projects, Axe, security, responsive behavior, and shell-readiness p95 |
| `.github/workflows/showcase_angular_preview.yml`      | Showcase: Angular Preview         | pull_request (selected lifecycle events), workflow_dispatch                                 | Angular package/host/release inputs; PR jobs additionally require a same-repository draft with the `angular-preview` label           | No        | Packed Angular 20/21/22 consumers, immutable host image, isolated preview deployment, health, CSP, and production fail-closed routing               |
...
- `showcase/**`: `Showcase: Validate`, `test / showcase frontend matrix`,
  `static / quality`, and `static / check binaries`. The complete matrix's
  port-opening jobs run automatically only while the PR is draft. A
  same-repository draft with the `angular-preview` label also runs the packed
  consumer and isolated preview pipeline when an Angular-owned path changes.
  `test / unit` does NOT fire (paths-ignore excludes `showcase/**`). Comment
  `/test-aimock` to run the separate legacy per-package aimock Playwright suite.
