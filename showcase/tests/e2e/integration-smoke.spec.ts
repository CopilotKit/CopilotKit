...
const INTEGRATIONS: Integration[] = [
    {
        ...
        demos: ["agentic-chat", "hitl", "tool-rendering", "gen-ui-tool-based", "gen-ui-agent", "shared-state-read", "shared-state-write", "shared-state-streaming", "subagents"],
    },
    {
        slug: "mastra",
        name: "Mastra",
...
test.describe("Level 1: Backend Health @health", () => {
    for (const integration of activeIntegrations) {
        test(`[health] ${integration.slug} backend is healthy @health`, async ({ request }) => {
...
test.describe("Level 3: Round-trip Chat @chat", () => {
    for (const integration of activeIntegrations) {
        test(`[chat] ${integration.slug} responds to a message @chat`, async ({ page }) => {
...
test.describe("Level 4: Tool Rendering @tools", () => {
    ...
    for (const integration of toolIntegrations) {
        test(`[tools] ${integration.slug} renders tool results @tools`, async ({ page }) => {
            test.slow();
