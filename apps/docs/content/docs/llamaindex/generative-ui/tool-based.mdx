...
If you don't already have an agent, you can use the [coagent starter](https://github.com/CopilotKit/CopilotKit/tree/main/examples/llamaindex/starter) as a starting point
...
Then, pass the tool to the agent and start the server:
...
pass control back to the frontend when the `get_weather` action is found in the model's response. 
