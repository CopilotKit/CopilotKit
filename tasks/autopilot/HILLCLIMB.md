...
- Required gates with current passing evidence: **1 / 18** (A03 pending committed-source rerun).
- Unresolved critical failures: **none observed yet; trust controls have not been implemented or exercised**.
- Live trials passed / attempted: **2 / 4** (T001 app failure; T002 product flow worked but harness failed; T003 and T004 passed). Manual browser trials are recorded separately in iteration 004.
...
- Next experiment: **verify manual flows on a commit, then add the reusable SDK execution seam**.
...
| A03  | Manual SaaS and persistent SQL                                         | not run | —             | Uncommitted manual trial M003 passed browser create/edit/cancel and user create/edit/deactivate with SQLite assertions, viewer/operator denials, changed-key and stale-version checks; separate app restart passed. Rerun on committed source, then mark passing.                                                             |
