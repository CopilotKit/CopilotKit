...
This composes with [Human in the loop](/langgraph-python/human-in-the-loop): gate on auth first, surface a confirmation card next, execute last.
...
<Callout type="warn" title="Three routes are not user-scoped">
  **`threads/events` and `threads/state`** back the [inspector](/langgraph-python/inspector). Both resolve the
...
See [Scope Rich Threads to the signed-in user](/langgraph-python/threads-lifecycle#scope-rich-threads-to-the-signed-in-user) for the full runtime contract.
...
Write a row when a conversation is first created — see [minting a thread with your own API](/langgraph-python/threads-lifecycle#creating-a-thread-with-your-own-api-on-the-first-message) for where that hooks into the chat lifecycle.
...
<Callout type="info">
  ...
  [Self-managed thread persistence](/langgraph-python/threads-self-managed).
</Callout>
