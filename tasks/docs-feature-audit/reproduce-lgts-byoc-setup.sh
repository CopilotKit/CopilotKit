...
#
# Replaces the 2026-09-13 version of this script, which copied ignored local
# state (.env, .next/, src/agent/.langgraph_api/), ran `npm ci
# --ignore-scripts` and never started a server. This is the boot variant
# first recorded as reproduce-lgts-byoc-setup-20260923.sh (kept unchanged as
# the evidence for that run), plus the old script's strict graph.ts check:
...
#   * type-checks graph.ts with the strict flags the 2026-09-13 script used;
...
# Strict graph.ts check carried over from the 2026-09-13 script.
./node_modules/.bin/tsc --noEmit --strict --module nodenext --moduleResolution nodenext \
  --target es2022 --skipLibCheck graph.ts
