...
| 46 | LangGraph Python on current stable | 38/40 original; smoke route false positive | `feb4591dce` deps; **40/40** first run; `0828d39b45` smoke fix (200-without-model → real run / 502); setup reproduction for both deployment tabs (`e5690a1586`) | Runtime qualified locally (replay) |
| 47 | Google ADK on current stable | Startup originally blocked; smoke route false positive; guide said Python 3.9+ | `470d4f597f` deps; **40/40** first run; `b2323ec714` smoke fix; `5a6670620e` Python 3.10 floor; runner `--demos` batching (`4671e32887`) | Runtime qualified locally (replay) |
| 48 | Planned pause checkpoint | — | [checkpoint-20260923.md](checkpoint-20260923.md): Strands, BIA recheck, full docs suite/build and reader review remain | Paused; no integration fully qualified |
