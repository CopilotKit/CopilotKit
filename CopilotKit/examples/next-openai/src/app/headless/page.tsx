...
function TravelPlanner() {
  const {
    ...
    initialSuggestions: [
      {
        title: "Plan a trip",
        message: "Help me plan a 7-day trip to Japan",
      },
      {
        title: "Plan a trip",
        message: "Help me plan a 7-day trip to Japan",
  ...
  useCopilotAction({
    ...
    render: (props) => {
      return (
        <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-xl p-4 my-2">
          <div className="text-blue-700 font-medium">🔧 Tool 4 Activated</div>
          <div className="text-blue-700 font-medium w-full">
  ...
  useCopilotChatSuggestions({
    instructions:
      "Suggest helpful travel-related topics and questions about trip planning, destinations, and travel tips.",
    minSuggestions: 3,
  });
  ...
  const displayRole = useCallback((role: string) => {
    switch (role) {
      case "tool":
        return "result";
      default:
              ...
              <StickToBottom.Content className="flex flex-col space-y-4">
                {visibleMessages.length === 0 ? (
                  ...
                  visibleMessages.map((message) => (
                    <div
                      key={message.id}
                      className={`group relative ${message.role === "user" ? "ml-8" : "mr-8"}`}
                    >
                      <div
