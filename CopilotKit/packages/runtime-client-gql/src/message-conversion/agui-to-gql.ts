...
function extractAgentName(message: any): string {
  return "agentName" in message && typeof message.agentName === "string" && message.agentName
    ? message.agentName
    : "unknown";
}

// Type guard for agent state message
function isAgentStateMessage(message: any): boolean {
...
export function aguiToGQL(
  ...
  for (const message of messages) {
    ...
    if (isAgentStateMessage(message)) {
      ...
      if ("render" in message && message.render && coAgentStateRenders) {
    ...
    if (message.role === "assistant" && message.toolCalls) {
      ...
      for (const toolCall of message.toolCalls) {
        ...
        if ("render" in message && message.render && actions) {
          const actionName = toolCall.function.name;
          if (actions[actionName]) {
            actions[actionName].render = message.render;
          }
        }
...
export function aguiMessageWithRenderToGQL(
  ...
  if (message.role === "assistant" && "render" in message && message.render && !message.toolCalls) {
    const gqlMessages: gql.Message[] = [];
    gqlMessages.push(
      new gql.AgentStateMessage({
        id: message.id,
