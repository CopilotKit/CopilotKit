...
describe("message-conversion", () => {
  ...
  describe("gqlToAGUI", () => {
    ...
    test("should handle agent state messages", () => {
      const gqlMessages = [
        new gql.AgentStateMessage({ id: "agent-state-1" })
      ];
    ...
    test("should handle action execution messages with actions context and render functions", () => {
      ...
      expect(result[0]).toHaveProperty("renderAndWaitForResponse");
      expect(typeof (result[0] as any).render).toBe("function");
      expect(typeof (result[0] as any).renderAndWaitForResponse).toBe("function");
    });

    test("should provide correct status in render function props", () => {
      const actionExecMsg = new gql.ActionExecutionMessage({
        id: "action-1",
        name: "testAction",
        arguments: { param: "value" },
    ...
    test("should handle render function props override", () => {
      const actionExecMsg = new gql.ActionExecutionMessage({
        id: "action-1",
        name: "testAction",
        arguments: { param: "value" },
    ...
    test("should handle missing render functions gracefully", () => {
      ...
      // Should have undefined render functions
      expect((result[0] as any).render).toBeUndefined();
      expect((result[0] as any).renderAndWaitForResponse).toBeUndefined();
    });
    ...
    test("should handle action not found in actions context", () => {
      ...
      // Should not have render functions when action not found
      expect(result[0]).not.toHaveProperty("render");
      expect(result[0]).not.toHaveProperty("renderAndWaitForResponse");
    });
    ...
    test("should handle agent state messages without coAgentStateRenders", () => {
      ...
      // Should not have render functions without coAgentStateRenders
      expect(result[0]).not.toHaveProperty("render");
    });
    ...
    test("should handle agent state messages with agent not found in coAgentStateRenders", () => {
      ...
      // Should not have render functions when agent not found
      expect(result[0]).not.toHaveProperty("render");
    });

    test("should handle agent state messages with renderAndWaitForResponse", () => {
      const agentStateMsg = new gql.AgentStateMessage({
        id: "agent-state-1",
        agentName: "testAgent",
        state: { status: "running", data: "test data" },
      ...
      const mockRender = vi.fn();
      const mockRenderAndWaitForResponse = vi.fn();
      const coAgentStateRenders = {
        testAgent: {
          name: "testAgent",
          render: mockRender,
      ...
      // Call the renderAndWaitForResponse function
      (result[0] as any).renderAndWaitForResponse?.();
      
      expect(mockRenderAndWaitForResponse).toHaveBeenCalledWith({
        state: { status: "running", data: "test data" }
      });
    });
