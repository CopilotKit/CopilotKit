...
export const reloadSuggestions = async (
  ...
  // Validate inputs
  if (!context) {
    console.error("reloadSuggestions: context is required");
    return;
  }

  if (!context.actions) {
    console.error("reloadSuggestions: context.actions is required");
    return;
  }

  if (!chatSuggestionConfiguration || Object.keys(chatSuggestionConfiguration).length === 0) {
    return;
  }
