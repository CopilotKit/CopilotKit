...
def test_wrap_model_call_strips_raw_x_keys_from_runtime_context():
    ...
    ``config["configurable"]`` AND into ``runtime.context``.  Auth tokens
    such as ``x-copilotkit-auth`` must never be rendered into the LLM prompt
    regardless of which channel they arrive through.  They are consumed only
    in graph node code via ``config["configurable"]["x-copilotkit-auth"]``.
    """
    middleware = CopilotKitMiddleware()
    request = _make_request(state={"messages": [HumanMessage("hi")], "copilotkit": {}})
    ...
    request.runtime.context = {
        "user_role": "admin",
        "x-copilotkit-auth": "super-secret-token",
    ...
    body = _system_message_text(seen)
    # The genuine app context key is still surfaced.
    assert "App Context:" in body
    assert "user_role" in body
    assert "admin" in body
    # Raw x-* credential keys must be stripped before the note is built.
    assert "x-copilotkit-auth" not in body, (
        "x-copilotkit-auth must never appear in model-visible App Context"
    )
    assert "super-secret-token" not in body, (
        "raw auth token must never appear in model-visible App Context"
    )
    assert "x-aimock-context" not in body, (
        "raw x-aimock-context header must never appear in model-visible App Context"
    )


def test_wrap_model_call_empty_after_stripping_all_x_keys():
    """When ``runtime.context`` contains ONLY raw ``x-*`` keys (no legitimate
    app context), stripping them leaves an empty context so no App Context note
    is injected at all.
    """
    middleware = CopilotKitMiddleware()
    request = _make_request(state={"messages": [HumanMessage("hi")], "copilotkit": {}})
    request.runtime.context = {
        "x-copilotkit-auth": "token-value",
        "x-request-id": "req-123",
    }

    seen, _ = _run_wrap(middleware, request)

    # No App Context note should appear — stripping all x-* keys left nothing.
    assert seen.system_message is None, (
...
def test_wrap_model_call_strips_x_keys_from_copilotkit_context_state():
    """Raw ``x-*`` keys in ``state['copilotkit']['context']`` (dict form) must
    also be stripped from the model-visible App Context note.

    This covers the path where the state-based context dict (not runtime.context)
    contains x-* keys — for example when a LangGraph version copies configurable
    headers into state.
    """
    middleware = CopilotKitMiddleware()
    # Use state-based copilotkit.context as a dict (not list form)
    request = _make_request(
        state={
            "messages": [HumanMessage("hi")],
            "copilotkit": {
    ...
    body = seen.system_message.content if seen.system_message else ""
    # Legitimate app context is still rendered.
    assert "user_plan" in body
    assert "pro" in body
    # Credential/infra headers are stripped.
    assert "x-copilotkit-auth" not in body, (
        "x-copilotkit-auth must never reach the LLM prompt via App Context"
    )
    assert "secret-token" not in body
    assert "x-session-id" not in body
