...
type FrontendTool<T extends Record<string, unknown> = Record<string, unknown>> = {
  name: string;
  description?: string;
  parameters?: StandardSchemaWithJSON<T>;
...
A schema implementing the `StandardJSONSchemaV1` interface (e.g. a zod, arktype or valibot schema) that defines and
validates the tool's input parameters. This ensures type safety and provides automatic validation of agent-provided
arguments.
