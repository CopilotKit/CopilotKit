...
Controls whether the tool can be invoked by the agent. When set to `"disabled"`, the tool remains defined in your app
code but is not advertised or executed by the runtime.

```typescript
{
  name: "checkout",
  available: "disabled",
  handler: async () => "This won't execute while disabled"
}
```
