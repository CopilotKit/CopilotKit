...
| Slot | Description |
|------|-------------|
| `messageView` | Container for the message list (user and assistant messages) |
| `scrollView` | Scrollable container with auto-scroll behavior |
| `scrollToBottomButton` | Button that appears when scrolled up |
| `input` | Text input with toolbar and transcription support |
| `inputContainer` | Wrapper around input and feather icon |
| `feather` | Decorative icon next to the input |
| `disclaimer` | Footer text below the input |
| `suggestionView` | Clickable suggestion chips |
| `welcomeScreen` | Initial screen before any messages |
...
CopilotChat uses a powerful [slot system](/reference/slot-system) that allows you to customize any part of the UI. Each slot accepts three types of values:
...
You can customize the appearance of suggestions through the `suggestionView` slot:

```tsx
<CopilotChat
  suggestionView="bg-gray-100 rounded-full px-4 py-2"
/>
```
...
For full setup instructions including backend configuration, see [Voice Transcription](/guides/voice-transcription).
...
<CopilotChat scrollToBottomButton="bg-blue-500 rounded-full shadow-lg" />
```
    ...
    <CopilotKitProvider runtimeUrl="/api/copilotkit">
      <div className="h-screen">
        <CopilotChat
          ...
          input="border-2 border-gray-200 rounded-xl"
...
- [CopilotChatView](/reference/copilot-chat-view) - Lower-level component for full control over messages and state
...
- [Voice Transcription](/guides/voice-transcription) - Voice input setup guide
