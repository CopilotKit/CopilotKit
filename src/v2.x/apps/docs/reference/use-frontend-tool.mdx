...
Controls whether the tool handler can be called by the agent. Use this when you want to keep the component mounted but
temporarily disable execution (for example, feature flags or permission gating).

```tsx
function CheckoutTool({ canCheckout }: { canCheckout: boolean }) {
  useFrontendTool(
    {
      name: "checkout",
      available: canCheckout ? "enabled" : "disabled",
      handler: async () => "checkout-complete",
    },
    [canCheckout],
  );
...
Use the approach that matches your lifecycle needs:

1. `available: "disabled"` when the component should stay mounted but the tool should not execute yet.
2. Conditional mount/unmount when the tool should be removed entirely with UI lifecycle.
3. `agentId` when the tool should only be callable by one agent.
