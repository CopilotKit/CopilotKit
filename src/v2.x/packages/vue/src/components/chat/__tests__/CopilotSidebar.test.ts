...
function mountSidebar(props: Record<string, unknown> = {}, slots: Parameters<typeof h>[2] = {}) {
...
describe("CopilotSidebar", () => {
  ...
  it("forwards wrapper props to the sidebar view", () => {
    ...
    expect(wrapper.get("[data-copilot-sidebar]").attributes("style")).toContain("--sidebar-width: 512px");
  });
  ...
  it("allows overriding the chat-view slot", () => {
    const wrapper = mountSidebar(
      {},
      {
        "chat-view": () => h("div", { "data-testid": "custom-sidebar-chat-view" }, "Custom sidebar"),
      },
    );

    expect(wrapper.find("[data-testid='custom-sidebar-chat-view']").exists()).toBe(true);
