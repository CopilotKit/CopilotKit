...
- **PR tour artifacts for showcase/docs changes** — before preparing a PR that touches showcase demos, showcase probes/fixtures, the dashboard matrix, or docs, read [.claude/docs/pr-tour.md](.claude/docs/pr-tour.md) and follow it. At minimum, include a PR section that makes Loom review easy:
  ...
  - Generate PR-tour videos with `npm --prefix showcase/scripts run pr-tour-videos -- ...` when local services needed for recording are available, attach/link them in the PR description itself with the commit SHA they were recorded from, and refresh them whenever a sufficiently major change lands after recording. Otherwise include the deterministic `--mode plan` output and the exact blocker.
