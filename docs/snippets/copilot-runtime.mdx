...
The runtime supports multiple agent types. The two main ones are:

- **`BuiltInAgent`** — pass a model string, CopilotKit handles everything. Best for quick setup. See [Quickstart](/built-in-agent/quickstart).
- **`Agent`** — bring your own AI SDK, TanStack AI, or custom LLM backend. Best when you need full control. See [Custom Agent](/built-in-agent/custom-agent).
