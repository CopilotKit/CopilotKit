<Callout type="info">
  **NOTE 1**: CopilotKit supports any LLM provider.<br/>
  The instructions below assume you are using OpenAI, but you can adopt them to [any LLM provider](/guides/bring-your-own-llm).
</Callout>

<Callout type="info">
  **NOTE 2**: If you are planning to use a single LangGraph agent in [agent-lock mode](/coagents/advanced/router-mode-agent-lock#agent-lock-mode) as your agentic backend, your LLM adapter will only be used for peripherals such as suggestions, etc.
