...
<Steps>
  ...
  <Step>
    ...
    Alternatively, if you want to configure MCP servers for all agents from one place, you can pass `mcpApps` directly to `CopilotRuntime` instead of using per-agent `.use()`:
