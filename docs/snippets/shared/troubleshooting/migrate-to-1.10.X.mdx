...
<Callout type="warning">
  This section describes the historical `useCopilotChatHeadless_c` hook. That
  hook is now deprecated. For new fully custom chat UIs, use
  [`useAgent`](/reference/v2/hooks/useAgent) with
  `useCopilotKit().copilotkit.runAgent`. See the
...
At the time of the 1.10 release, `useCopilotChatHeadless_c` was introduced as the advanced headless UI path. That recommendation is now superseded by the v2 `useAgent` pattern.
...
**Use the v2 `useAgent` pattern if:**
