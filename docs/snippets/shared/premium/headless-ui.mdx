...
<Callout type="warning">
  **Deprecated.** `useCopilotChatHeadless_c` is still supported during the
  ...
  [`useAgent`](/reference/v2/hooks/useAgent) with
...
CopilotKit previously offered *fully headless UI* through the `useCopilotChatHeadless_c` hook. By using this hook, you can build your own chat interfaces
...
export const Chat = () => {
  ...
  useRenderTool({
    ...
    render: ({ parameters, result, status }) => (
      <WeatherCard location={parameters.location} result={result} status={status} />
    ),
  });
