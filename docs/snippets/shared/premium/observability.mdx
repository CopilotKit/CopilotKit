...
CopilotKit provides enterprise-grade observability capabilities to help you monitor your application and understand user behavior. Get insights into chat interactions, user events, and system errors with powerful hooks.
...
- ✅ Requires a `publicLicenseKey` or `publicApiKey` from [Copilot Cloud](https://cloud.copilotkit.ai)
...
2. **Get your public license key, or public API key** from the dashboard
...
- **Always provide `publicLicenseKey` or `publicApiKey` ** for observability hooks to work
...
- **Don't expect hooks to work without a valid key** - they're designed to be secure
...
1. **Check your publicLicenseKey or publicApiKey** - all observability hooks require a valid key
...
1. **Check your publicLicenseKey or publicApiKey ** - error observability requires a valid key
2. **Verify the key format** - should start with `ck_pub_`
3. **Ensure the key is set** - check your environment variables
...
- Missing or invalid `publicLicenseKey` or `publicApiKey`
- Observability hooks defined but not on the right components
- Network issues preventing validation

**Error observability not working:**

- Missing `publicLicenseKey` or `publicApiKey` in CopilotKit provider
