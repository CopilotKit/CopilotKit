...
The factory receives an `AgentFactoryContext` (from `@copilotkit/runtime`):
...
<Tab value="TanStack AI">
```typescript title="src/copilotkit.ts"
import { BuiltInAgent, convertInputToTanStackAI } from "@copilotkit/runtime";
...
<Tab value="AI SDK">
```typescript title="src/copilotkit.ts"
import { BuiltInAgent, convertMessagesToVercelAISDKMessages } from "@copilotkit/runtime";
import { streamText } from "ai";
...
These utilities are exported from `@copilotkit/runtime` to help convert between CopilotKit's input format and your backend's expected format:
