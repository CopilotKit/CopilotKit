...
# Backend Actions & "Copilot Skills"
...
For a refresher, see [Self-Hosting](/concepts/self-hosting) (or alternatively, revisit the [quickstart](/quickstart)).
...
<Tabs items={['TypeScript / Node.js', 'LangChain JS', 'LangServe', 'Remote Actions + LangGraph (Python)']}>
  ...
  **Note that `actions` is not merely an array of actions, but rather a _generator_ of actions.**
  ...
  as well as custom properties you can pass from the frontend.
    ...
    ```tsx filename="/api/copilotkit/route.ts" showLineNumbers
  ...
  <Tabs.Tab>
    ...
    We provide a [Python backend SDK]() which includes support for [LangGraph-powered CoAgents!]().
...
For more advanced use cases, refer to the [Backend Actions documentation](/concepts/agents).
