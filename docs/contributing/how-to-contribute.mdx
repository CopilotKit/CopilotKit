...
Thanks for showing interest in contributing to CopilotKit!
...
export OPENAI_PAI_KEY=<your-openai-api-key>
pnpm run start:dev
```

<Note>
We use the `pnpm run start:dev` command to run examples, which is different from the `turbo run dev` command we use to work on the individual packages.
</Note>
...
## (Advanced): Package Linking
 ...
 Check out the [Advanced: Package Linking](/contributing/advanced-package-linking) guide to learn how to do that.
