...
<PropertyReference name="action" type="Action" required>
  The function made available to the Copilot. See [Action](#action).
    ...
    <PropertyReference name="renderAndWaitForResponse" type="(props: ActionRenderProps<T>) => void">
      This is the same as `render`, but instead of returning a string or a component, it will wait for a response from the user.
    </PropertyReference>
