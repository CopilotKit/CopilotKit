...
This hook enables you to dynamically generate UI elements and render them in the copilot chat. For more information, check out the [Generative UI](/generative-ui) page.
...
<PropertyReference name="description" type="string">
  A description of the tool. This is used to instruct the Copilot on how to use
  the tool.
</PropertyReference>
...
<PropertyReference name="followUp" type="boolean">
  Whether to report the result of the tool call to the LLM which will then
  provide a follow-up response. Pass `false` to disable.
</PropertyReference>
...
<PropertyReference name="available" type="'disabled' | 'enabled'">
  Whether the tool is available. Set to "disabled" to prevent the tool from
  being called.
</PropertyReference>
