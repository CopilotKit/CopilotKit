...
function useAgentContext(contexts: AgentContextInput[]): void;
```

## Parameters

<PropertyReference name="context" type="AgentContextInput | AgentContextInput[]" required>
  An object, or an array of objects, describing the context to expose to the agent.
...
### Multiple Contexts from One Component

Pass an array when one component owns several independent pieces of context.
...
- **Multiple contexts**: Multiple `useAgentContext` calls across your component tree, or multiple items passed in one array, are all visible to the agent concurrently. Each is identified by its description and value.
