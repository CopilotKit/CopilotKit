...
| **Agent instance** | `new HttpAgent({ notificationThrottle })` | Per-agent (all subscribers) | `onMessagesChanged` / `onStateChanged` subscriber notifications at the source |
...
- **`notificationThrottle` on the agent** — best when you control agent instantiation and want to throttle for ALL consumers (not just one React component). Supports both time-based (`intervalMs`) and character-count-based (`minChunkSize`) thresholds.
- **Both together** — the agent-level throttle reduces notification frequency at the source, and the hook-level throttle further smooths the React rendering. They compose naturally.

### Agent-level notification throttle

When creating an agent directly (e.g., `HttpAgent`), you can configure notification throttling:
...
Mutations are always applied immediately (`agent.messages` and `agent.state` stay up to date) — only subscriber notifications are coalesced.
