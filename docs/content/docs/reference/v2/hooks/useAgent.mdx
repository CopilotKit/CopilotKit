...
<PropertyReference name="agentId" type="string" default='"default"'>
  ...
  <PropertyReference name="updates" type="UseAgentUpdate[]" default="[OnMessagesChanged, OnStateChanged, OnRunStatusChanged, OnStreamingPhaseChanged]">
    ...
    - `UseAgentUpdate.OnStreamingPhaseChanged` - Re-render when streaming phase changes (reasoning, tool calling, streaming)
...
<PropertyReference name="object" type="{ agent: AbstractAgent; streamingStatus: StreamingStatus }">
  Object containing the agent instance and streaming status.
  ...
  <PropertyReference name="streamingStatus" type="StreamingStatus">
    Real-time streaming phase of the current agent run. Updated via `UseAgentUpdate.OnStreamingPhaseChanged` (on by default).

    <PropertyReference name="phase" type="StreamingPhase">
      Current phase: `"idle"`, `"reasoning"`, `"tool_calling"`, or `"streaming"`.
    </PropertyReference>
    ...
    <PropertyReference name="toolName" type="string | null">
      Name of the tool currently being called, or `null`. Only set when `phase` is `"tool_calling"` — automatically cleared on phase transitions.
    </PropertyReference>

    <PropertyReference name="toolCallId" type="string | null">
      ID of the active tool call, or `null`. Only set when `phase` is `"tool_calling"` — automatically cleared on phase transitions.
    </PropertyReference>
...
function StatusBadge() {
  ...
  if (!streamingStatus.isRunning) return null;

  return (
    <span>
      {streamingStatus.phase === "reasoning" && "Thinking..."}
      {streamingStatus.phase === "tool_calling" &&
        `Calling ${streamingStatus.toolName}...`}
      {streamingStatus.phase === "streaming" && "Writing response..."}
      {streamingStatus.phase === "idle" && "Starting..."}
    </span>
  );
}
```
...
// Only re-render when messages change (no streaming phase updates)
