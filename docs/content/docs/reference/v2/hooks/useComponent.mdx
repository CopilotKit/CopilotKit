...
- `config.parameters`: optional Zod schema for typed/validated args. When provided, `render` props are inferred from the schema.
- `config.render`: React component rendered with parsed tool args. Accepts any props when `parameters` is omitted.
