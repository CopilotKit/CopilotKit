...
- **Tool Call Rendering**: Tool calls embedded in the assistant message are rendered by the `toolCallsView` slot. Each tool call is resolved using the `useToolRenderingResolver` hook, which looks up registered renderers by tool name.
...
- [`useToolRenderingResolver`](/reference/v2/hooks/useToolRenderingResolver) -- Hook used internally for resolving tool call renderers
