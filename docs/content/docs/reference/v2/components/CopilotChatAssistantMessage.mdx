...
<PropertyReference name="markdownRenderer" type="SlotProp<typeof CopilotChatAssistantMessage.MarkdownRenderer>">
  Slot for the Markdown rendering engine. Defaults to the built-in streaming renderer (`StreamingMarkdownDefaultRenderer` from [`@copilotkit/markdown-renderer`](/reference/v2/packages/markdown-renderer)) — streaming-safe incremental rendering with per-token animation, GFM, tables, autolinks, and citations, with **no** syntax highlighting or math. A renderer is any component that accepts `{ content: string; isStreaming?: boolean }`. `BasicMarkdownRenderer` (marked-based) is available as a lighter alternative. This per-message slot takes precedence over a `markdownRenderer` set on `CopilotKitProvider`.
