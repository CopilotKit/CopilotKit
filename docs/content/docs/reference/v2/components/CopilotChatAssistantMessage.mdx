...
<PropertyReference name="markdownRenderer" type="SlotProp<typeof CopilotChatAssistantMessage.MarkdownRenderer>">
  Slot for the Markdown rendering engine. Defaults to the built-in streaming renderer (`StreamingMarkdownDefaultRenderer` from `@copilotkit/react-core/v2`, a thin adapter over [`@copilotkit/markdown-renderer`](/reference/v2/packages/markdown-renderer)'s `StreamingMarkdownRenderer`) — streaming-safe incremental rendering with per-token animation, GFM, tables, autolinks, and citations, with **no** syntax highlighting or math. A renderer is any component that accepts `MarkdownRendererProps` (`{ content: string; isStreaming?: boolean; className?: string }`, exported from `@copilotkit/react-core/v2`). `BasicMarkdownRenderer` (marked-based) is available as a lighter alternative. This per-message slot takes precedence over a `markdownRenderer` set on `CopilotKitProvider`. `markdownRenderer` also accepts a config object (`DefaultMarkdownRendererProps`) to configure the built-in renderer without writing a wrapper — e.g. `markdownRenderer={{ nodeRenderers: { codeBlock: Shiki } }}`.
  ...
  **As a replacement component** (here, restoring Streamdown — install `streamdown` first):
  ...
  To apply a renderer to every message at once, set it on the provider instead (define it outside render so its identity is stable):
  ```tsx
  ...
  For a full walkthrough of all three approaches (keep the default, configure it, or replace it) across React, Vue, and React Native, see the [Markdown Rendering](/markdown-rendering) guide.
</PropertyReference>
