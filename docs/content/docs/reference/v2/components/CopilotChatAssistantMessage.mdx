...
<PropertyReference name="markdownRenderer" type="SlotProp<typeof CopilotChatAssistantMessage.MarkdownRenderer>">
  Slot for the Markdown rendering engine. Defaults to the built-in basic-GFM renderer (`BasicMarkdownRenderer`) — GFM tables, lists, links, and plain fenced code blocks, with **no** syntax highlighting or math. A renderer is any component that accepts `{ content: string; isStreaming?: boolean }`. This per-message slot takes precedence over a `markdownRenderer` set on `CopilotKitProvider`.
...
- **Markdown Support**: The built-in renderer supports basic GitHub Flavored Markdown — headings, bold/italic/strikethrough, lists, links, inline and fenced code, tables, blockquotes, and images — with no external markdown dependencies. It escapes raw HTML and sanitizes link/image URLs. Syntax highlighting, math (KaTeX), and diagrams are **not** included by default; plug in a custom `markdownRenderer` to add them (see [Migrate the Markdown Renderer](/migration-guides/migrate-markdown-renderer)).
