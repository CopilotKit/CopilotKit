...
<Callout type="warning">
  `useRenderToolCall` is still supported in v1, but we recommend migrating renderer registrations to [`useRenderTool`](/reference/v2/hooks/useRenderTool) from the v2 API.
</Callout>
