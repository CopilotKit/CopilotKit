...
<Callout type="info">
  ...
  [`useRenderToolCall`](/reference/hooks/useRenderToolCall) for render-only
  tools - [`useFrontendTool`](/reference/hooks/useFrontendTool) for tools with
  handler + render - [`useHumanInTheLoop`](/reference/hooks/useHumanInTheLoop)
  for human-in-the-loop tools
</Callout>
