...
<Steps>
  ...
  <Step>
    ...
    use the `setAgentState` function to update the agent state.
    ...
    ```
    <Callout type="info">
      The `setAgentState` function in `useAgent` will update the working memory and trigger a rerender when the state changes.
    </Callout>
  </Step>
  <Step>
    ### Give it a try!
