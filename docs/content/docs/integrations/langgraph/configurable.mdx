...
LangChain agents are able to take execution parameters, such as auth tokens and similar properties.
...
<Callout type="info" title="Looking for authentication?">
  If you need to pass authentication tokens specifically, see the dedicated
  [Authentication guide](/langgraph/auth) which covers LangGraph Platform and
  self-hosted authentication patterns in detail.
</Callout>

## Implementation

By default, LangChain agents are invoked with a `config` argument. This config has a `configurable` property which can be accessed and filled with your data.
