...
<Step>
    ### Call `interrupt` in your LangChain agent
    Now we can call `interrupt` in our LangChain agent.
...
<Step>
    ...
    At this point, your LangChain agent's `interrupt` will be called. However, we currently have no handling for rendering or
