...
Now that we have integrated the LangChain agent into the application, we can start utilizing features that will enhance the
...
`search_node` of our LangChain agent.
...
With CoAgents, the LangChain agent's state is only emitted when node change occurs (i.e, an edge is traversed). This means
