...
<Callout type="info">
  This example demonstrates reading from shared state in the [CopilotKit Feature Viewer](https://feature-viewer.copilotkit.ai/claude-agent-sdk-typescript/feature/shared_state).
</Callout>
...
<Steps>
  ...
  <Step>
    ...
    Create your Claude Agent SDK adapter. The adapter automatically provides the `ag_ui_update_state` tool, which the agent uses to update shared state:
...
When you need to change the language, use the ag_ui_update_state tool to update the language field.`,
    ...
    The adapter injects the current state into the agent's context automatically, and the `ag_ui_update_state` tool is always available for the agent to call.
