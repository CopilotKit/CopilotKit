...
The Claude Agent SDK adapter supports bidirectional state synchronization via the AG-UI protocol. CopilotKit allows you to render this state in your application with custom UI components, which we call **Agentic Generative UI**.
...
<Steps>
  ...
  <Step>
    ...
    Create your Claude Agent SDK adapter with a system prompt that instructs the agent to use the `ag_ui_update_state` tool for state management. The adapter automatically provides this tool.
    ...
    const adapter = new ClaudeAgentAdapter({
      ...
      systemPrompt: `You are a helpful assistant that can change language settings.
When you need to update the shared state, use the ag_ui_update_state tool.
The current state will be provided in the context.`,
    });
    ```
