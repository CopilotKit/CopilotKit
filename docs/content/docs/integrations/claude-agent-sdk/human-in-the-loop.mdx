...
icon: "lucide/User"
description: Create frontend tools and use them within your Claude Agent SDK agent for human-in-the-loop interactions.
---
...
<IframeSwitcher
  ...
  exampleUrl="https://feature-viewer.copilotkit.ai/claude-agent-sdk-typescript/feature/human_in_the_loop?sidebar=false&chatDefaultOpen=false"
  codeUrl="https://feature-viewer.copilotkit.ai/claude-agent-sdk-typescript/feature/human_in_the_loop?view=code&sidebar=false&codeLayout=tabs"
  exampleLabel="Demo"
  codeLabel="Code"
  height="700px"
/>
...
Frontend tools enable you to define client-side functions that your Claude Agent SDK agent can invoke, with execution happening entirely in the user's browser. When your agent calls a frontend tool,
the logic runs on the client side, giving you direct access to the frontend environment.

This can be utilized to let [your agent control the UI](/claude-agent-sdk/frontend-tools), [generative UI](/claude-agent-sdk/frontend-tools), or for Human-in-the-loop interactions.
