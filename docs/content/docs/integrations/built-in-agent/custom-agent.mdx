...
The `Agent` class gives you full control over the LLM call. You provide a factory function that talks to any backend — CopilotKit handles converting the stream to AG-UI events, managing lifecycle, and wiring it into the runtime.
...
| **AI SDK support** | Vercel AI SDK v6 only | Any backend: AI SDK, TanStack AI, or custom |
| **Best for** | Quick setup, standard use cases | Full control, non-standard backends, async initialization |

<Callout type="info">
If `BuiltInAgent` covers your needs, stick with it — it's simpler. Use `Agent` when you need control that `BuiltInAgent` doesn't offer.
</Callout>
...
The frontend setup is the same as [BuiltInAgent](/built-in-agent/quickstart) — wrap your app with `<CopilotKit>` and add a chat component.

## How It Works

The `Agent` class accepts a config with two fields:
...
The factory receives an `AgentFactoryContext`:

```typescript
interface AgentFactoryContext {
  input: RunAgentInput;        // messages, tools, state, context, threadId, runId, forwardedProps
  abortController: AbortController;  // for TanStack AI (uses AbortController)
  abortSignal: AbortSignal;          // for AI SDK / fetch (uses AbortSignal)
}
```
...
<Tab value="TanStack AI">
```typescript title="src/copilotkit.ts"
import { Agent, convertInputToTanStackAI } from "@copilotkit/runtime/v2";
...
const agent = new Agent({
  ...
  factory: ({ input, abortSignal }) => {
    ...
    // Add shared application state (useCopilotChatSuggestions, etc.)
