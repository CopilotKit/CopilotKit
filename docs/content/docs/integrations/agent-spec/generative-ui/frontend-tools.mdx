...
<Steps>
    ...
    <Step>
        ### Give the tool to the Agent Spec Agent

        In Agent Spec, `ClientTool` declares a tool that executes on the "client" side, in our case it's the CopilotKit runtime in the frontend.
        It corresponds to the concept of frontend tools in CopilotKit.
        Note that, when creating the Agent Spec `ClientTool`, its name, description, and input schemas need to match those of the CopilotKit frontend tool.
