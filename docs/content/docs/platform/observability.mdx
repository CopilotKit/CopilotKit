...
<Callout type="info">
  **All observability hooks require a `publicApiKey`** - Get yours free at
...
Track user interactions and chat behavior with comprehensive observability hooks (requires a `publicApiKey`):
...
- ✅ Requires a `publicApiKey` from [CopilotKit Platform](https://cloud.copilotkit.ai)
...
- ✅ Requires `publicApiKey` from [CopilotKit Platform](https://cloud.copilotkit.ai)
- ✅ Only triggers when `publicApiKey` is provided
...
## Getting Started with CopilotKit Platform

To use observability hooks (event hooks and error observability), you'll need a CopilotKit Platform account:
...
<Callout type="info">
  CopilotKit Platform is free to get started and provides production-ready
