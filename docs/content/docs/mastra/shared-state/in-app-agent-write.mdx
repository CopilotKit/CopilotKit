...
<Steps>
  ...
  <Step>
    ...
    Mastra Agents are stateful. As you transition through the flow, that state is updated and available to the next function. For this example,
...
By default, the Mastra Agent state will only update _between_ Mastra Agent node transitions --
...
See **[emit intermediate state](/mastra/shared-state/predictive-state-updates).** 