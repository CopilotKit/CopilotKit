...
A Mastra Agent's state updates discontinuosly; only across function transitions in the flow.
...
When a function in your Mastra agent finishes executing, **its returned state becomes the single source of truth**.
