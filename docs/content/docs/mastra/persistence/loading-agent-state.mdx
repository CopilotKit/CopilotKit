...
(See [Message Persistence](/mastra/persistence/message-persistence) for more details.)
...
<Callout>
  **Important:** For agent state to be loaded correctly, you must first ensure
  ...
  guides on [Threads &
  Persistence](/mastra/persistence/loading-message-history) and [Message
  Persistence](/mastra/persistence/message-persistence).
</Callout>
...
- [Reading agent state](/mastra/shared-state/in-app-agent-read)
- [Writing agent state](/mastra/shared-state/in-app-agent-write)
