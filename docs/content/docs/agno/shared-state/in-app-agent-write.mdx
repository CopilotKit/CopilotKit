...
<Callout>
  ...
  starter](https://github.com/CopilotKit/CopilotKit/tree/main/examples/coagents-starter-agno)
...
<Steps>
  ...
  <Step>
    ...
    Agno Agents are stateful. As you transition through the flow, that state is updated and available to the next function. For this example,
...
By default, the Agno Agent state will only update _between_ Agno Agent node transitions --
...
See **[emit intermediate state](/agno/shared-state/predictive-state-updates).** 