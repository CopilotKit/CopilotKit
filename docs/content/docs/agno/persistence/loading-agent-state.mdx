...
(See [Message Persistence](/agno/persistence/message-persistence) for more details.)
...
<Callout>
  **Important:** For agent state to be loaded correctly, you must first ensure
  ...
  guides on [Threads &
  Persistence](/agno/persistence/loading-message-history) and [Message
  Persistence](/agno/persistence/message-persistence).
</Callout>
...
- [Reading agent state](/agno/shared-state/in-app-agent-read)
- [Writing agent state](/agno/shared-state/in-app-agent-write)
