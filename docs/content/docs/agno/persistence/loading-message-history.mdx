---
title: Threads
description: Learn how to maintain persistent conversations across sessions with Agno Agents.
icon: "lucide/MessagesSquare"
---

# Understanding Thread Persistence

Agno Agents supports threads, a way to group messages together and maintain a continuous chat history across sessions. CopilotKit provides mechanisms to ensure conversation state is properly persisted between the frontend and backend.
...
<Callout>
  **Note:** While the frontend uses `threadId` to manage conversation sessions,
  ...
  [Message Persistence](/agno/persistence/message-persistence))
  to save and load the state associated with each `threadId`.

See the [sample agent implementation](https://github.com/CopilotKit/CopilotKit/blob/main/examples/coagents-starter-agno/agent-py/sample_agent/agent.py#L291)
