...
Here are the key terms and concepts used throughout agents:
...
| Agent | Terminology referring to CopilotKit's suite of tools for building agentic applications with Microsoft Agent Framework. Typically interchangeable with agentic copilot. |
| Agent State | The current data and context maintained by a Microsoft Agent Framework agent during its execution, including both internal state and data that can be synchronized with the frontend UI. |
...
| Ground Truth | In agents, CopilotKit serves as the "ground truth" for the full chat session, maintaining the persistent chat history and ensuring conversational continuity across different agents. |
...
| [Microsoft Agent Framework](https://learn.microsoft.com/en-us/agent-framework/) | The agent framework integrated with CopilotKit that provides the orchestration layer for agents via the AG-UI protocol. |
...
These terms are referenced throughout the documentation and are essential for understanding how agents work and how to implement them effectively in your applications.
