...
<Tabs groupId="language" items={['Python', 'TypeScript']} default="Python">
    <Tab value="Python">
        ```python
        from copilotkit.agent framework import copilotkit_customize_config
    ...
    <Callout type="warn" title="BEWARE!">
        In agent framework Python, the `config` variable in the surrounding namespace is **implicitly** passed into agent LLM calls, even when not explicitly provided.
        ...
        # it will affect every subsequent agent LLM call in the same namespace, even when `config` is not explicitly provided
    ...
    <Tab value="TypeScript">
        ```typescript
        import { copilotkitCustomizeConfig } from '@copilotkit/sdk-js/agent framework';
