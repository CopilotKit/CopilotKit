...
<Frame>
    <img src="https://cdn.copilotkit.ai/docs/copilotkit/images/microsoft-agent-framework/multi-agent-flows.png" alt="Multi-Agent Flows" />
</Frame>
...
### Router Mode (default)
Router Mode is enabled by default when using agents. To use it, specify a runtime URL prop in the `CopilotKit` provider component and omit the `agent` prop, like so:
```tsx
...
You can find more information about it in the ["Exiting the agent loop" section](https://docs.copilotkit.ai/microsoft-agent-framework/advanced/exit-agent).
...
In this mode, CopilotKit is configured to work exclusively with a specific agent. This mode is useful when you want to focus on a particular task or domain. Whereas in Router Mode the LLM and CopilotKit's router are free to switch between agents to handle user requests, in Agent Lock Mode all requests are directed to the same agent, ensuring focused interaction.
