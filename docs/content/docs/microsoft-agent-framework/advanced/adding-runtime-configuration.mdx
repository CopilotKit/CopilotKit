...
## What is this?
agent framework agents are able to take execution parameters, such as auth tokens and similar properties.
You can add these using this feature.

If you wish to read further, you can refer to [the configuration guide by agent framework](https://agent-ai.github.io/agent framework/how-tos/configuration/)
...
By default, agent framework agents are invoked with a `config` argument. This config has a `configurable` property which can be accessed and filled with your data.
...
<Step>
    ...
    You can read more about this (here)[https://agent-ai.github.io/agent framework/how-tos/configuration/#define-graph]
    <Tabs groupId="language" items={['Python', 'TypeScript']} default="Python">
        <Tab value="Python">
            ```python
