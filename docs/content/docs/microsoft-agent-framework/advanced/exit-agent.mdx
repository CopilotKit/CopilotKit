...
In this example from [our email-sending app](https://github.com/copilotkit/copilotkit/tree/main/examples/agents-qa), the `send_email` node explicitly exits, then manually sends a response back to the user as a `ToolMessage`:
...
<Steps>
    ...
    <Step>
        ### Exit the agent loop
        This will exit the agent session as soon as the current agent framework run is finished, either by a breakpoint or by reaching the `END` node.

        <Tabs groupId="language" items={['Python', 'TypeScript']} default="Python">
            <Tab value="Python">
                ```python
                from copilotkit.agent framework import (copilotkit_exit)
                # ...
            ...
            <Tab value="TypeScript">
                ```typescript
                import { copilotkitExit } from "@copilotkit/sdk-js/agent framework";
