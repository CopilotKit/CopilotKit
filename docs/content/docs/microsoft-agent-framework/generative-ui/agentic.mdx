...
<IframeSwitcher
  ...
  exampleUrl="https://feature-viewer.copilotkit.ai/agent framework/feature/agentic_generative_ui?sidebar=false&chatDefaultOpen=false"
  codeUrl="https://feature-viewer.copilotkit.ai/agent framework/feature/agentic_generative_ui?view=code&sidebar=false&codeLayout=tabs"
  exampleLabel="Demo"
  codeLabel="Code"
  height="700px"
/>
...
All agent framework agents are stateful. This means that as your agent progresses through nodes, a state object is passed between them perserving
...
<Steps>
  ...
  <Step>
    ...
    agent framework agents are [stateful](https://agent-ai.github.io/agent framework/concepts/low_level/). As you progress through nodes, a state object is passed between them. CopilotKit
  ...
  <Step>
    ...
    <Tabs groupId="language" items={['Python', 'TypeScript']} default="Python">
      <Tab value="Python">
        ...
        from copilotkit.agent framework import copilotkit_emit_state # [!code highlight]
      ...
      <Tab value="TypeScript">
        ...
        import { copilotkitEmitState, CopilotKitStateAnnotation } from "@copilotkit/sdk-js/agent framework";
  ...
  <Step>
    ...
    Now we can utilize `useagentStateRender` to render the state of our agent **in the chat**.

    ```tsx title="app/page.tsx"
    import { useagentStateRender } from "@copilotkit/react-core";

    // For type safety, redefine the state of the agent. If you're using
    // using agent framework JS you can import the type here and share it.
    type AgentState = {
      searches: {
        query: string;
        done: boolean;
    ...
    function YourMainContent() {
      ...
      useagentStateRender<AgentState>({
  ...
  <Step>
    ...
    // Define the state of the agent, should match the state of the agent in your agent framework.
    type AgentState = {
      searches: {
        query: string;
        done: boolean;
    ...
    function YourMainContent() {
      ...
      // [!code highlight:3]
      const { state } = useagent<AgentState>({
        name: "sample_agent", // the name the agent is served as
      })
