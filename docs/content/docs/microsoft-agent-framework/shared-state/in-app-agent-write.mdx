...
<video src="https://cdn.copilotkit.ai/docs/copilotkit/images/microsoft-agent-framework/write-agent-state.mp4" className="rounded-lg shadow-xl" loop playsInline controls autoPlay muted />
<Callout>
...
<Steps>
  ...
  <Step>
    ...
    `useagent` returns a `setState` function that you can use to update the agent state. Calling this
    ...
    function YourMainContent() {
      const { state, setState } = useagent<AgentState>({ // [!code highlight]
        name: "sample_agent",
...
function YourMainContent() {
  // [!code word:run:1]
  const { state, setState, run } = useagent<AgentState>({
    name: "sample_agent",
    initialState: { language: "spanish" }  // optionally provide an initial state
  });
