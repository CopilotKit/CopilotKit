...
// Removed agent framework-specific run-and-connect snippet
import { Tabs, Tab } from "fumadocs-ui/components/tabs";

<IframeSwitcher
  id="human-in-the-loop-example"
  exampleUrl="https://feature-viewer.copilotkit.ai/agent framework/feature/human_in_the_loop?sidebar=false&chatDefaultOpen=false"
  codeUrl="https://feature-viewer.copilotkit.ai/agent framework/feature/human_in_the_loop?view=code&sidebar=false&codeLayout=tabs"
  exampleLabel="Demo"
  codeLabel="Code"
  height="700px"
/>
...
<Callout type="info">
  This example demonstrates interrupt-based human-in-the-loop (HITL) in the [CopilotKit Feature Viewer](https://feature-viewer.copilotkit.ai/agent framework/feature/human_in_the_loop).
</Callout>

## What is this?

[agent framework's interrupt flow](https://agent-ai.github.io/agent framework/concepts/human_in_the_loop/) provides an intuitive way to implement Human-in-the-loop workflows.
...
<Tabs groupId="language" items={['Python', 'TypeScript']} default="Python">
    ...
    <Tab value="TypeScript">
        ...
        // ...
        import { Annotation } from "@agent/agent framework";
        import { CopilotKitStateAnnotation } from "@copilotkit/sdk-js/agent framework";
        // ...
...
<Step>
    ### Call `interrupt` in your agent framework agent
    Now we can call `interrupt` in our agent framework agent.

    <Callout type="info">
        Your agent will not be aware of the `interrupt` interaction by default in agent framework.
    ...
    <Tabs groupId="language" items={['Python', 'TypeScript']} default="Python">
        <Tab value="Python">
            ```python title="agent/sample_agent/agent.py"
            from agent framework.types import interrupt # [!code highlight]
            from agent_core.messages import SystemMessage
        ...
        <Tab value="TypeScript">
            ```typescript title="agent-js/src/agent.ts"
            import { interrupt } from "@agent/agent framework"; // [!code highlight]
            import { SystemMessage } from "@agent/core/messages";
...
<Step>
    ...
    At this point, your agent framework agent's `interrupt` will be called. However, we currently have no handling for rendering or
    responding to the interrupt in the frontend.

    To do this, we'll use the `useagent frameworkInterrupt` hook, give it a component to render, and then call `resolve` with the user's response.

    ```tsx title="app/page.tsx"
    import { useagent frameworkInterrupt } from "@copilotkit/react-core"; // [!code highlight]
    // ...
...
<video src="https://cdn.copilotkit.ai/docs/copilotkit/images/microsoft-agent-framework/interrupt-flow.mp4" className="rounded-lg shadow-xl" loop playsInline controls autoPlay muted />
</Step>
</Steps>
...
When rendering multiple `interrupt` events in the agent, there could be conflicts between multiple `useagent frameworkInterrupt` hooks calls in the UI.
...
<Steps>
    ...
    <Step>
        ...
        With two `useagent frameworkInterrupt` hooks in our page, we can leverage the `enabled` property to enable each in the right time:
...
import { useagent frameworkInterrupt } from "@copilotkit/react-core";
