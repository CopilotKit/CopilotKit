...
<IframeSwitcher
  id="frontend-tool-based-hitl-example"
  exampleUrl="https://feature-viewer.copilotkit.ai/microsoft-agent-framework/feature/human_in_the_loop?sidebar=false&chatDefaultOpen=false"
...
<Steps>
    ...
    <Step>
        ...
        Frontend tools registered with `useHumanInTheLoop` are forwarded to your AG-UI server in the `tools` parameter. You can bind them to your agent to make them callable.
        ...
        {
            // agentInput.Tools contains frontend tools from CopilotKit
            // including the "humanApprovedCommand" tool defined with useHumanInTheLoop
            ...
            // Frontend tools are available to the agent through the AG-UI protocol
        ...
        <Callout type="info">
          Frontend tools defined with `useHumanInTheLoop` are automatically available to your agent. The tool execution and rendering happen on the frontend, providing seamless human-in-the-loop interactions.
        </Callout>
    </Step>
    <Step>
        ### Give it a try!
