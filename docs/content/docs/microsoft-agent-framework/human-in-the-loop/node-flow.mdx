...
<Callout type="error">
    The usage of node based interrupt is [now discouraged](https://agent-ai.github.io/agent framework/concepts/v0-human-in-the-loop/) by both agent framework and CopilotKit.
    As of agent framework 0.2.57, the recommended way to set breakpoints is using [the interrupt function](https://docs.copilotkit.ai/microsoft-agent-framework/human-in-the-loop/interrupt-flow) as it simplifies human-in-the-loop patterns.
</Callout>

<video src="https://cdn.copilotkit.ai/docs/copilotkit/images/microsoft-agent-framework/node-hitl.mp4" className="rounded-lg shadow-xl" loop playsInline controls autoPlay muted />
...
## What is this?
[Node based flows](https://agent-ai.github.io/agent framework/concepts/v0-human-in-the-loop/#dynamic-breakpoints) are predicated on agent framework concept
...
<Steps>
    ...
    <Step>
    ### Setup the agent framework Agent
    Now we'll setup the agent framework agent. Node-based flows are hard to understand without a complete example, so below
    ...
    <Tabs groupId="language" items={["Python", "TypeScript"]}>
      <Tab value="Python">
        ...
        from agent framework.graph import StateGraph, END
        from agent framework.checkpoint.memory import MemorySaver
        from agent framework.types import Command
      ...
      <Tab value="TypeScript">
        ...
        import { tool } from "@agent/core/tools";
        import { ToolNode } from "@agent/agent framework/prebuilt";
        import { AIMessage, HumanMessage, SystemMessage, ToolMessage } from "@agent/core/messages";
        import { Command, END, MemorySaver, START, StateGraph } from "@agent/agent framework";
        ...
        import { convertActionsToDynamicStructuredTools } from "@copilotkit/sdk-js/agent framework";
