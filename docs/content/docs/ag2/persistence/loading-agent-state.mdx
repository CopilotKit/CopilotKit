...
(See [Message Persistence](/ag2/persistence/message-persistence) for more details.)
...
<Callout>
  **Important:** For agent state to be loaded correctly, you must first ensure
  ...
  guides on [Threads &
  Persistence](/ag2/persistence/loading-message-history) and [Message
  Persistence](/ag2/persistence/message-persistence).
</Callout>
...
- [Reading agent state](/ag2/shared-state/in-app-agent-read)
- [Writing agent state](/ag2/shared-state/in-app-agent-write)
