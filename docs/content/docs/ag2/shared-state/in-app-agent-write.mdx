...
<Steps>
  ...
  <Step>
    ...
    AG2 Agents are stateful. As you transition through the flow, that state is updated and available to the next function. For this example,
...
By default, the AG2 Agent state will only update _between_ AG2 Agent node transitions --
...
See **[emit intermediate state](/ag2/shared-state/predictive-state-updates).** 