...
<Callout>
  ...
  project](https://github.com/CopilotKit/CopilotKit/tree/main/examples/coagents-starter-adk).
</Callout>
...
<Tabs groupId="language" items={['Python']} default="Python">
    <Tab value="Python">
        ...
        time_agent = Agent(
          ...
          name="time_agent",
          instruction=f"""
              You are an expert time teller that can tell the current time in a specific timezone.
              """
        )

        adk_agent_time = ADKAgent(
          adk_agent=time_agent,
          app_name="time_demo"
          user_id="demo_user"
        )
        ...
        # Create FastAPI app
        app = FastAPI(title="ADK Middleware Time Tool Agent")

        # Add the ADK endpoint
        add_adk_fastapi_endpoint(app, adk_agent_time, path="/")
        ```
    </Tab>
