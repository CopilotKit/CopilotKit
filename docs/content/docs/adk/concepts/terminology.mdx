...
| Agent State                  | The current data and context maintained by an ADK agent during its execution, including both internal state and data that can be synchronized with the frontend UI.                                |
...
| [ADK](https://google.github.io/adk-docs/) | The agent framework integrated with CopilotKit that provides the orchestration layer for CoAgents, enabling sophisticated multi-step reasoning and state management.                               |
