...
There are two thread persistence paths:

- **Managed Intelligence threads** — Copilot Cloud or self-hosted Enterprise Intelligence stores thread metadata, messages, events, and state. This is the path used by `useThreads`, the Inspector threads tab, realtime thread metadata, thread retention policies, rename, archive, and delete.
- **Self-managed runner persistence** — an `AgentRunner`, `SqliteAgentRunner`, or custom runner can persist and replay conversation data for your runtime. This can power resumable chat, but it does not automatically provide the managed `useThreads` thread list, metadata mutations, realtime thread management, or Inspector thread browsing unless your runtime also exposes compatible REST thread endpoints.

For local development, the default `InMemoryAgentRunner` exposes temporary REST thread endpoints so the Inspector can show thread history during the current process. Those threads are not durable across restarts.
