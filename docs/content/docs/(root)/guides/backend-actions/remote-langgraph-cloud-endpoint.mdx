...
const runtime: CopilotRuntimeConfig = {
    ...
    remoteEndpoints: [ // [!code highlight:5]
        langGraphCloudEndpoint({
            deploymentUrl: "http://localhost:8000/copilotkit_remote",
            langsmithApiKey: "http://localhost:8000/copilotkit_remote",
            // List here all agents which are available under "graphs" list in your langgraph.json file.
            // The agent name must strictly match one of the keys in that list
...
After setting up the remote endpoint and modifying your `CopilotRuntime`, you can test your implementation by asking the copilot to perform actions that invoke your LangGraph agent.
