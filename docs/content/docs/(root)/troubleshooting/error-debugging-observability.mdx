---
title: "Observability & Event Hooks"
description: "Monitor your CopilotKit application with comprehensive event hooks and error observability. Track user interactions, chat events, and system errors."
icon: "lucide/Activity"
---

CopilotKit provides comprehensive observability capabilities to help you monitor your application and understand user behavior. Track chat interactions, user events, and system errors with powerful hooks that only work when you provide a `publicApiKey`.

## Quick Start

<Callout type="info">
  **All observability hooks require `publicApiKey`** - Get yours free at
...
Track user interactions and chat events with comprehensive event hooks:
...
Track user interactions and chat behavior with comprehensive event hooks (requires `publicApiKey`):
...
- ✅ Works with `CopilotModal`, `CopilotChat`, and all pre-built components
- ✅ Only triggers when `publicApiKey` is provided

<Callout type="warn">
  **Important:** Event hooks will **not trigger** without a valid
  `publicApiKey`. This is a security feature to ensure observability hooks only
...
The error observability hooks provide programmatic access to detailed error information for monitoring and analytics (requires `publicApiKey`):
...
<Callout type="warn">
  **Important:** Error observability hooks will **not trigger** without a valid
  `publicApiKey`. The `onError` hook is specifically for **error observability**
...
To use observability hooks (event hooks and error observability), you'll need a CopilotKit Cloud account:
...
- **Always provide `publicApiKey`** for observability hooks to work
- **Use event hooks for user analytics** - track message patterns, engagement, and feedback
...
- **Don't expect hooks to work without `publicApiKey`** - they're designed to be secure
...
### Event Hooks Not Working

If your event hooks aren't being called:

1. **Check your publicApiKey** - all observability hooks require a valid API key
...
**Event hooks not triggering:**

- Missing or invalid `publicApiKey`
- Event hooks defined but not on the right components
- Network issues preventing validation

**Error observability not working:**

- Missing `publicApiKey` in CopilotKit provider
- Incorrect key format (should start with `ck_pub_`)
- Environment variables not loaded correctly

## Next Steps

- **Get your publicApiKey** at [CopilotKit Cloud](https://cloud.copilotkit.ai) - required for all observability hooks
- Explore the [CopilotModal reference](/reference/components/chat/CopilotModal) for event hooks
- Check out the [CopilotKit reference](/reference/components/CopilotKit) for error observability
- Learn about [CopilotKit Cloud features](https://cloud.copilotkit.ai) for production monitoring
- Review [troubleshooting guides](/troubleshooting/common-issues) for common issues
