...
<Callout type="info">
**Easy MCP Integration**: CopilotKit now provides automatic MCP client handling that works out of the box! Simply add your MCP server endpoints and CopilotKit will handle the connection automatically. No additional configuration needed.
</Callout>

### Simple Usage (Works Out of the Box)

MCP servers now work automatically with no additional setup:
...
For advanced use cases, you can still provide your own MCP client:

```typescript
import { CopilotRuntime, OpenAIAdapter, createDefaultMCPClient } from "@copilotkit/runtime";

const runtime = new CopilotRuntime({
  mcpServers: [
    { endpoint: "https://your-mcp-server.com/sse" }
  ],
  createMCPClient: createDefaultMCPClient  // Optional: use custom implementation
});
