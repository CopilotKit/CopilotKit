...
{/* What we would like to do here, is simply import SelfHostingCopilotRuntimeConfigureCopilotKitProvider. But code snippets inside imported snippets look funky so alas we cannot.  */}
...
Point the `<CopilotKit>` provider at the Copilot Runtime URL you configured in the previous step.
