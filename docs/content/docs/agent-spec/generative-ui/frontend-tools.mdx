...
This can be utilized for to let [your agent control the UI](/agent-spec/frontend-actions), [generative UI](/agent-spec/generative-ui/frontend-tools), or for Human-in-the-loop interactions.
...
<Steps>
    ...
    <Step>
        ...
        sayhello_tool = ClientTool( # ClientTool are used as frontend tools
            name="sayHello",
            description="Say hello to the user.",
        ...
        runtime = "langgraph" # you can also choose "crewai" or "wayflow"
        router = APIRouter()
        add_agentspec_fastapi_endpoint(
            ...
            path=f"/{runtime}/backend_tool_rendering",
        )
