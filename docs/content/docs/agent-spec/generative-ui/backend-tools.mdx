...
<IframeSwitcher
  ...
  urlProps={{ integration: ['langgraph', 'wayflow'] as const }}
  exampleUrl={({ integration }) => `https://feature-viewer.copilotkit.ai/agent-spec-${integration}/feature/backend_tool_rendering?sidebar=false&chatDefaultOpen=false`}
  codeUrl={({ integration }) => `https://feature-viewer.copilotkit.ai/agent-spec-${integration}/feature/backend_tool_rendering?view=code&sidebar=false&codeLayout=tabs`}
  exampleLabel="Demo"
  codeLabel="Code"
  height="700px"
/>
...
<Callout>
  This example demonstrates the [implementation](#implementation) section applied in the <a href="https://feature-viewer.copilotkit.ai/agent-spec/feature/agentic_chat" target="_blank">CopilotKit feature viewer</a>.
</Callout>
