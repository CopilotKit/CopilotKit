...
<Callout type="tip">
  The dev console shows error banners and toasts directly in your UI, making it
  easy to spot issues during development. **No API key required** for this
  feature.
</Callout>
...
For production applications, use the `onError` handler with CopilotKit Cloud:
...
<Callout type="info">
  **Need a publicApiKey?** Go to
  ...
  free! The `onError` handler is a premium CopilotKit Cloud feature.
</Callout>
...
- ✅ **Visual error banners** - errors appear as toasts in your UI
...
- Premium CopilotKit Cloud feature
