...
<ReferenceChips header="Related Concepts" references={[
    { title: "Multi-agent workflows", link: "/concepts/coagents/multi-agent-flows" },
]} />
...
Router Mode is enabled by default when using Agentic Copilots. To use it, specify a runtime URL prop in the `CopilotKit` provider component and omit the `agent` prop, like so:
