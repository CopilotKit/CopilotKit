...
<Steps>
    ...
    <Step>
        ### Define the state
        We'll be defining a `written_report` field in the state, which will be updated as the agent writes different sections of the report.
        ...
        <Tabs groupId="language" items={["Python", "TypeScript"]}>
            ...
            <Tab value="TypeScript">
                ...
                export const AgentStateAnnotation = Annotation.Root({
                    written_report: Annotation<string>,
    ...
    <Step>
        ...
        >
            ...
            <TailoredContentOption
                ...
                icon={<FaWrench />}
            >
                For long-running tasks, you can configure CopilotKit to automatically emit intermediate state updates when specific tool calls are made. In this example, we'll configure CopilotKit to update the frontend whenever the LLM calls the report writing tool.
                <Tabs groupId="language" items={['Python', 'TypeScript']} default="Python">
                    <Tab value="Python">
                        ```python
                        ...
                        def frontend_actions_node(state: AgentState, config: RunnableConfig):
                            # Configure CopilotKit to treat report writing tool calls as predictive of the final state
                            config = copilotkit_customize_config(
                                config,
                                emit_intermediate_state=[
                                    {
                                        "state_key": "written_report",
                                        "tool": "ReportWriterTool",
                                        "tool_argument": "report_text"
                                    },
                                ]
                            )

                            # Define the report writing tool
                            @tool("ReportWriterTool")
                            def write_report_section(report_text: str) -> str:
                                """Writes a section of the report."""
                                return report_text

                            # Provide the actions to the LLM
                            model = ChatOpenAI(model="gpt-4").bind(
                                functions=[write_report_section]
                            )
                        ...
                        async function frontendActionsNode(state: AgentState, config: RunnableConfig): Promise<AgentState> {
                            ...
                            const modifiedConfig = copilotkitCustomizeConfig(
                                ...
                                emitIntermediateState=[
                                    {
                                        "stateKey": "written_report",
                                        "tool": "ReportWriterTool",
                                        "toolArgument": "report_text",
                                    },
                                ],
                            );

                            # Define the report writing tool
                            const writeReportSection = tool("ReportWriterTool", {
                                description: "Writes a section of the report",
                                parameters: {
                                    report_text: { type: "string" }
                                },
                                function: async (report_text: string) => {
                                    return report_text;
                                },
                            });
