...
description: CopilotKit's CoAgents are the missing link that makes it easy to connect
...
Chatbots and other copilot-like AI experiences are useful, but can be limited. AIs perform best when they work alongside humans, not just for them, proactively asking for and taking feedback to ensure the best possible output. By including human users in agent-driven workflows — which is often simpler and easier than trying to fully automate those tasks — we can achieve amazing things.

Building good workflows to enable human-AI collaboration on the web has been no small task — until now. **CoAgents** from CopilotKit simplify integrating LangGraph-powered agents into any React application. On the backend, our Python SDK makes it simple to enable a new or existing agent for use in your web apps. On the frontend, our React components and hooks make agent-driven AI experiences as easy to implement as basic chat.
...
We will soon add native & seamless Agent Q&A support in `useCoAgentRenderState`.

In the meantime, you can achieve agent Q&A in the following way:

1. Use `useCoAgentRenderState` to render your "Question" LangGraph node (via the exposed state).
2. In your LangGraph, go to an `END` node, or use a breakpoint, to wait for user input.
3. In the `useCoAgentRenderState` render function, call `setState` provided by `useCoAgent` to provide the "answer" in the graph state.
