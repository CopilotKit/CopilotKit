...
I.e. you can render to the user what the agent is currently doing in the chat UI.
Continuously rendering the agent's state is the gold standard for agentic UX; it keeps users **engaged** and **builds trust** by demonstrating that the agent is on the right track.
