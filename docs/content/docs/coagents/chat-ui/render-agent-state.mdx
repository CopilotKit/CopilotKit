...
name: "basic_agent", // the name of the agent
nodeName: "optionally_specifiy_a_specific_node", // Optional! You can specify a specific node you want generative UI for. Omit this argument if you want to render the agent's state for any node.
render: ({ status, state, nodeName }) => {
  return (
