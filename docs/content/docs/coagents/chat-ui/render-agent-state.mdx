---
title: Rendering agent state in chat
...
basic_agent_graph = workflow.compile(checkpointer=MemorySaver())
```
...
With the `useCoAgentRenderState` hook, we'll listen for this agent to return a structured weather forecast, and return it as a multiline string.
...
To render state in the chat window, use the `useCoAgentRenderState` hook:

```tsx
useCoAgentRenderState<WeatherAgentState>({
