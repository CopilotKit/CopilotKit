...
<Callout type="warning">
  The name of the `EmailTool` class and its argument `the_email` MUST match what we set up on the frontend. Ensure that the `name` in `useCopilotAction` is "EmailTool" and the parameter name is "the_email" for consistency between frontend and backend.
</Callout>
