...
- Tavily API key: This can be retrieved from the Tavily console [here](https://tavily.com/).
...
With that created, we'll want to add your OPENAI_API_KEY and TAVILY_API_KEY to the file.
...
We're going to use the LangGraph CLI to run the agent.
...
The CLI will open up a new browser tab to LangSmith. Here you will be able to see a LangGraph Studio session to visualize
and interact with the agent.
<Callout>
The Deployment URL of the agent will be https://localhost:8000. We'll use this in later steps.
</Callout>
