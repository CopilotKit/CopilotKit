...
<Tabs groupId="hosting" items={['Copilot Platform', 'Self-Hosted']}>
<Tab value="Copilot Platform">
We're going to use the CopilotKit CLI to setup a tunnel between our locally running LangGraph agent and Copilot Platform. You'll 
...
✔ 🚀 Local tunnel is live and linked to Copilot Platform!
```
...
file. When we deploy our agent to Copilot Platform this is automatically handled for us. When self-hosting, we need to specify the name of the agent in the `langGraphPlatformEndpoint` constructor.
</Step>
<Step>
