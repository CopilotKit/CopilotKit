...
If you'd like to learn even more about human in the loop before proceeding, checkout our [Human in the Loop concept guide](/coagents/concepts/human-in-the-loop).
...
In this case, we are checking if the user decided to cancel the operation. If so, we return the state with a new message that communicates the user's decision. Any other decision returned will result in the requested actions being performed.
