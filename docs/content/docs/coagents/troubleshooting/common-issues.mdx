...
This is typically because the `messages` state is being updated in the middle of a node but those edits are not being
persisted at the end of a node. 

LangGraph agents are stateful. As a graph is traversed, the state is saved at the end of each node. However, you can
emit state and stream updates *in the middle of a node*. By default, CopilotKit will stream messages and tool calls
being actively generated to the frontend chat that initiated the interaction. **If that message is not persisted at the
end of the node, it will disappear in the frontend chat**.

<Accordions>
    <Accordion title="Give me a visual">
        <img src="/images/coagents/message-streaming.svg" />
    </Accordion>
