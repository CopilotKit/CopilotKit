...
<Step>
## Go through the primary CopilotKit quickstart
Please first go through the [primary CopilotKit quickstart](/quickstart) if you haven't yet.
...
<Step>
## Setup a _Remote Endpoint_ using Python + FastAPI
For now, CoAgents only targets LangGraph + Python (Support for LangGraph.js and LangGraph Cloud is coming soon.)

To connect a LangGraph agent to your application, first setup a [_CopilotKit **Remote Endpoint**_ and attach it to your Copilot Runtime instance](/guides/backend-actions/remote-backend-endpoint).
...
<Step>
## Add your LangGraph agent!
For now, CoAgents only targets LangGraph + Python (Support for LangGraph.js and LangGraph Cloud is coming soon.)

Setup a _CopilotKit **Remote Endpoint**_ and attach it to your application's Copilot Runtime instance.
</Step>
...
LangGraph agents are written in Python, using the [LangGraph Python SDK by LangChain](https://langchain-ai.github.io/langgraph/) and an API runtime server such as FastAPI. This guide presumes you're familiar with LangGraph and FastAPI, and are looking to use CopilotKit to build a great user experience for your AI agent.

## Setup a _Remote Endpoint_ using Python + FastAPI
For now, CoAgents only targets LangGraph on Python (we will expand support soon to LangGraph.js and LangGraph Cloud.)
