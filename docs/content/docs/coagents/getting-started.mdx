...
At the moment CoAgents integrates with LangGraph-powered agents using **LangGraph-Python**, either self hosted or deployed to LangGraph Platform.
...
- In this tutorial, we'll hook up the 2 middle components:
  - **CopilotKit Runtime**
  - **Remote Endpoint (FastAPI/LangGraph Platform + CopilotKit Python SDK)**
...
<Step>
  Then, setup a CopilotKit Remote-Endpoint using FastAPI:<br />
  If you're using LangGraph Platform, follow the [LangGraph Platform Endpoint quickstart](/guides/backend-actions/langgraph-platform-endpoint),
  otherwise, follow the [Remote Endpoint quickstart](/guides/backend-actions/remote-backend-endpoint).<br />
