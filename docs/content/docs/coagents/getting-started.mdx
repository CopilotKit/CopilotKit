...
At the moment CoAgents integrates with LangGraph-powered agents (for both Python and TypeScript), either self hosted (Python only) or deployed to LangGraph Platform.
...
- In this tutorial, we'll hook up the 2 middle components:
  - **CopilotKit Runtime**
  - **Remote Endpoint (FastAPI/LangGraph Platform + CopilotKit Python/TypeScript SDK)**
...
<Step>
  If you want to use LangGraph Platform, follow the [LangGraph Platform guide](/guides/backend-actions/langgraph-platform-endpoint) and stop at this step.<br/>
  Otherwise, follow the [Remote Endpoint quickstart](/guides/backend-actions/remote-backend-endpoint) to setup a FastAPI Remote-Endpoint.<br/>
...
<CopilotKit
  ...
  graph="basic_agent" // agent-lock the Copilot, see 'agent-lock vs router-mode' // [!code highlight]
>
  {...}
</CopilotKit>
```
