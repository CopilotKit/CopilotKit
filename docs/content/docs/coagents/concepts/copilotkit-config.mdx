...
## Emitting Tool Calls
Emission of tool calls are on by default. This means that all tool calls will be sent to Copilotkit for processing and rendering.
However, within a node you can extend the LangGraph `config` object to only emit specific tool calls or no tool calls at all to Copilotkit.
