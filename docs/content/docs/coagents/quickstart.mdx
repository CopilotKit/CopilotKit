...
<TailoredContent>
  <TailoredContentOption 
    title="Self-hosted"
    description="I want to write my LangGraph agent in Python and self-host it."
    icon={<FaPython />}
  >
    <Steps>
      <Step>
  ...
  >
    <Steps>
      ...
      <Step>
        ...
        Follow the official [LangGraph Cloud deployment guide](https://langchain-ai.github.io/langgraph/cloud/deployment/cloud/).

        A successful deployment will yield an API URL (often referred to here as "deployment URL") in the following format: 

        ```
        https://{project-identifiers}.langgraph.app
        ```
