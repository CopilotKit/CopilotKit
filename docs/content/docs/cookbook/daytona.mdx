...
[Built-in Agent](/built-in-agent) that runs code inside one of those sandboxes and returns the result to
...
Add a `runCode` [server tool](/built-in-agent/server-tools) and register it on your `BuiltInAgent`. The
...
Built-in Agent app. If your agent supports installable skills, the `copilotkit-daytona` skill packages this
same setup (tool code, references, and an eval) as a reusable Agent Skill:
...
3. In my CopilotKit runtime route, create a Daytona client with `new Daytona()` and define a
   ...
   - execute: create a sandbox with `daytona.create({ language })`, run the code with
     `sandbox.process.codeRun(code)`, return `{ stdout, exitCode }`, and `sandbox.delete()` in a
     finally block.
