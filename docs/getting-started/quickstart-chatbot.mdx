...
export default function RootLayout({children}) {
  return (
    <CopilotKit chatApiEndpoint="/path_to_copilotkit_endpoint/see_below">
      <CopilotSidebar>
        {children}
      </CopilotSidebar>
    </CopilotKit>
...
Setup a backend endpoint (and pass its URL as the `chatApiEndpoint` argument to `CopilotKit`, as above).
