...
<Tabs>
  <Tab title="Manually specified endpoint: nodeJS (typescript)">
    ...
    <Tab title="Manually specified endpoint: flask (python)">

      Currently all you require is an endpoint which serves as a proxy to an OpenAI-like LLM endpoint.
      See below for an example using a Flask server.
