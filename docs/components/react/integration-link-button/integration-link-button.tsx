...
export const IntegrationLinkButton = ({ label, Icon, href, rounded = false }: IntegrationLinkButtonProps) => {
  return (
    <Link href={href} className='w-full no-underline h-9 md:h-[60px] hover:opacity-100'>
      <div
        className={`flex gap-2 justify-between items-center p-1 w-full h-full ${
          rounded ? 'rounded-full' : 'rounded-md'
        } bg-[#FFFFFF80]/50 dark:bg-background/50 hover:bg-white/50 dark:hover:bg-[#1A2022] group pr-4`}>
        <div className='flex gap-2 justify-start items-center w-full'>
          <div
            className={`w-9 md:w-[52px] h-9 md:h-[52px] flex flex-shrink-0 items-center justify-center bg-[#0105070D] dark:bg-white/5 ${
              rounded ? 'rounded-full' : 'rounded-md'
            } group-hover:bg-[#BEC2FF] dark:group-hover:bg-[#7076D5] transition-all duration-200`}>
            <Icon className='text-foreground' />
          </div>
          <span className='text-xs md:text-sm font-medium no-underline'>{label}</span>
        </div>

        <div className='hidden md:block invisible group-hover:visible text-foreground'>
