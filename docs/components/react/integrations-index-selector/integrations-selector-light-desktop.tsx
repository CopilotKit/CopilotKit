export function IntegrationsSelectorLightDesktop({ className, rows = 3, rowHeight = 60 }: { className?: string; rows?: number; rowHeight?: number }) {
  // Unique ID prefix to avoid conflicts
  const idPrefix = `light-desktop-${rowHeight}`;
  ...
  return (
    ...
    >
      ...
      <defs>
        {Array.from({ length: rows }, (_, i) => (
          <linearGradient key={i} id={`${idPrefix}-connector${i}`} x1={CONNECTOR_START_X} y1={KITE_CENTER_Y} x2={CONNECTOR_END_X} y2={getRowY(i)} gradientUnits='userSpaceOnUse'>
            <stop stopColor='#BEC2FF' stopOpacity='0' />
            <stop offset='0.4' stopColor='#BEC2FF' />
