export function IntegrationsSelectorDarkMobile({ className, rows = 7, rowHeight = 60 }: { className?: string; rows?: number; rowHeight?: number }) {
  ...
  const idPrefix = `dark-mobile-${rowHeight}`;
  ...
  return (
    <svg
      ...
      />
      <circle opacity='0.3' cx='57.6' cy='57.6' r='57.1' stroke={`url(#${idPrefix}-paint5)`} />
      
      {/* Main trunk line */}
      <path d={trunkPath} stroke='#7076D5' />
      
      {/* Branch connectors */}
      {branchPaths.map((d, i) => (
        <path key={i} d={d} stroke='#7076D5' />
      ))}
