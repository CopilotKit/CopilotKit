/**
 * The `lookup_discord_user` parameter schema used to be a Zod object. It is
 * now built by `singleStringParameterSchema` from `@copilotkit/channels-core`,
 * so that `@copilotkit/channels-discord` declares no `zod` dependency (PE-30).
 *
...
import { lookupDiscordUserTool } from "../built-in-tools.js";

describe("lookup_discord_user parameter schema", () => {
  it("emits the same JSON Schema the Zod object emitted", () => {
    const [descriptor] = toAgentToolDescriptors([lookupDiscordUserTool]);

    expect(descriptor).toBeDefined();
    expect(descriptor?.name).toBe("lookup_discord_user");
  ...
  it("accepts a valid query", async () => {
    const r = await parseToolArgs(lookupDiscordUserTool.parameters, {
      query: "ada",
    });
  ...
  it("rejects a non-object argument payload", async () => {
    const r = await parseToolArgs(lookupDiscordUserTool.parameters, null);
    expect(r.ok).toBe(false);
  });
});
