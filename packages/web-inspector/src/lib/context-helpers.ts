...
export function updateSizeFromElement(state: ContextState, element: HTMLElement, fallback: Size): void {
...
export function clampSize(size: Size, viewport: Size, edgeMargin: number, minWidth: number, minHeight: number): Size {
...
export function keepPositionWithinViewport(state: ContextState, viewport: Size, edgeMargin: number): void {
  state.position = constrainToViewport(state, state.position, viewport, edgeMargin);
}

export function centerContext(state: ContextState, viewport: Size, edgeMargin: number): Position {
...
export function updateAnchorFromPosition(state: ContextState, viewport: Size, edgeMargin: number): void {
...
export function applyAnchorPosition(state: ContextState, viewport: Size, edgeMargin: number): Position {
  const maxHorizontalOffset = Math.max(edgeMargin, viewport.width - state.size.width - edgeMargin);
  const maxVerticalOffset = Math.max(edgeMargin, viewport.height - state.size.height - edgeMargin);
  ...
  const x =
    state.anchor.horizontal === "left" ? horizontalOffset : viewport.width - state.size.width - horizontalOffset;
