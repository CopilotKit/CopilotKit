...
public sealed class McpAppServer
{
    ...
    public string Hash => Convert.ToHexStringLower(MD5.HashData(Encoding.UTF8.GetBytes(new JsonObject { ["type"] = "http", ["url"] = Url.OriginalString }.ToJsonString(new JsonSerializerOptions { Encoder = JavaScriptEncoder.UnsafeRelaxedJsonEscaping }))));
        ...
        foreach (var server in servers)
        {
            try
            {
                ...
                do
                {
                    ...
                    {
                        if (A2UIValidation.Text(tool["_meta"]?["ui/resourceUri"]) is not { } resource || A2UIValidation.Text(tool["name"]) is not { } name) continue;
