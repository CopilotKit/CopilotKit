...
export function convertJsonSchemaToZodSchema(
  ...
  if (Array.isArray(unionVariants) && unionVariants.length > 0) {
    if (unionVariants.length === 1) {
      return convertJsonSchemaToZodSchema(unionVariants[0], required, defs, visitedRefs);
    }
    const schemas = unionVariants.map((v: any) => convertJsonSchemaToZodSchema(v, true, defs, visitedRefs));
