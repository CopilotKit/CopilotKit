...
export class WhatsAppClient {
  ...
  async sendMessage(to: string, payload: WhatsAppOutbound): Promise<WhatsAppMessageRef> {
  ...
  async uploadMedia(bytes: Uint8Array, mimeType: string, filename = "upload.bin"): Promise<string> {
  ...
  async downloadMedia(mediaId: string): Promise<DownloadedMedia> {
    ...
    const blobRes = await this.fetchImpl(meta.url, { headers: this.authHeader });
    if (!blobRes.ok) throw new Error(`WhatsApp media download failed: ${blobRes.status}`);
