...
export function CopilotChatInput({
  ...
  const BoundSendButton = renderSlot(sendButton, CopilotChatInput.SendButton, {
    ...
    children: isProcessing && canStop ? <Square className="cpk:size-[18px] cpk:fill-current" /> : undefined,
  });
    ...
    if (!shouldExpand) {
      ...
      if (cache && cache.compactWidth > 0) {
        const compactInnerWidth = Math.max(
          cache.compactWidth - (measurementsRef.current.paddingLeft || 0) - (measurementsRef.current.paddingRight || 0),
          0,
        );
  ...
  }, [adjustTextareaHeight, ensureMeasurements, mode, resolvedValue, updateContainerCache, updateLayout]);
  ...
  useEffect(() => {
    ...
    const containerTargets = new Set<Element>([grid, addContainer, actionsContainer]);
  ...
  const slashMenu = slashMenuVisible ? (
    ...
    >
      {filteredCommands.length === 0 ? (
        <div className="cpk:px-3 cpk:py-2 cpk:text-sm cpk:text-muted-foreground">No commands found</div>
      ) : (
          ...
          return (
            <button
              ...
              className={twMerge(
                ...
                isActive ? "cpk:bg-muted cpk:dark:bg-[#2f2f2f]" : "cpk:bg-transparent",
              )}
      ...
      >
        ...
        <div
          className={twMerge(
            ...
            isExpanded ? "cpk:col-span-3 cpk:row-start-1" : "cpk:col-start-2 cpk:row-start-1",
          )}
        >
          {mode === "transcribe" ? (
            BoundAudioRecorder
        ...
        <div
          ...
          className={twMerge(
            ...
            isExpanded ? "cpk:col-start-3 cpk:row-start-2" : "cpk:col-start-3 cpk:row-start-1",
          )}
        >
          {mode === "transcribe" ? (
            <>
  ...
  return (
    <div
      ...
      className={cn(
        ...
        positioning === "absolute" && "cpk:absolute cpk:bottom-0 cpk:left-0 cpk:right-0",
        className,
      )}
      style={{
...
export namespace CopilotChatInput {
  export const SendButton: React.FC<React.ButtonHTMLAttributes<HTMLButtonElement>> = ({
    className,
    children,
    ...props
  }) => (
