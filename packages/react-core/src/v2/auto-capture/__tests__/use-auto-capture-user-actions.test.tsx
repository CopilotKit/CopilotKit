...
vi.mock("../../hooks/use-learn-from-user-action", () => ({
  useLearnFromUserAction: vi.fn(),
}));
...
beforeEach(() => {
  ...
  mockUseLearnFromUserAction.mockReturnValue(recorder);
...
describe("useAutoCaptureUserActions", () => {
  ...
  it("never records the platform's own /annotate POST (no loop)", async () => {
    const { unmount } = renderHook(() =>
      useAutoCaptureUserActions({ enabled: true }),
    );

    await globalThis.fetch(`${RUNTIME_URL}/annotate`, {
      method: "POST",
      body: "{}",
      headers: { "content-type": "application/json" },
    });
  ...
  it("runs a developer-supplied transform end-to-end (envelope already redacted)", async () => {
    ...
    const { unmount } = renderHook(() =>
      useAutoCaptureUserActions({
        ...
        transform: (env) => {
          envelopeSeen = env;
          return { title: "custom-title", data: { upgraded: true } };
        },
      }),
    );
