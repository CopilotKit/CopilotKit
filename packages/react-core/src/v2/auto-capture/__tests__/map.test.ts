...
describe("defaultMapToAction", () => {
  ...
  it("puts the request body and request meta in the flat data snapshot", () => {
    const action = defaultMapToAction(captured, true);
  ...
  it("omits the response body when capture is disabled", () => {
    ...
    expect(action.data).not.toHaveProperty("responseBody");
  });
  ...
  it("preserves the full url (including query string) in data.url", () => {
    const action = defaultMapToAction(captured, true);
