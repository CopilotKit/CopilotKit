...
/**
 * Whether `url` is the platform's own `${runtimeUrl}/annotate` endpoint.
 * This is the loop guard: the recorder's POST flows through the patched fetch,
 * so it must always be excluded regardless of config — otherwise auto-capture
 * would re-capture its own recording requests and loop forever.
 */
export function isRecorderEndpoint(
  url: string,
  runtimeUrl: string | undefined,
): boolean {
  if (!runtimeUrl) return false;
  const target = stripToOriginPath(
    toAbsoluteUrl(`${runtimeUrl.replace(/\/$/, "")}/annotate`),
  );
...
export interface PipelineContext {
  ...
  record: (input: LearnFromUserActionInput) => void;
...
export function shouldCapture(
  ...
  if (isRecorderEndpoint(url, ctx.runtimeUrl)) return false;
