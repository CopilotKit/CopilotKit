 ...
 * for one captured request. It is exactly {@link LearnFromUserActionInput}
 * minus `threadId`, which auto-capture resolves and attaches itself (see
 * {@link AutoCaptureUserActionsConfig.threadId}).
 */
export type AutoCapturedUserAction = Omit<LearnFromUserActionInput, "threadId">;
...
export interface AutoCaptureUserActionsConfig {
  ...
  /**
   * URL patterns to never capture. The platform's own `${runtimeUrl}/annotate`
   * endpoint is always excluded regardless of this list (this is the loop guard
   * and cannot be overridden). Matching follows the same hostname-aware rules as
   * {@link allowUrls}.
   */
