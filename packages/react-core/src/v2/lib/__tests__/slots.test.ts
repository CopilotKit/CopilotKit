...
describe("useShallowStableRef", () => {
  it("returns the same reference when called twice with shallowly equal plain objects", () => {
    const initial = { a: 1 };
    const { result, rerender } = renderHook(({ value }: { value: { a: number } }) => useShallowStableRef(value), {
  ...
  it("handles undefined without crashing", () => {
    const { result } = renderHook(() => useShallowStableRef(undefined as unknown as { a: number }));
    expect(result.current).toBeUndefined();
  });

  it("handles null without crashing", () => {
    const { result } = renderHook(() => useShallowStableRef(null as unknown as { a: number }));
    expect(result.current).toBeNull();
  });

  it("does not shallow-compare arrays — treats them by reference", () => {
    const arr1 = [1, 2, 3];
    const { result, rerender } = renderHook(({ value }: { value: number[] }) => useShallowStableRef(value), {
