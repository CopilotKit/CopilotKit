...
/**
 * Detects default/placeholder message IDs generated by AI SDK providers
 * (e.g., @ai-sdk/openai-compatible) that should not be treated as unique
 * message identifiers. These IDs follow patterns like "0", "txt-0",
 * "reasoning-0" and reset for each response, causing message ID collisions.
 */
          ...
          for await (const part of response.fullStream) {
            switch (part.type) {
              ...
              case "reasoning-start": {
                // New reasoning message starting - use the SDK-provided id
                // Use randomUUID() if part.id is falsy or matches a default provider pattern
                // (e.g., "0", "reasoning-0") to prevent message merging issues
                const providedId = "id" in part ? part.id : undefined;
                if (
                  providedId &&
                  !DEFAULT_PROVIDER_ID_PATTERN.test(providedId)
                ) {
              ...
              case "text-start": {
                ...
                // Use randomUUID() if part.id is falsy or matches a default provider pattern
                // (e.g., "0", "txt-0", "reasoning-0") to prevent message merging issues
                const providedId = "id" in part ? part.id : undefined;
                messageId =
                  providedId && !DEFAULT_PROVIDER_ID_PATTERN.test(providedId)
