...
function resolveStreamMessageId(providedId: string | undefined): string {
  return providedId && !DEFAULT_PROVIDER_ID_PATTERN.test(providedId)
    ? providedId
    : randomUUID();
}
          ...
          for await (const part of response.fullStream) {
            switch (part.type) {
              ...
              case "reasoning-start": {
                // Replace default provider ids with a fresh UUID so reasoning blocks
                // from different responses cannot collide.
                const providedId = "id" in part ? part.id : undefined;
                reasoningMessageId = resolveStreamMessageId(providedId);
              ...
              case "text-start": {
                // Replace default provider ids with a fresh UUID so text blocks
                // from different responses cannot collide.
                const providedId = "id" in part ? part.id : undefined;
                messageId = resolveStreamMessageId(providedId);
                break;
              }
