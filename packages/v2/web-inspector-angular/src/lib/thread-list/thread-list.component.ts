...
// The shape of a thread row — matches ɵThread from @copilotkitnext/core.
...
@Component({
  ...
  template: `
    <div class="cpk-thread-list">
      ...
      >
        ...
        <span class="cpk-thread-list__truncate cpk-thread-list__user-id">{{ thread.id }}</span>
        <span>{{ thread.name ?? 'Untitled' }}</span>
        <span class="cpk-thread-list__truncate">{{ thread.summary ?? '—' }}</span>
  ...
  styles: [`
    ...
    .cpk-thread-list__header {
      ...
      align-items: center;
      background:
        radial-gradient(ellipse 340px 340px at 14% 30%,  rgba(117, 124, 242, 0.08) 0%, transparent 100%),
        radial-gradient(ellipse 380px 380px at 52% 50%, rgba(91, 228, 187, 0.08)  0%, transparent 100%),
        radial-gradient(ellipse 370px 370px at 97% 0%,  rgba(255, 172, 77, 0.08)  0%, transparent 100%),
