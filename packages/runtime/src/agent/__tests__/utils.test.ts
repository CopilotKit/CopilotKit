...
describe("convertJsonSchemaToZodSchema", () => {
  ...
  it("should preserve string format (uuid) constraints", () => {
    const jsonSchema = {
      type: "object" as const,
      properties: {
    ...
    const result = convertJsonSchemaToZodSchema(
      jsonSchema,
      true,
    ) as z.ZodObject<any>;
    const idSchema = result.shape.id;
  ...
  it("should preserve string pattern constraints", () => {
    const jsonSchema = {
      type: "object" as const,
      properties: {
        slug: { type: "string" as const, pattern: "^[a-z]+$" },
      },
      required: ["slug"],
    };
    ...
    const slugSchema = result.shape.slug;

    expect(slugSchema.safeParse("abc").success).toBe(true);
    expect(slugSchema.safeParse("Abc1").success).toBe(false);
  });
  ...
  it("should preserve string length constraints", () => {
    const jsonSchema = {
      type: "object" as const,
      properties: {
        code: { type: "string" as const, minLength: 2, maxLength: 4 },
      },
      required: ["code"],
    };
    ...
    const codeSchema = result.shape.code;

    expect(codeSchema.safeParse("ab").success).toBe(true);
    expect(codeSchema.safeParse("a").success).toBe(false);
    expect(codeSchema.safeParse("abcde").success).toBe(false);
  });

  // The regression behind the demo's save_memory failures: an OPTIONAL
  // uuid string must still reject "" (LLMs fill optional string fields
  // with "" instead of omitting them). Dropping the format turned this
  // into a bare optional string that accepts "", which then failed the
  // strict MCP server downstream.
  it("should keep format constraints on an optional string (reject empty)", () => {
    const jsonSchema = {
      type: "object" as const,
      properties: {
        supersedes: { type: "string" as const, format: "uuid" },
      },
      required: [] as string[],
    };
    ...
    const supersedesSchema = result.shape.supersedes;

    // optional → omitting is valid
    expect(supersedesSchema.safeParse(undefined).success).toBe(true);
    // but a provided value must still be a uuid; "" is rejected
    expect(supersedesSchema.safeParse("").success).toBe(false);
    expect(
      supersedesSchema.safeParse("123e4567-e89b-12d3-a456-426614174000")
        .success,
    ).toBe(true);
  });
