 ...
 */
export function getChatCompletionsForStreaming(openai: OpenAI): OpenAI["chat"]["completions"] {
...
export function submitToolOutputsStream(
  ...
  if (isOpenAIV5(openai)) {
    ...
    const submit = openai.beta.threads.runs.submitToolOutputsStream as {
      (...args: unknown[]): ReturnType<typeof openai.beta.threads.runs.submitToolOutputsStream>;
    };
