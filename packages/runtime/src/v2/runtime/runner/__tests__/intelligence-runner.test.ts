...
import { MockChannel, MockSocket } from "../../../../../../core/src/__tests__/test-utils";
...
describe("IntelligenceAgentRunner", () => {
  ...
  describe("run", () => {
    ...
    it("pushes agent events to the Phoenix channel", async () => {
      ...
      expect(ch.pushLog[0].payload.metadata.cpki_event_id).toEqual(expect.any(String));
    ...
    it("finalizes open message streams before completing", async () => {
      ...
      expect(ch.pushLog.map((p) => p.payload.metadata.cpki_event_seq)).toEqual([1, 2, 3, 4]);
    });
    ...
    it("preserves runner event order with increasing cpki_event_seq", async () => {
      ...
      expect(ch.pushLog.map((entry) => entry.payload.metadata.cpki_event_seq)).toEqual([1, 2, 3, 4, 5]);
    });
    ...
    it("emits RUN_ERROR and completes when channel join times out", async () => {
      ...
      expect(events).toHaveLength(1);
      expect(events[0].type).toBe(EventType.RUN_ERROR);
      expect((events[0] as RunErrorEvent).message).toBe("Timed out joining channel");
    });
  });
  ...
  describe("run with joinCode", () => {
    it("uses joinCode for the channel topic when provided", async () => {
      ...
      const eventsPromise = collectEvents(runner.run({ threadId, agent, input, joinCode }));
      const ch = mockChannels[0];
