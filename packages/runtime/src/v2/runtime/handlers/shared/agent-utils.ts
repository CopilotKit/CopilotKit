...
/**
 * Merge forwardable inbound request headers onto a per-request agent clone.
 *
 * Shared by the run path (`configureAgentForRequest` below) and the connect
 * path (`handleConnectAgent`), so both clones carry the same header set.
 *
...
export function configureAgentForRequest(params: {
  ...
  // Same header preparation as the connect path (`handleConnectAgent`):
  // forward eligible inbound headers, server-configured headers win.
