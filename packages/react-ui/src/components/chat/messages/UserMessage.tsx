...
const getMediaParts = (content: UserMessageContent | undefined) => {
  ...
  return content.filter(
    (part) => part.type === "image" || part.type === "audio" || part.type === "video" || part.type === "document",
  ) as Array<{
...
export const UserMessage = (props: UserMessageProps) => {
  ...
  const isLegacyImageMessage = message && "image" in message && Boolean((message as any).image);
  ...
  if (mediaParts.length === 0) {
    return <div className="copilotKitMessage copilotKitUserMessage">{textContent}</div>;
  }
