...
describe("Thread.post image routing", () => {
  ...
  it("throws on arbitrary JSX passed to postEphemeral()", async () => {
    const adapter = new FakeAdapter();
    const thread = makeThread(
      adapter,
      vi.fn(async () => new Uint8Array()),
    ...
    await expect(
      thread.postEphemeral("u1", createElement("div", null) as never, {
        fallbackToDM: true,
      }),
    ).rejects.toThrow(/does not support arbitrary JSX/);
  });

  it("post(image) with no opts uses the default filename and width", async () => {
    const adapter = new FakeAdapter();
    const filePosts: unknown[] = [];
    adapter.postFile = async (_t, args) => {
      filePosts.push(args);
    ...
    // No render config at all, so `postImage` falls through to its hard
