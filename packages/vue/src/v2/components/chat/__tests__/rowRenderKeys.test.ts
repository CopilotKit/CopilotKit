 ...
 * the component's post-flush watcher does. Anchors only exist once the DOM has
 * been patched, so a test that spans renders has to commit between them.
 */
function renderPass(
...
describe("resolveRowRenderKeys", () => {
  ...
  it("does not let a discarded evaluation's key reach the patched DOM", () => {
    // Vue can evaluate the keys computed without ever patching the DOM with
    // the result. If evaluating had recorded tc:call_A -> resp_1, the row the
    // user is actually looking at (still lc_run--1) would re-key and be torn
    // down on its next render — the flash this module exists to prevent.
    const store = createRowKeyStore();
  ...
  it("records anchors once the DOM is patched", () => {
    const store = createRowKeyStore();
