...
| ------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------- | ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `useAgent`, `useAgentContext`, `useSuggestions`, `useConfigureSuggestions`, `useFrontendTool`, `useHumanInTheLoop`, `useInterrupt`, `useThreads` | Same-named Vue composables in `src/hooks`                                          | matched                | Headless/data-oriented parity model.                                                                                                                         |
...
| `useRenderTool`, `useDefaultRenderTool`, `useComponent`                                                                                          | Same-named Vue composables in `src/hooks`                                          | matched                | Secondary programmatic APIs now exist with dedicated parity tests, including SFC/component renderer support; slots remain the primary customization surface. |
...
| `CopilotChat.attachments.test.tsx`               | `CopilotChat.attachments.test.ts`                 | matched | Dedicated strict counterpart mirrors React `onUploadFailed` coverage (`invalid-type`, `file-too-large`, `upload-failed`, multi-reject, and valid-file no-failure) and validates Vue attachment drop flow through `CopilotChat`.                               |
...
| `CopilotChatActivityRendering.e2e.test.tsx`      | `CopilotChatActivityRendering.e2e.test.ts`        | matched | Suite/case titles and approach now match React, including per-thread clone forwarding to activity renderers (`getThreadClone(...)` parity assertion).                                                                                                         |
...
| `CopilotChatInput.test.tsx`                      | `CopilotChatInput.test.ts`                        | matched | Dedicated strict counterpart mirrors the React suite/case structure and depth, including slash-command filtering, execution, and action-driven input population behavior.                                                                                     |
...
| `use-agent-throttle.test.tsx`            | `use-agent-throttle.test.ts`                                                    | matched | Dedicated counterpart now covers `defaultThrottleMs` provider fallback, hook override precedence, and invalid-value fallback behavior.                                      |
...
| `use-threads.test.tsx`                   | `use-threads.test.ts`                                                           | matched | Dedicated strict counterpart preserves the full React case set and wording; Vue-only reactivity coverage is isolated in a trailing `Vue-specific reactive semantics` block. |
...
| React test anchor              | Current Vue state  | Classification | Next action                                                                                                              |
| ------------------------------ | ------------------ | -------------- | ------------------------------------------------------------------------------------------------------------------------ |
| `use-katex-styles.test.tsx`    | No Vue counterpart | `api-gap`      | Confirm whether Vue intends to expose an equivalent public hook; if not, document it as out of scope for package parity. |
| `use-keyboard-height.test.tsx` | No Vue counterpart | `api-gap`      | Confirm whether Vue intends to expose an equivalent public hook; if not, document it as out of scope for package parity. |
