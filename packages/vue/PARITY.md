...
| `CopilotChatActivityRendering.e2e.test.tsx`      | `CopilotChatActivityRendering.e2e.test.ts`        | matched | Suite/case titles and approach now match React, including per-thread clone forwarding to activity renderers (`getThreadClone(...)` parity assertion) and both `MockReconnectableAgent` durable-compaction restoration cases (`a2ui-surface` and `open-generative-ui`). The `IntelligenceAgent /connect` gateway-replay variant is pending `N3`.                                                                                                                                                                       |
