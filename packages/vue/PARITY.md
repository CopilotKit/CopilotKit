...
- A2UI built-in `render_a2ui` tool registration (Vue-specific correction):
  - Vue removes the built-in no-op `render_a2ui` registration when A2UI deactivates, without clearing user `useRenderTool` / slot registrations.
...
| `A2UIMessageRenderer.test.tsx`                                                                     | `A2UIMessageRenderer.test.ts` + `A2UIRecoveryStates.test.ts` | matched | Dedicated strict counterparts mirror React's A2UI rendering matrix (surface render, in-place update, multi-surface independence), OSS-162 recovery lifecycle states (building/retrying/failed skeletons, token phases, retry thresholds, debug exposure precedence), and `runA2UIAction` interception. Default loading uses React's `A2UIBuildingState` skeleton (`"Building interface"`), not the retired `"Generating UI..."` tool-progress UI. Custom `loadingComponent` applies only to `building`, never `retrying`/`failed`.                                                                                                                    |
