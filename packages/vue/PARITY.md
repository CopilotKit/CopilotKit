...
- A2UI built-in `render_a2ui` tool registration (Vue-specific correction):
  - The built-in no-op lives in `CopilotKitProvider`'s computed `allRenderToolCalls` list (stable singleton identity) while A2UI is active, so later prop/frontend-tool sync cannot drop it.
  - On deactivation the built-in is omitted from the computed list; user `useRenderTool` / slot registrations and any provider `renderToolCalls` entry are preserved.
  - React currently lacks equivalent cleanup and may later overwrite registry entries; Vue documents this as an intentional lifecycle divergence.
...
| `A2UIMessageRenderer.test.tsx`                                                                     | `A2UIMessageRenderer.test.ts` + `A2UIRecoveryStates.test.ts` | partial | Recovery (13 cases), basic surface render/update/multi-surface, and action interception (4 cases) are ported. Still missing: DeferredChild/binder lifecycle, offscreen readiness/fallback, tool registration/precedence/cleanup, theme delivery, processor-error immediacy, custom-loading building-only restriction, and expanded provider matrix.                                                                                                                                                                                                                                                                                                   |
