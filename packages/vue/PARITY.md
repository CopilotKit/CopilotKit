...
| `CopilotChatMessageView` `subagent` slot                                                                                                                         | `#subagent` slot on `CopilotChatMessageView` (forwarded from `CopilotChat`)        | intentional-divergence | Slot translation, behavior parity required. The slot gets `subagentRunId`, `subagent` and `messages`, plus a `body` component in place of React `children`: `<component :is="body" />` renders the group's messages and nested groups.                                                                                                                                                                                                                                            |
...
| `CopilotChatSubagents.e2e.test.tsx`                                                                | `CopilotChatSubagents.e2e.test.ts`                | matched | Strict counterpart with React-matching case names: parallel subagent groups under their tool calls, every group collapsed by default with a user toggle that survives status changes, nesting, unannounced ids, a subagent-attributed tool result pairing with the parent card, the `#subagent` slot, and an unattributed run with no groups.                                                                                                                                                                                                                                                                                                         |
...
| `use-subagents.test.tsx`                 | `use-subagents.test.ts`                                                         | matched | Strict counterpart with React-matching case names: follows the chat thread, reads an explicit thread, and keeps the same array between renders. Returns a readonly shallow ref.                                                                                                                                                                                                                                                                                                                                                                                                                                   |
