...
| `CopilotChat.attachments.test.tsx`               | `CopilotChat.attachments.test.ts`                 | matched | Dedicated strict counterpart mirrors React `onUploadFailed` coverage (`invalid-type`, `file-too-large`, `upload-failed`, multi-reject, and valid-file no-failure), validates Vue attachment drop flow through `CopilotChat`, and now asserts stable attachment action identities across input-only rerenders. |
...
| `CopilotChatInput.test.tsx`                      | `CopilotChatInput.test.ts`                        | matched | Dedicated strict counterpart mirrors the React suite/case structure and depth, including slash-command behavior plus the trailing container-dimension cache block (resize invalidation, warm-cache keystrokes, and fallback paths).                                                                           |
