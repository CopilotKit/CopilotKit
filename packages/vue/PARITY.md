...
### A2UI Source Layout Mapping

Vue A2UI lives at `packages/vue/src/v2/a2ui/` and mirrors two React layers:

#### Core Glue Layer (`v2/a2ui/` ↔ `react-core/src/v2/a2ui/`)

| React file                      | Vue file                          | Role                                       |
| ------------------------------- | --------------------------------- | ------------------------------------------ |
| `A2UIToolCallRenderer.tsx`      | `A2UIToolCallRenderer.ts`         | Built-in `render_a2ui` progress indicator  |
| `A2UIMessageRenderer.tsx`       | `A2UIMessageRenderer.ts`          | Activity message renderer (hosts surfaces) |
| `A2UICatalogContext.tsx`        | `A2UICatalogContext.ts`           | Catalog context registration               |
| —                               | `A2UISurfaceActivityRenderer.vue` | Vue SFC renderer (no React equivalent)     |
| —                               | `operations.ts`                   | Operation types/helpers (React inlines)    |
| —                               | `types.ts`                        | `A2UITheme`, `A2UISurfaceOperationPayload` |

#### Renderer Layer (`v2/a2ui/vue-renderer/` ↔ `@copilotkit/a2ui-renderer/src/react-renderer/`)

Vue does not have a separate `@copilotkit/a2ui-vue-renderer` package — the renderer internals live
inside the Vue package since they are tightly coupled to Vue's component system.

| React file                          | Vue file                              | Role                                      |
| ----------------------------------- | ------------------------------------- | ----------------------------------------- |
| `a2ui-react/adapter.tsx`            | `vue-renderer/adapter.ts`             | Component factory (GenericBinder wrapper) |
| `a2ui-react/A2uiSurface.tsx`       | `vue-renderer/A2uiSurface.ts`        | Surface tree renderer (DeferredChild)     |
| `a2ui-react/catalog/basic/index.ts` | `vue-renderer/catalog/basic/index.ts` | Basic catalog (18 components)             |
| `a2ui-react/catalog/basic/utils.ts` | `vue-renderer/catalog/basic/utils.ts` | Style utilities                           |

#### Key Differences from React

1. **No separate renderer package** — Vue owns its renderer locally because it uses Vue render functions (`h(...)`) and Vue's reactivity system directly.
2. **`operations.ts`** — Vue extracts shared operation-parsing into a helper file; React inlines `getOperationSurfaceId` in `A2UIMessageRenderer.tsx`.
3. **`A2UISurfaceActivityRenderer.vue`** — Vue uses an SFC for the surface host; React composes this inline within `A2UIMessageRenderer.tsx` via `ReactSurfaceHost`.
...
| `A2UIMessageRenderer.test.tsx`                                                                     | `A2UIMessageRenderer.test.ts`                     | matched | Dedicated strict counterpart mirrors the React A2UI rendering matrix (surface render, same-surface in-place update via `updateComponents` text flip, multi-surface independence) using the Vue-local renderer bridge (`src/v2/a2ui/vue-renderer/`), and asserts strict default-loading parity with React's `DefaultA2UILoading` (animated dot + `"Generating UI..."` label + three shimmer bars at 80% / 60% / 40% widths with staggered `cpk-a2ui-pulse` opacity animation and inline `@keyframes`). Custom `loadingComponent` override contract stays unchanged.                                                                                      |
| `A2UIToolCallRenderer.tsx` (via provider)                                                          | `A2UIToolCallRenderer.test.ts`                    | matched | Dedicated Vue counterpart covers the built-in `render_a2ui` ownership model: auto-inclusion when A2UI enabled, survival across prop changes, suppression by user-provided prop renderer, hook override semantics, factory stability, and progress-state cleanup on completion.                                                                                                                                                                                                                                                                                                                         |
| — (no React counterpart)                                                                           | `A2uiSurface.lifecycle.test.ts`                   | vue-only | Vue-specific lifecycle invariant suite for `DeferredChild` subscription cleanup on prop changes (surface/id), adapter `GenericBinder` subscription + dispose on context change and unmount, and `ComponentContext` stability across re-renders. No React counterpart because React relies on StrictMode double-invoke and useEffect cleanup semantics to enforce these invariants; Vue must validate them directly.                                                                                                                                                                                     |
