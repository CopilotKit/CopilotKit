...
export function renderEntry(opts: RenderEntryOptions): string {
  ...
  const lines: string[] = [
    ...
    // Note: v2 chat CSS is loaded via a <link> tag from view-provider.ts
    // pointing at `dist/webview/copilotkit-v2.css` (copied at build time
    // from `@copilotkit/react-core/dist/v2/index.css`). We don't import it
    // here because the codegen entry's import path doesn't always resolve
    // from arbitrary user workspaces.
