...
test("StreamingMarkdownRenderer renders markdown blocks and citations from the AST", () => {
  const { container } = render(
    <StreamingMarkdownRenderer isComplete options={{ segmenter: false }}>
      {
...
test("StreamingMarkdownRenderer creates one span per parsed text segment", () => {
  ...
  const segments = container.querySelectorAll(
    "span.cpk-streaming-markdown-segment",
  );
...
test("StreamingMarkdownRenderer keeps unresolved citation punctuation glued", () => {
  ...
  const unresolvedCitation = container.querySelector(
    'sup[data-streaming-markdown-node="citation"]',
  );
  // An unresolved citation must not be a navigable link (it renders a plain
  // <span role="doc-noteref">, never an <a>).
  const unresolvedCitationLink = container.querySelector(
    'sup[data-streaming-markdown-node="citation"] a',
  );
  const segments = Array.from(
    container.querySelectorAll("span.cpk-streaming-markdown-segment"),
  );
  ...
  expect(unresolvedCitationLink).toBeNull();
  expect(punctuationSegment?.textContent?.startsWith("\u2060.")).toBe(true);
});
...
test("StreamingMarkdownRenderer preserves existing segment dom identity across updates", () => {
  ...
  const nextSegments = container.querySelectorAll(
    "span.cpk-streaming-markdown-segment",
  );
...
test("StreamingMarkdownRenderer keeps optimistic word-tail segment identity as it grows", () => {
  const { container, rerender } = render(
    <StreamingMarkdownRenderer options={{ segmenter: { granularity: "word" } }}>
      hello wo
    </StreamingMarkdownRenderer>,
  );
...
test("StreamingMarkdownRenderer renders a caret at the end of an open paragraph when enabled", () => {
  ...
  const paragraph = container.querySelector(
    'p[data-streaming-markdown-node="paragraph"]',
  );
...
\`\`\``}
  ...
  const inlineCode = container.querySelector(
    'code[data-streaming-markdown-node="inline-code"]',
  );
  ...
  const hardBreak = container.querySelector(
    'br[data-streaming-markdown-node="hard-break"]',
  );
  const thematicBreak = container.querySelector(
    'hr[data-streaming-markdown-node="thematic-break"]',
  );
...
test("StreamingMarkdownRenderer drops javascript: and data:text/html hrefs on links", () => {
  ...
  const anchors = Array.from(
    container.querySelectorAll('a[data-streaming-markdown-node="link"]'),
  );

  // dangerous schemes: href omitted entirely (still rendered as text, just not navigable)
  expect(anchors[0]?.hasAttribute("href")).toBe(false);
  expect(anchors[1]?.hasAttribute("href")).toBe(false);
  // safe scheme survives
  expect(anchors[2]?.getAttribute("href")).toBe("https://example.com");
});

test("StreamingMarkdownRenderer never produces a navigable javascript: anchor", () => {
  const { container } = render(
    <StreamingMarkdownRenderer isComplete>
  ...
  // Whether the dangerous scheme is parsed as an autolink/link (href dropped by
  // the sanitizer) or left as plain text (no anchor at all), no navigable
  // javascript: anchor is ever rendered.
  expect(container.querySelector('a[href^="javascript" i]')).toBeNull();
});

test("StreamingMarkdownRenderer drops data:image/svg and non-image data URLs but keeps safe image sources", () => {
  const { container } = render(
    <StreamingMarkdownRenderer isComplete>
      {
        "![a](data:image/svg+xml,<svg onload=alert(1)>)\n\n![b](data:text/html,x)\n\n![c](https://images.example.com/x.png)\n\n![d](data:image/png;base64,iVBORw0KGgo=)"
      }
    </StreamingMarkdownRenderer>,
  );

  const images = Array.from(
    container.querySelectorAll('img[data-streaming-markdown-node="image"]'),
  );

  // data:image/svg (script-capable) and data:text/html are dropped
  expect(images[0]?.hasAttribute("src")).toBe(false);
  expect(images[1]?.hasAttribute("src")).toBe(false);
  // https and non-svg data:image survive
...
test("StreamingMarkdownRenderer renders a citation with a javascript: url as a non-link superscript", () => {
  const { container } = render(
    <StreamingMarkdownRenderer isComplete>
      {"cite [^ref]\n\n[^ref]: Ref javascript:alert(1)"}
    </StreamingMarkdownRenderer>,
  );

  // unsafe citation url => no navigable link, falls back to the plain superscript label
  const citationLink = container.querySelector('sup a[role="doc-noteref"]');
...
test("StreamingMarkdownRenderer override receives and can reuse default node", () => {
  ...
  const defaultStrong = container.querySelector(
    'strong[data-streaming-markdown-node="strong"]',
  );
