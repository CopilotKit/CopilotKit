...
import type { StreamingMarkdownAstNode, StreamingMarkdownParserOptions } from './index';

function parseInChunks(
  chunks: string[],
  options: Partial<StreamingMarkdownParserOptions> = {},
) {
  let state = createStreamingMarkdownParserState({ segmenter: false, ...options });
...
function findSingle(nodes: StreamingMarkdownAstNode[], type: StreamingMarkdownAstNode['type']) {
