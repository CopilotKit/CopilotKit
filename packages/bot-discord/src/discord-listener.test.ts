...
describe("attachDiscordListener", () => {
  ...
  it("maps message attachments onto the emitted turn", () => {
    const client = fakeClient();
    const onTurn = vi.fn();
    attachDiscordListener({
      client: client as any,
    ...
    client.emit(
      ...
      message({
        ...
        content: "<@bot-1> look",
        attachments: { values: () => files[Symbol.iterator]() },
      }),
    );
    expect(onTurn).toHaveBeenCalledWith(
      expect.objectContaining({
  ...
  it("emits undefined attachments when the message carries none", () => {
    const client = fakeClient();
    const onTurn = vi.fn();
    attachDiscordListener({
      client: client as any,
    ...
    expect(onTurn.mock.calls[0]![0].attachments).toBeUndefined();
  });
