/**
 * `read_thread` — a frontend tool that hands the agent the full Discord
 ...
 * the adapter targets the current thread, so no channel/id plumbing is
 * needed.
 */
import { z } from "zod";
...
export const readThreadTool = defineBotTool({
  name: "read_thread",
  description:
    "Fetch the messages in the current Discord thread so you can summarize or " +
