 ...
 *
 * HITL is a BLOCKING FRONTEND TOOL — the handler calls
 * `await thread.awaitChoice(<ConfirmWrite .../>)`, which posts the picker and
 * BLOCKS until the user clicks Create/Cancel, then resolves to the clicked
 * button's `value` (`{ confirmed: boolean }`). The agent only performs the
 * write once this returns `{ confirmed: true }`.
 */
import { z } from "zod";
