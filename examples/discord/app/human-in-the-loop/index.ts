/**
 * App-specific human-in-the-loop components — interactive embed cards the
 * agent can render to ask the user a structured question and wait for the
 * answer.
 *
 * `confirm_write` is a JSX component (`ConfirmWrite`) used as a BLOCKING
 * FRONTEND TOOL: a tool handler calls `await thread.awaitChoice(<ConfirmWrite
 * .../>)`, which posts the picker and resolves to the clicked button's `value`.
 * Add new HITL components here and re-export them.
 */
