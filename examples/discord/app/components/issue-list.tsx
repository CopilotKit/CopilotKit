/**
 * `issue_list` — renders a set of Linear issues as a compact embed card:
 ...
 * regardless of issue count: a card-per-issue layout would be very noisy on
 * long lists. We instead inline up to `MAX` issues into a single section and
 * surface the overflow in the footer.
 *
 * The agent fetches issues from the Linear MCP server and passes the fields
 * it wants shown; the Discord formatting lives here. For a single issue (or
...
/** Render a list of Linear issues as a compact, fixed-size embed card. */
