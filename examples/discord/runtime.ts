 ...
 * and acts as an on-call / triage assistant inside Discord: it pulls and
...
if (mcpServers.length === 0) {
  console.warn(
    "[discord-runtime] No MCP servers configured. Set LINEAR_API_KEY and/or " +
...
const SYSTEM_PROMPT = [
  "You are an on-call triage assistant living in a Discord server. You help",
  ...
  "- To act on a Discord conversation (e.g. 'write this thread up'), call the",
  ...
  "posts a Discord table. If render_diagram returns an error, fix the Mermaid and",
  ...
  "",
  "Acting per-user: each turn's context names the Requesting Discord user, with",
  ...
  "",
  "FORMATTING: Discord renders standard Markdown (bold, italic, inline code,",
  "fenced code blocks, bullet lists). Discord renders NO tables — never emit a",
  "Markdown table. For tabular data, either call render_table (preferred) or",
  "put the data in a fenced code block.",
  "",
...
try {
  ...
  console.error(`[discord-runtime] invalid AGENT_MODEL "${model}":`, err);
  process.exit(1);
}
...
createServer(listener).listen(port, () => {
  ...
  console.log(
    `[discord-runtime] agent "triage" ready · MCP: ${
