...
async function curate(state: typeof StateAnnotation.State) {
  const response = await model().invoke(
    [
      new SystemMessage('Return 5 most relevant article URLs as JSON: {urls: ["url1",...]}'),
      new HumanMessage(`Topic: ${state.agentState.topic}\nArticles: ${state.agentState.searchResults}`),
    ],
  ...
  const filtered = searchResults.filter((r: any) => urls.includes(r.metadata.source));
  return {
    agentState: {
      ...state.agentState,
...
async function critique(state: typeof StateAnnotation.State) {
  const feedback = state.agentState.critique ? `Previous critique: ${state.agentState.critique}` : "";
  const response = await model().invoke([
    new SystemMessage("Review article. Return [DONE] if good, or provide brief feedback."),
...
async function revise(state: typeof StateAnnotation.State) {
  const response = await model().invoke([
    ...
    new HumanMessage(`Article: ${state.agentState.article}\nCritique: ${state.agentState.critique}`),
  ]);
  return {
    agentState: { ...state.agentState, article: response.content as string },
  };
