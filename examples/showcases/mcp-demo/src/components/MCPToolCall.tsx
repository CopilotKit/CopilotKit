...
export default function MCPToolCall({ status, name = "", args, result }: ToolCallProps) {
  ...
  return (
    <div
      ...
      style={{ maxWidth: 420 }}
    >
      <div className="p-2 flex items-center justify-between cursor-pointer group" onClick={() => setIsOpen(!isOpen)}>
        <div className="flex items-center gap-2">
          ...
          <span className="text-gray-900 text-xs truncate max-w-xs">{name || "MCP Tool Call"}</span>
        </div>
      </div>
