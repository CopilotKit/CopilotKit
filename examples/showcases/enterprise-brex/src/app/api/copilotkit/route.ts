  ...
  actions: ({ properties }) => {
    ...
    return [
      ...
      // },
      {
        ...
        description: 'Search articles to find the correct answer. If an answer was not provided, do not attempt to provide one yourself',
        parameters: [
            { name: 'query', type: 'string', description: 'The query/user issue for query result.' }
        ],
        handler: async ({ query }: { query: string }) => {
          ...
          const docs: Document[] = articles.map(({ text, number, ...article }) => new Document({ pageContent: text, id: number, metadata: article }));
