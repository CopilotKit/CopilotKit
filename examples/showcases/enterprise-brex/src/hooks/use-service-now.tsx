...
export const useServiceNow = async (currentUserEmail: string) => {
    ...
    useCopilotAction({
        ...
        renderAndWait: ({ args, handler }) => {
            ...
            const handleConfirm = async () => {
                ...
                handler?.(response.status === 200 ? 'new incident was confirmed and created by the system' : 'incident confirmed but failed to create')
            }
    ...
    useCopilotAction({
        ...
        description: 'Fetch all service requests for a user ordered by creation, desc. The default limit is last 5 service requests',
        parameters: [
            {
                name: 'limit',
                description: 'The number of results to limit to',
    ...
    useCopilotAction({
        ...
        renderAndWait: ({ args, handler }) => {
            ...
            const handleConfirm = async() => {
                ...
                handler?.(response.status === 200 ? 'service request was confirmed and created by the system' : 'service request confirmed but failed to create')
            }
    ...
    useCopilotChatSuggestions({
        instructions: 'Suggest the following: Get my incidents, Get my service requests, Report a new incident, Create a new service request',
        maxSuggestions: 4,
        minSuggestions: 4,
    })
}