...
async def delete_todos(
    ctx: RunContext[StateDeps[TodoState]], id: list[str]
) -> StateSnapshotEvent:
...
async def set_todos(
    ctx: RunContext[StateDeps[TodoState]], todos: list[TodoItem]
) -> StateSnapshotEvent:
...
async def demonstrate_error() -> str:
    ...
    raise ValueError(
        "This is a deliberate error to show error handling! The agent will gracefully recover."
    )
