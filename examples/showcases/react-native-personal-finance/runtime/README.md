# AI + Finance Tracker — CopilotKit Runtime

A minimal **Next.js (App Router, TypeScript)** service that hosts the CopilotKit
runtime the React Native "AI + Finance Tracker" app points its
...
| ----------------- | ---------------- | ---------------------------------------------------------------------------------------- |
| `/api/copilotkit` | `POST` (+ `GET`) | CopilotKit runtime (AG-UI) hosting the `default` finance assistant agent.                |
| `/api/receipt`    | `POST`           | Vision-based receipt parser → `{ merchant, amount, currency, date, suggestedCategory }`. |

## What's inside

- **`app/api/copilotkit/route.ts`** — wires `CopilotRuntime` +
  `copilotRuntimeNextJSAppRouterEndpoint` from
  [`@copilotkit/runtime`](https://www.npmjs.com/package/@copilotkit/runtime),
  registering a single agent under the id **`default`** (the id the RN client
  uses via `useAgent({ agentId: "default" })`).
- **`lib/finance-agent.ts`** — the agent itself: a v2
  [`BuiltInAgent`](https://docs.copilotkit.ai/built-in-agent/quickstart) from
  `@copilotkit/runtime/v2`, configured with a vision-capable model
  (`openai/gpt-4o` by default) and the finance-assistant system prompt. It
  speaks **AG-UI** natively — the same protocol `@copilotkit/react-native`
  (`@copilotkit/core` + `@ag-ui/client`) connects with.
...
- An **OpenAI API key** (the default model `openai/gpt-4o` is vision-capable and
  powers both the agent and `/api/receipt`). An Anthropic key works too if you
  switch the model — see below.
...
| ------------------- | ------------- | ----------------------------------------------------------------------------------------- |
| `OPENAI_API_KEY`    | yes (default) | Used by the `openai/gpt-4o` agent and the receipt parser.                                 |
| `ANTHROPIC_API_KEY` | optional      | Used if you switch to an `anthropic/claude-*` model.                                      |
| `AGENT_MODEL`       | optional      | Override the chat agent model (e.g. `openai/gpt-4o-mini`, `anthropic/claude-sonnet-4.5`). |
...
Set `RUNTIME_URL` (the value passed to `CopilotKitProvider runtimeUrl` in the RN
app's `App.tsx`) to this server's `/api/copilotkit` route:

- **iOS / Android simulator** (shares the host's network):
...
- **Physical device** (must reach your machine over the LAN — `localhost` on the
  ...
  const RUNTIME_URL = "http://<LAN-IP>:3000/api/copilotkit";
  ```
...
> The CopilotKit client appends AG-UI sub-paths to this base URL
> (`/info`, `/agent/default/run`, `/agent/default/connect`), so point
> `runtimeUrl` at the **`/api/copilotkit` base** — not at a sub-path.
...
/ the AI SDK, e.g. `openai/gpt-4o`, `openai/gpt-4o-mini`,
