...
import { useCopilotReadable, useHumanInTheLoop, useFrontendTool } from "@copilotkit/react-core";
import { CARD_COLORS, CardBrand, NewCardRequest, Transaction } from "@/app/api/v1/data";
...
export default function Page() {
  ...
  const [state, dispatch] = useReducer<React.Reducer<ChangePinState, Partial<ChangePinState>>>(
  ...
  const handleChangePinSubmit = async ({ pin, cardId }: { pin?: string; cardId?: string }) => {
  ...
  useHumanInTheLoop({
    ...
    parameters: [
      ...
      },
      {
        name: "color",
        type: "string",
        description: "The color of the card (generated by copilot, bg-blue-500 for visa, bg-red-500 for mastercard)",
        required: true,
      },
      {
        name: "pin",
      ...
      return (
        <div className="rounded-lg border bg-white p-4 shadow-sm space-y-4">
          ...
          <div className="text-sm space-y-1">
            <p>
              <span className="text-gray-500">Card:</span> {card ? `${card.type} ending in ${card.last4}` : cardId}
            </p>
            <p>
    ...
    render: ({ args, respond, status }) => {
      ...
      return (
        <div className="rounded-lg border bg-white p-4 shadow-sm space-y-4">
          ...
          <div className="text-sm space-y-1">
            <p>
              <span className="text-gray-500">Transaction:</span> {transaction?.title ?? transactionId}
            </p>
            <p>
  ...
  useFrontendTool({
    ...
    description: "Displays a list of transactions upon request. At least one parameter is required per request",
    ...
    render: ({ status, args }) => {
      ...
      if (card4Digits) {
        filteredTransactions = filterTransactionsByCardLast4(transactions, cards, card4Digits);
      } else if (policyId) {
        filteredTransactions = filterTransactionsByPolicyId(transactions, policyId);
      } else if (transactionTitle) {
        filteredTransactions = filterTransactionByTitle(transactions, transactionTitle);
      }
    ...
    parameters: [
      {
        name: "transactionId",
        type: "string",
        description: "The id of pending transaction to present to the given department admin (provided by copilot)",
        required: true,
      },
    ],
    render: ({ args, respond, status }) => {
      ...
      async function handleChangeTransactionStatus({ id, status }: { id: string; status: Transaction["status"] }) {
      ...
      return (
        <TransactionsList
          transactions={transactions.filter((t) => transactionId.includes(t.id))}
  ...
  useCopilotReadable({
    ...
    value: Object.keys(PERMISSIONS).filter(
      (key) => !PERMISSIONS[key as keyof typeof PERMISSIONS].includes(currentUser.role),
    ),
  });
  ...
  return (
    <div className="container mx-auto p-6">
      <div className="flex justify-between items-center mb-6">
        ...
        <AddCardDropdown handleAddCard={handleAddCard} currentUser={currentUser} />
      </div>
        ...
        {cards.length ? (
          cards.map((card) => (
            <CreditCardDetails
              ...
              onChangePinModalOpen={() => dispatch({ dialogOpen: true, cardId: card.id })}
            />
          ))
        ) : (
