...
export const Carousel: React.FC<CarouselProps> = ({ results, favorites = [], onSelectFruit, onToggleFavorite }) => {
  ...
  return (
    <div ref={scrollContainerRef} className="carousel-scroll-container w-full overflow-x-auto overflow-y-visible pl-8">
      <div ref={carouselContainerRef} className="overflow-visible">
        <Animate className="flex gap-4">
          {results.map((item, index) => {
            const fruit = item.fruit ?? (item as { name?: string }).name ?? "Item";
            return (
              <CarouselItem
                key={`${fruit}-${index}`}
                fruit={fruit}
                ...
                onToggleFavorite={onToggleFavorite ? () => onToggleFavorite(fruit) : undefined}
              />
            );
          })}
        </Animate>
