...
const LAYER = "[keel/forget-memories]";

/**
 * Per-request timeout. NINE buckets are swept SERIALLY on a presenter reset
 * (`memoryScopeUserIds()` derives one per persona, one per role slug, plus the
 * default), so an unbounded fetch means the Reset button spins forever against a
 * wedged backend — the one moment a presenter has no time to debug anything.
 */
...
export async function forgetAllMemories(
  ...
  while (passes < maxPasses) {
     ...
     * presenter resets Keel — deleting data this skin does not own, to solve a
     * problem it does not have.
     *
     * Keel owns nothing at project scope BY CONSTRUCTION: `seed-memories.ts`
     * writes `scope: "user"` on both rows and says why, and the prompt plus
     * `teach-mode-directives.ts`'s `SAVE_PROCEDURE_CONFIRMED` both instruct
     ...
     * swallowed: `dev/reset` reports it, so a non-zero number after a keel-only
     * session is the signal to go look.
     */
