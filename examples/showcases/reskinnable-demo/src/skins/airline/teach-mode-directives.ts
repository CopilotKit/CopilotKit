/**
 * BEAT 6 — the strings Aeronova's teach chain settles with, and the rules that
 * read them back.
 *
 ...
 *
 * React-free and server-safe on purpose: these readers are the part of those cards
 * worth a unit test, and a pure module is testable without mounting a provider, a
 * thread and an agent registration. Modelled on
 * `src/skins/logistics/teach-mode-directives.ts` and
 * `src/skins/commerce/teach-mode-directives.ts`, which `docs/teach-mode/README.md`
 * names as the reference precisely because their replay behaviour is pinned by a
 ...
 *
 * ⚠️ NOTHING HERE NAMES A FARE-EXCEPTION CATEGORY, and nothing may. This module is
 * read by the AGENT — `buildDemonstrationDirective`'s output is a tool result — so
 * it is on the withheld side of beat 6's asymmetry. The category it reports is
 * whatever the passenger actually filed, passed in at runtime; the catalogue is
 * never imported. See `data/fare-waiver-codes.ts`.
 */
 ...
 * would also miscount any step label containing a numeral, and Aeronova's labels
 * carry them constantly ("Re-attempted the reissue on AV3PL9 onto AV2216").
 */
export function buildDemonstrationDirective({
  ...
  steps: string[];
  /** The exception category the passenger actually filed, or `null` if none. */
  code: string | null;
}): string {
  const observed = steps
    ...
    .join("\n");
  return (
    `The passenger finished after ${steps.length} ${steps.length === 1 ? "step" : "steps"}. ` +
    `Observed steps:\n${observed || "(nothing captured)"}\n` +
    (code
      ? `The exception category they filed was ${code}.`
      : "No exception category was captured — ask the passenger which one they used before saving anything.")
  );
}
...
export function readDemonstratedStepCount(result: unknown): number | null {
  ...
  const match = /^The passenger finished after (\d+) steps?\./.exec(
    result.trim(),
  );
...
/**
 * Settles the save card when the passenger confirms. Names the scope and kind
 ...
 * concierge already knowing the answer: it would never decline, never offer to
...
export const SAVE_PROCEDURE_CONFIRMED =
  "The passenger confirmed. Persist this with save_memory now (scope 'user', kind 'operational'), then say in one sentence that you have it.";

/** Settles the save card when the passenger declines. Forbids the durable write. */
export const SAVE_PROCEDURE_DECLINED =
  "The passenger declined to save it. Do not call save_memory.";

/**
 * `pending` — not answered yet (render the live card).
 * `saved` — the passenger confirmed; the procedure was persisted.
 * `declined` — the passenger refused; NOTHING was written.
...
export const OFFER_ACCEPTED =
  "The passenger agreed to demonstrate. Call awaitDemonstration now and wait — do not guess any steps and do not tell them where to click.";

export const OFFER_DECLINED =
  "The passenger declined to demonstrate. Stop here and do not retry the refused reissue.";
