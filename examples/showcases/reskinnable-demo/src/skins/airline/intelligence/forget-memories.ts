...
const LAYER = "[airline/forget-memories]";

/**
 * Per-request timeout. Every bucket is swept SERIALLY on a presenter reset, so
...
export interface ForgetResult {
   ...
   * every remaining bucket, so a single bad row left every remaining bucket untouched.
   */
...
export async function forgetAllMemories(
  ...
  while (passes < maxPasses) {
     ...
     * presenter resets Aeronova — deleting data this skin does not own, to
     * solve a problem it does not have.
     *
     * Aeronova owns nothing at project scope BY CONSTRUCTION: its seed file
     ...
     * swallowed: `dev/reset` reports it, so a non-zero number after an
     * Aeronova-only session is the signal to go look.
     */
