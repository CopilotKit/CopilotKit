 ...
 *
 * PORTED from banking's equivalent rather than imported: a skin's only inbound
 * dependency is the shell's contract, so src/skins/bookstore/** must never reach
 ...
 * from banking's version for the reason below — bookstore has the same reason
 * to depart.
 *
...
export interface ForgetResult {
   ...
   * swallowed, so a non-zero value after a bookstore-only session is the
   * signal to go investigate.
   */
...
export async function forgetAllMemories(
   ...
   * every time a presenter resets bookstore — deleting data this skin does
   * not own, to solve a problem it does not have.
   *
   * Bookstore owns nothing at project scope by construction: its seed file
   * writes only user scope. So skipping project rows still leaves the
   * invariant that matters intact (learned memory cleared, seeds re-armed).
   * The skipped count is RETURNED rather than swallowed, so a non-zero value
   * after a bookstore-only session is the signal to investigate.
   */
