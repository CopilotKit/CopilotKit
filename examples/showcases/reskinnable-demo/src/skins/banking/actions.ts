...
// several components (the dashboard page, the banking Tools component where the
// chat's approveTransaction/finalize tools live, page.tsx, …). A mutation made
// through one instance therefore would NOT refresh another — e.g. the agent
// approving a charge in chat (the Tools component's instance) left the dashboard's pending
...
export default function useCreditCards() {
  ...
  const changePin = async ({
    ...
    try {
      const response = await fetch(`/api/banking/v1/cards/${cardId}`, {
        method: "PUT",
        headers: {
  ...
  const fetchCards = async () => {
    try {
      const response = await fetch("/api/banking/v1/cards");
      if (!response.ok) {
  ...
  const fetchPolicies = async () => {
    try {
      const response = await fetch("/api/banking/v1/policies");
      if (!response.ok) {
        throw new Error("Network response was not ok");
      }
      const data = await response.json();
  ...
  const fetchTransactions = async () => {
    try {
      const response = await fetch("/api/banking/v1/transactions");
      if (!response.ok) {
        throw new Error("Network response was not ok");
      }
      const data = await response.json();
  ...
  const assignPolicyToCard = async ({
    ...
    try {
      const response = await fetch(`/api/banking/v1/cards/${cardId}/policy`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const addNoteToTransaction = async ({
    ...
    try {
      const response = await fetch(
        `/api/banking/v1/transactions/${transactionId}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const changeTransactionStatus = async ({
    ...
    try {
      const response = await fetch(`/api/banking/v1/transactions/${id}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const openPolicyException = async ({
    ...
    try {
      const response = await fetch("/api/banking/v1/exceptions", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const finalizePolicyException = async ({
    ...
    try {
      const response = await fetch(
        `/api/banking/v1/exceptions/${exceptionId}/finalize`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
