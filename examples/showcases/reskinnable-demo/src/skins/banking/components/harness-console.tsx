      ...
      {open ? (
        ...
        >
          {lines.length === 0 ? (
            ...
            lines.map((line) => {
              // Tailwind cannot reliably discover interpolated `pl-${...}`
              // classes. Keep the existing 0.75rem-per-level rhythm as an
              // explicit style so arbitrary nesting remains visible.
              const indent =
                line.depth > 0
                  ? { paddingLeft: `${line.depth * 0.75}rem` }
                  : undefined;
              if (line.kind === "started") {
                return (
                  <div key={line.key} style={indent} className="py-0.5 text-brand">
                    ┌ {line.text}
                  </div>
                );
              }
              ...
              if (line.kind === "tool") {
                return (
                  <div key={line.key} style={indent} className="py-0.5 text-ink">
                    {line.text}
                  </div>
                );
              }
