...
// the client skin module — it pulls in @copilotkit/runtime, which must never
// reach the browser bundle. Keyed by the same id as the skin: "banking".
...
const BANKING_PROMPT = `You are the Northwind Copilot, an assistant embedded in a corporate
 ...
 * (mirroring airline's `airlineAgent`) so the runtime route and a future
 * per-skin agent map can key it the same way. Keyed by the same id as the skin:
 * "banking".
 */
export const bankingAgent = () =>
  new BuiltInAgent({
