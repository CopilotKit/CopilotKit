...
} from "@/skins/airline/intelligence/user-id";
...
/**
 * Presenter/booth reset — put Aeronova back to the state the demo starts from.
 *
 * It used to reset the STORE ONLY and say so in a `memoryBeats: "unarmed"` field,
 * because this skin had no seed-memories module. That field is gone in the same
 * change that added one (`data/beat-map.md` § "What this slot did NOT build",
 * trap 3, asked for exactly that ordering) — the button now genuinely re-arms the
 * memory beats, and a response still claiming otherwise would be the same lie in
 * the opposite direction.
 *
 * Four things have to be true afterwards, and all four are easy to get wrong:
 *   1. The trip record is re-seeded — beat 5's reissue, seat and 🚨 notice all
 *      live ON the booking, beat 6's filed exceptions and the `activeExceptionId`
 *      links go with them, and beat 3d's trip briefs come off the record. That
 *      last one matters as much as the rest: a reset that left last run's brief
 *      on the trip would open the demo with an artifact whose document was never
 *      ingested in front of THIS audience.
...
export const POST = async () => {
  ...
  // to sibling demos sharing this backend, not to Aeronova. Reported rather
  // than hidden: a non-zero count after an Aeronova-only session means
  ...
  // hardcoded list here missed it entirely — the reset scrubbed `aeronova-*`
  ...
  console.warn(
    `[airline] presenter reset: forgetting memories at ${apiUrl} for ${userIds.join(", ")}`,
  );
  ...
  try {
    ...
    if (memory !== "seeded") {
      ...
      // sidebar Reset button in src/skins/airline/layout.tsx, branches solely on
      ...
      console.error(
        `[airline] presenter reset: seeded ${seeded}/${expectedSeeds} memories (${memory})` +
          (wipeIncomplete
    ...
    console.error(`[airline] presenter reset: ${detail}`);
    return Response.json(
      {
        ok: false,
        reset: ["store"],
