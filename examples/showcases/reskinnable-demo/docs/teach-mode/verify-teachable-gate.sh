...
#
# USAGE
#   ./verify-teachable-gate.sh                       # against http://localhost:3000
...
#     `next dev` defaults to :3000; this script defaults to :3000 to match the
...
#     in-memory and seeded from `src/skins/banking/data/seed.json`; restart the dev server to
#     reset. Each scenario below uses a DIFFERENT seeded transaction so a single
#     run does not need a reset between scenarios.
#
# SEED FACTS THIS SCRIPT RELIES ON (src/skins/banking/data/seed.json):
...
#
# JUSTIFYING vs DECOY codes (src/skins/banking/data/policy-exception-codes.ts):
...
# Route: PUT /api/banking/v1/transactions/[id]  (src/app/api/banking/v1/transactions/[id]/route.ts)
# ===========================================================================
...
#   open  → POST /api/banking/v1/exceptions            (src/app/api/banking/v1/exceptions/route.ts)
#   final → POST /api/banking/v1/exceptions/[id]/finalize
#           (src/app/api/banking/v1/exceptions/[id]/finalize/route.ts)
