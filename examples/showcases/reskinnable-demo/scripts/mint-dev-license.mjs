...
const tmpSigner = resolve(
  repoTmpDir,
  `_mint-reskinnable-license.${process.pid}.ts`,
);
...
const payload = createLicensePayload(
  { organizationId: ${JSON.stringify(ORG_ID)}, organizationName: 'reskinnable-demo', contactEmail: 'demo@northwind.example',
    tier: 'enterprise', planCode: 'enterprise', entitlementSource: 'enterprise_override', issuer: 'reskinnable-demo-local',
    seatLimit: 0, features: { ...getDefaultFeatures('enterprise'), memory: true }, removeBranding: true,
    expiresAt: new Date('2099-01-01T00:00:00Z'), telemetryId: 'reskinnable-demo-local' },
  keyId, 'lic_reskinnable_demo_local',
);
