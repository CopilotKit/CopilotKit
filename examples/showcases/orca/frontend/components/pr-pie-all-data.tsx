...
export function PRPieData({ args }: any) {
  ...
  useEffect(() => {
    ...
    const pieData = Object.entries(getStatusCounts(prData.filter((pr: PRData) => pr.userId === args?.userId))).map(
      ([status, count]) => ({
        name: status,
        value: count,
      }),
...
export const CustomPieTooltip = ({ active, payload }: any) => {
  if (active && payload && payload.length) {
    ...
    return <div className="bg-white p-2 rounded shadow text-black">{name.split("_").join(" ")}</div>;
  }
  return null;
};
