...
import { DollarSign, TrendingUp, FileText, Receipt, ArrowUpRight, ArrowDownRight } from "lucide-react";
...
export function MetricCards({ config }: { config: MetricCardsWidget["config"] }) {
  ...
  return (
    <div
      className={cn(
        "grid gap-4 h-full",
        config.stacked ? "grid-cols-1" : filtered.length === 1 ? "grid-cols-1" : "grid-cols-1 sm:grid-cols-2",
      )}
    >
      {filtered.map((kpi) => {
        ...
        return (
          <Card key={kpi.label} className="flex flex-col justify-center" size="sm">
            <CardContent>
