  ...
  for (const key in recipe) {
    if (
      ...
      if (typeof agentValue === "string") {
        agentValue = agentValue.replace(/^'(.*)'$/, "$1").replace(/\\n/g, "\n");
      }

      // Handle arrays of objects (like ingredients)
      if (Array.isArray(agentValue)) {
        agentValue = agentValue.map((item) => {
          if (typeof item === "object" && item !== null) {
            const cleanItem = { ...item };
            Object.keys(cleanItem).forEach((itemKey) => {
              if (typeof cleanItem[itemKey] === "string") {
                cleanItem[itemKey] = cleanItem[itemKey].replace(
                  /^'(.*)'$/,
                  "$1"
                );
              }
