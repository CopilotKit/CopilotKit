...
function Haiku() {
  ...
  const validateAndCorrectImageNames = (
    ...
    if (correctedNames.length < 3) {
      const availableFallbacks = VALID_IMAGE_NAMES.filter(
        (name) => !usedValidNames.has(name)
      );
    ...
    while (correctedNames.length < 3 && VALID_IMAGE_NAMES.length > 0) {
      const fallbackName =
        VALID_IMAGE_NAMES[Math.floor(Math.random() * VALID_IMAGE_NAMES.length)];
      correctedNames.push(fallbackName);
    }
            ...
            .map((haiku, index) => (
              ...
              >
                {haiku.japanese.map((line, lineIndex) => (
                  ...
                  >
                    <p className="text-4xl font-bold text-gray-600 w-auto">
                      {line}
                    </p>
