...
export function buildAgents(): Record<string, HttpAgent> {
  ...
  for (const [name, url] of Object.entries(agentUrls)) {
    if (url) {
      ...
      agent.use(new MCPAppsMiddleware({ mcpServers: [{ type: "http", url: mcpServerUrl, serverId: "example_mcp_app" }] }))
      agents[name] = agent
    }
  }

  if (Object.keys(agents).length === 0) {
    agents.default = new HttpAgent({ url: requireEnv("AGENTCORE_AG_UI_URL"), headers: {} })
  }
...
export function buildApp() {
  ...
  if (!defaultAgent) throw new Error("At least one CopilotKit agent URL must be configured")
