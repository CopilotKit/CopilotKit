...
CopilotChat is built on a composable component hierarchy. Understanding this structure helps you customize exactly what
you need.
...
When you provide a `threadId`, CopilotChat automatically loads the conversation history for that thread, including any
messages that are currently streaming. This enables seamless continuation of conversations across page reloads or
component remounts.
...
CopilotChat uses a powerful [slot system](/reference/slot-system) that allows you to customize any part of the UI. Each
slot accepts four types of values:
...
For full details on assistant message slots and customization options, see
[CopilotChatAssistantMessage](/reference/copilot-chat-assistant-message).
...
CopilotChat automatically manages suggestion chips through the `useSuggestions` hook. Suggestions are generated by the
agent and displayed as clickable chips that users can select to quickly send messages.
...
CopilotChat supports voice input through transcription. To enable it, configure your CopilotKitProvider with
...
Once enabled, a microphone button appears in the input toolbar. Users can record audio which is transcribed and inserted
