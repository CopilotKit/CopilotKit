...
`CopilotChatAssistantMessage` is the default component used by
[CopilotChatMessageView](/reference/copilot-chat-message-view) to render AI assistant responses. It handles markdown
rendering, feedback collection, and tool call display.
...
Note: `thumbsUpButton` and `thumbsDownButton` only show when their respective callbacks (`onThumbsUp`, `onThumbsDown`)
are provided.
