  ...
  return (
    <div className="flex gap-3 items-start justify-end">
      <div className="flex-1">
        <CopilotChatUserMessage message={message} className="bg-blue-600 text-white" {...props} />
      </div>
...
function ChatWithEdit() {
  ...
  return (
    <>
      ...
      {editingMessage && <EditMessageModal message={editingMessage} onClose={() => setEditingMessage(null)} />}
    </>
  );
}
```
