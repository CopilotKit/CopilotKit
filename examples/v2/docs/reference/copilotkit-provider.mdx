...
<CopilotKitProvider toolCallRenderers={toolCallRenderers}>{children}</CopilotKitProvider>;
```

<Note>
  The `toolCallRenderers` array must be stable across renders. Define it outside your component or use `useMemo`. For
  dynamic tool rendering, use the `useToolCallRenderer` hook instead.
</Note>
