...
const PopupChat = defineComponent({
  ...
  setup() {
    ...
    return () =>
      h(CopilotPopup, { defaultOpen: true, clickOutsideToClose: true });
  },
});
</script>
...
<template>
  <CopilotKitProvider runtime-url="/api/copilotkit" show-dev-console="auto">
    ...
    >
      <main
        class="mx-auto flex w-full max-w-5xl flex-col gap-10 px-6 py-12 pb-40"
      >
        <section class="space-y-4">
          <span
            class="inline-flex items-center rounded-full bg-slate-900/10 px-3 py-1 text-xs font-medium text-slate-700"
          >
            Overlay Chat Demo
          </span>
        ...
        <section
          class="rounded-2xl border border-dashed border-slate-300 bg-white/70 p-6 shadow-inner"
        >
