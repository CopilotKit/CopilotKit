...
The agent has no order-specific tool. `autopilot_readPage` and `autopilot_findControls` return short-lived references to visible controls. `autopilot_navigate` accepts only a discovered link reference, and `autopilot_activateControl` accepts only a discovered button reference. The package-level `BrowserPageMap`, `BrowserNavigator`, and `BrowserControlActivator` resolve and recheck these references; no tool maps an order label or route to business logic. The app's ordinary `CancelOrder` handler opens its existing confirmation, checks the local approval binding, then calls the same role-, tenant-, version-, and operation-key-checked server route used for manual cancellation. The agent cannot accept that dialog.
