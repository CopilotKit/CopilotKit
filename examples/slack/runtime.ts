...
const SYSTEM_PROMPT = [
  ...
  "read — images directly, and CSV/JSON/text as decoded text. When a user",
  "uploads data and wants a chart, parse it and call render_chart with a",
  "Chart.js config (as a JSON string) — pick a sensible type (bar/line/pie) and",
  "inline the data. When asked to diagram a flow/architecture/timeline, call",
  "render_diagram with Mermaid source. Both render to an image posted in the",
  "thread; if render_diagram returns an error, fix the Mermaid and retry. These",
  "are read/reply actions — no confirm_write needed.",
