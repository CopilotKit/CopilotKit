    ...
    <div className="flex-1 p-8 overflow-hidden">
      <div className="flex gap-8 h-full overflow-x-auto overflow-y-hidden">
        {statuses.map((status) => (
          <div
            key={status}
            className="bg-white/10 rounded-2xl p-6 border border-white/10 flex flex-col h-full min-w-[350px] w-[350px] flex-shrink-0"
          >
            ...
            <div className="flex-1 space-y-4 overflow-y-auto">
              {state?.tasks
                ...
                .map((task) => {
                  ...
                  return <TaskCard key={task.id} task={task} assignedUser={assignedUser} />;
                })}
              {state?.tasks?.filter((task) => task.status === status).length === 0 && (
                <div className="text-center py-8 text-gray-300 text-sm">No tasks in this column</div>
              )}
            </div>
          </div>
        ))}
...
function TaskCard({ task, assignedUser }: TaskCardProps) {
  return (
    <div className="bg-white/10 rounded-xl p-4 border border-white/10 hover:bg-white/20 transition-all duration-200 hover:shadow-lg cursor-pointer group">
      <div className="font-semibold text-white mb-2 group-hover:text-white/90 transition-colors">{task.name}</div>
      <div className="text-sm text-gray-200 mb-4 leading-relaxed">{task.description}</div>
      {assignedUser && (
