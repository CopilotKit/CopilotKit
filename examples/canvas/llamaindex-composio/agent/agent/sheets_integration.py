...
def convert_sheet_to_canvas_items(
    sheet_data: Dict[str, Any], original_sheet_id: str = ""
) -> Dict[str, Any]:
    """
    ...
    if not sheet_data or not sheet_data.get("rows"):
        return {
            ...
            "syncSheetId": original_sheet_id
            or sheet_data.get("spreadsheet_info", {}).get("spreadsheet_id", "")
            if sheet_data
            else "",
    ...
    sync_sheet_id = original_sheet_id or sheet_data.get("spreadsheet_info", {}).get(
        "spreadsheet_id", ""
    )
...
def determine_item_type(row: List[str], headers: List[str]) -> str:
    ...
    numeric_count = sum(
        1 for cell in row if cell and cell.replace(".", "").replace("-", "").isdigit()
    )
    if numeric_count >= 2:
        return "chart"
...
def sync_canvas_to_sheet(
    sheet_id: str, canvas_state: Dict[str, Any], sheet_name: Optional[str] = None
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if not target_sheet_name:
            ...
            if not sheet_names:
                return {
                    "success": False,
                    "error": "Failed to get sheet names from spreadsheet",
                }
        ...
        if current_row_count > new_row_count:
            ...
            print(
                f"Deleting {rows_to_delete} rows from sheet (current: {current_row_count}, new: {new_row_count})"
            )
        ...
        return {"success": False, "error": f"Exception during sync: {str(e)}"}
...
def create_new_sheet(title: str = "Canvas Data") -> Dict[str, Any]:
    ...
    try:
        ...
        return {"success": False, "error": f"Exception during sheet creation: {str(e)}"}
