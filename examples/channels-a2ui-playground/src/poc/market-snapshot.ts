...
import type { ClickHandler, Renderable } from "@copilotkit/channels-ui";
import { createChannelA2UICatalog } from "./catalog.js";
import { previewActionId } from "./preview-action-id.js";
import { z } from "zod";
...
export function createMarketSnapshotCatalog() {
  ...
  return createChannelA2UICatalog(
    ...
    {
      MarketSnapshot: ({ props, componentId, surfaceId, dispatch }) => {
        const onAcknowledge: ClickHandler = (interaction) =>
          dispatch(
            { event: { name: "acknowledge_search_result" } },
            interaction,
          );
        (onAcknowledge as { id?: string }).id = previewActionId(
          surfaceId,
          componentId,
          "acknowledge_search_result",
        );
        return renderMarketSnapshot(props, onAcknowledge);
      },
    },
    {
