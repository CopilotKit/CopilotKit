...
import type { ChannelNode, Renderable } from "@copilotkit/channels-ui";
import {
  BASIC_FUNCTIONS,
  ButtonApi,
  CardApi,
...
const Button = defineChannelA2UIComponent(
  ...
  ({ props, children, componentId, surfaceId, rawProps, dispatch }) => {
    ...
    const onClick: ClickHandler = (interaction) =>
      dispatch(rawProps.action, interaction);
    (onClick as { id?: string }).id = previewActionId(
      surfaceId,
      componentId,
      actionName(rawProps),
    );
    ...
    return {
      ...node,
      key: previewActionId(surfaceId, componentId, actionName(rawProps)),
    };
  },
);
