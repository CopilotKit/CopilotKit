...
async def test_wait_until_ready_success_timeout_and_immediate_rejections(
    ...
    registry = createSkillRegistryMiddleware(client(skills), CONTAINER_ID)
    ...
    cold = createSkillRegistryMiddleware(client(FakeSkillsClient()), CONTAINER_ID)
    ...
    denied = createSkillRegistryMiddleware(client(denied_skills), CONTAINER_ID)
    ...
    clock = FakeClock()
    stale = createSkillRegistryMiddleware(
        client(stale_skills), CONTAINER_ID, clock=clock
    )
    await stale.load()
    clock.seconds = 30
...
@pytest.mark.asyncio
async def test_join_event_sink_failure_is_shared_by_every_caller_without_leaks(
    tmp_path,
) -> None:
    from copilotkit_intelligence_langgraph import createSkillRegistryMiddleware
...
async def test_close_suppresses_inflight_terminal_state_and_telemetry(
    ...
    assert registry.status == "closed"
    assert events == [("load.started", None), ("status.changed", "closed")]
...
async def test_fresh_cached_revoked_and_empty_snapshots(tmp_path) -> None:
    ...
    assert cached.rendered_skills[0].text == "# Skill\n"
    with pytest.raises(FrozenInstanceError):
        cached.status = "stale"

    revoked = await registry.preload()
    assert revoked.status == "revoked"
    assert revoked.rendered_skills == ()
    assert registry.ready

    empty = await registry.preload()
    assert empty.status == "ready"
    assert empty.rendered_skills == ()
...
async def test_denial_clears_active_snapshot(tmp_path) -> None:
    ...
    assert (await registry.load()).rendered_skills
    with pytest.raises(IntelligenceAccessDeniedError):
        await registry.load()
    assert registry.status == "denied"
...
async def test_invalid_utf8_and_legacy_projection_fail_closed(tmp_path) -> None:
    ...
    registry = createSkillRegistryMiddleware(client(invalid_skills), CONTAINER_ID)
    ...
    registry = createSkillRegistryMiddleware(client(legacy_skills), CONTAINER_ID)
