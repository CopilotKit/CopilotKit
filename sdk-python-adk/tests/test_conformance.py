...
def _seed_initial_snapshot(registry, case: dict[str, Any], tmp_path: Path) -> None:
    from copilotkit_intelligence_adk._snapshot import render_verified_skills
    from copilotkit_intelligence_adk.registry import RegistrySnapshot
    ...
    if initial["refreshDue"]:
        attempted = -30000
    registry._snapshot = RegistrySnapshot(
...
async def test_adapter_conformance(case: dict[str, Any], tmp_path: Path) -> None:
    ...
    registry = SkillRegistry(client(skills), CONTAINER_ID, clock=clock, telemetry=emit)
    ...
    if "result" in generic and "freshness" in generic["result"]:
        ...
        assert len(registry.snapshot.skills) == result["skillCount"]
        assert (
            sum(record.byte_length for record in registry.snapshot.skills)
    ...
    actual_records = [record.as_native() for record in registry.snapshot.skills]
    ...
    native_registry = _SnapshotRegistry(registry.snapshot, native_error)
    if expected["nativeHook"]["proceed"]:
        tools = await SkillToolset(native_registry).get_tools()
        native_records = [
